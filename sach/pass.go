...
type Report struct {
	...
	Removed map[Stage]map[vo.Reason]int64 `json:"removed,omitempty"`
	...
	Phoi phoi.Tally `json:"phoi"`
	Sang sang.Tally `json:"sang"`
	Che  che.Tally  `json:"che"`
...
func (p *Pass) Run(ctx context.Context, parts []kho.Stored, note Progress) (Report, error) {
	if p.Workers < 1 {
		p.Workers = 1
	}
	...
	work := make(chan kho.Stored)

	var wg sync.WaitGroup
	for range p.Workers {
		wg.Go(func() {
			line := &Line{Limits: sang.Default(), Level: che.L1, Seen: seen}
			for part := range work {
...
func (p *Pass) held(ctx context.Context) (map[string]bool, error) {
	files, err := p.To.List(ctx, kho.DataDir)
	if err != nil {
		return nil, err
	}
...
type tally struct {
	removed map[Stage]map[vo.Reason]int64
	phoi    phoi.Tally
	sang    sang.Tally
	che     che.Tally
	textIn  int64
...
func (p *Pass) part(ctx context.Context, line *Line, from kho.Stored, done bool) (Cleaned, tally, error) {
	...
	out, err := kho.CreatePart(p.Dir, from.Path, p.Clean, p.stamp(from))
	if err != nil {
		return c, t, err
	}

	t.removed = map[Stage]map[vo.Reason]int64{}
	err = kho.ScanRows(from.Path, src, func(row kho.Row) error {
		d := kho.DocumentOf(row)
		t.textIn += int64(len(d.Text))
		v := line.Run(d)

		t.phoi.Add(v.Normalized)
		if v.Stage != StageNormalize {
			t.sang.Add(line.Limits, v.Measured)
		}
		if v.Kept {
			t.che.Add(line.Level, v.Found)
		...
		if t.removed[v.Stage] == nil {
			t.removed[v.Stage] = map[vo.Reason]int64{}
		}
...
func (p *Pass) stamp(from kho.Stored) kho.Stamp {
	snapshot, _, _, _ := kho.ParseStagePath(from.Path)
	return kho.Stamp{Snapshot: snapshot, Stage: "gao-sach@" + PipelineVersion, Box: p.Box}
}
...
func (r *Report) fold(c Cleaned, t tally) {
	...
	if r.Removed == nil {
		r.Removed = map[Stage]map[vo.Reason]int64{}
	}
	for stage, reasons := range t.removed {
		if r.Removed[stage] == nil {
			r.Removed[stage] = map[vo.Reason]int64{}
		}
		for reason, n := range reasons {
			r.Removed[stage][reason] += n
		}
	}
	addPhoi(&r.Phoi, t.phoi)
	addSang(&r.Sang, t.sang)
	addChe(&r.Che, t.che)
}

func addPhoi(into *phoi.Tally, from phoi.Tally) {
...
func addSang(into *sang.Tally, from sang.Tally) {
	into.Documents += from.Documents
	into.Kept += from.Kept
	into.Syllables += from.Syllables
	for k, n := range from.Rejected {
		if into.Rejected == nil {
			into.Rejected = map[vo.Reason]int64{}
		}
		into.Rejected[k] += n
	}
...
func addChe(into *che.Tally, from che.Tally) {
	into.Documents += from.Documents
	...
	for k, n := range from.ByKind {
		if into.ByKind == nil {
			into.ByKind = map[che.Kind]int64{}
		}
		into.ByKind[k] += n
	}
}
