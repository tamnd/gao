...
// boxes is every box that may hold corpus bytes, which is three of the four as
...
func TestTheSplitHandsOutEveryFileTheManifestPins(t *testing.T) {
	...
	if s.Files != harvest.Files() {
		t.Errorf("the split covers %d files, the manifest pins %d", s.Files, harvest.Files())
	}
	if s.Bytes != harvest.TotalBytes() {
		t.Errorf("the split covers %d bytes, the manifest pins %d", s.Bytes, harvest.TotalBytes())
	}
...
func TestTheWholeIngestIsTheGroupsEndToEnd(t *testing.T) {
	...
	if s.Alone() <= s.Seconds() {
		t.Errorf("three boxes take %.0f seconds and the fastest alone takes %.0f, so the boxes bought nothing", s.Seconds(), s.Alone())
	}
}
...
func TestABoxThatMayNotHoldCorpusBytesDrawsNothing(t *testing.T) {
	s := Divide(append(boxes(), reading("server2", 50)))
...
func TestAGroupWithTooFewFilesToDivideSaysSoWithoutCallingItAFault(t *testing.T) {
	...
	if len(s.Blocking()) > 0 {
		t.Fatalf("the boxes is refused a schedule: %q", s.Blocking())
	}
	waiting := s.Waiting()
	if len(waiting) == 0 {
...
func TestJobsAreHandedOutHeaviestFirstWithinAnOrder(t *testing.T) {
	jobs := Jobs()
	if len(jobs) != harvest.Files() {
		t.Fatalf("Jobs covers %d files, the manifest pins %d", len(jobs), harvest.Files())
	}
