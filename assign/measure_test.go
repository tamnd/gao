...
// cares about.
func entry(box string, bytes int64, finished string) harvest.Entry {
...
func TestMeasureDoesNotCareWhatOrderTheLedgerIsIn(t *testing.T) {
	forward, err := Measure([]harvest.Entry{
	...
	backward, err := Measure([]harvest.Entry{
		entry("server1", 4_300_000_000, "2026-08-18T12:00:00Z"),
		entry("server1", 4_300_000_000, "2026-08-18T11:00:00Z"),
	})
	if err != nil {
...
func TestMeasureRefusals(t *testing.T) {
	cases := []struct {
		...
		{
			"one file", []harvest.Entry{entry("server3", 9_000_000_000, "2026-08-18T06:19:00Z")},
			"one finish time is not a duration",
		},
		{
			"two boxes", []harvest.Entry{
				entry("server1", 4_300_000_000, "2026-08-18T11:00:00Z"),
		...
		{
			"an unlabelled box", []harvest.Entry{
		...
		{
			"too little moved", []harvest.Entry{
		...
		{
			"a finish that is not a time", []harvest.Entry{
				entry("server3", 2_100_000_000, "2026-08-18T06:19:00Z"),
