...
func TestLineDropsTheSecondCopy(t *testing.T) {
	...
	if v.Stage != StageMill || v.Reason != reject.ReasonDuplicate {
		t.Errorf("the copy was removed at %s for %s, want %s for %s", v.Stage, v.Reason, StageMill, reject.ReasonDuplicate)
	}
...
func TestLineWithoutASetKeepsEveryCopy(t *testing.T) {
	...
	l := &Line{Limits: New(1).Limits, Level: cover.L1}
	for i := range 3 {
		if v := l.Run(raw(article)); !v.Kept {
...
func TestLineCoversPersonalDataAndDropsTheSpans(t *testing.T) {
	...
	if d.PIILevel != doc.RedactionLevel(cover.L1) {
		t.Errorf("pii_level is %q, want %q", d.PIILevel, doc.RedactionLevel(cover.L1))
	}
}
