...
// closed set for the same reason [reject.Reason] is: the report breaks removals
...
func (l *Line) Run(d *doc.Document) Verdict {
	...
	v.Normalized = normalize.Normalize(d.Text)
	d.Text = v.Normalized.Text
	if reason, bad := normalize.Reject(v.Normalized); bad {
		v.Stage, v.Reason = StageNormalize, reason
		return v
	}

	v.Measured = sift.Measure(d.Text)
	...
	if l.Seen != nil && !l.Seen.Add(d.DupCluster) {
		v.Stage, v.Reason = StageMill, reject.ReasonDuplicate
		return v
	}
	d.IsRepresentative = true

	text, found := cover.Redact(d.Text, l.Level)
	d.Text = text
	v.Found = found
	...
	if err := d.Admit(); err != nil {
		v.Stage, v.Reason = StageContract, reject.ReasonContract
		return v
	}
	v.Kept = true
	return v
...
func (l *Line) measure(d *doc.Document, r sift.Result) {
...
//
// It is the first sixteen bytes of blake3 of [mill.Key], which is the text with
...
func Cluster(text string) doc.Cluster {
	sum := doc.SumString(mill.Key(text))
	var c doc.Cluster
	copy(c[:], sum[:len(c)])
	return c
}
