...
That one moved `pipeline_version` too, to 0.4.0, and for a different reason than the others. No column changed meaning. A frontier refusal never becomes a row, so the only trace of the old behavior is which hosts are in the corpus at all, and somebody counting what share of this crawl is Vietnamese gets a different answer either side of the boundary.
...
Half an hour of that says it is the larger of the two. Hosts outside `.vn` discovered in the window fell from 8,234 to 1,728, requests to `.vn` hosts went from 13,225 to 17,387 in a window of the same length, and net yield went from 0.057 to 0.119. Taken together with the reach rule the crawl went from 0.028 to 0.119 against a gate of 0.15, on the same seeds and the same boxes, entirely by declining to ask for pages it had no reason to want.

It is not at the gate yet and the remaining distance is the same shape as the distance already covered. 16,389 requests in that window still went to 1,728 hosts outside `.vn` that kept nothing, which is 46 percent of the crawl, and those hosts are reached from Vietnamese pages that link outwards for ordinary reasons. The next measurement worth taking is what those links are, because a Vietnamese newspaper linking to Facebook and a Vietnamese newspaper linking to a foreign wire service are not the same case and the second one is worth following.

The seventh defect is the one that stops a shard rather than the one that wastes it, and it was found by looking at why `server3` was doing 0.8 pages a second while the other two held three and a half. Between two and a half hours and four and a half hours into the run it fetched one page. It had no connections open and it was using no CPU, and all twenty of its workers were inside the politeness scheduler on the same host: nineteen queued for the one request that host is allowed at a time, and the twentieth asleep on a gap twenty seven minutes wide that a `503` with a `Retry-After` had opened.

Every rule in that scheduler was doing what it was written to do. One request in flight per host, a gap between requests, and a server that says it is busy gets left alone for the time it named. What was missing is that all three were being paid for by a worker rather than by the schedule, and a worker holding still for a host is a worker not fetching any of the million URLs queued behind it. A host that already has a request in flight, or whose next gap is further out than thirty seconds, now hands the URL straight back to the queue. The host is not treated any faster: the gap it was owed is left exactly where it was, and a worker that gave up does not spend it, which matters because twenty workers each moving a busy host's next slot out would push it into next week between them.
