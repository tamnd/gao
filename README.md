...
The five reconnects a file gets are consecutive, and that distinction cost two runs to learn. Counted over the life of a file the budget is spent by a link that drops once every few gigabytes, since a reconnect that worked gave nothing back, and `gamingpc` lost the same 26.4 GB HPLT shard twice that way, the second time at byte 25,137,168,622 with 95% of it already through. A reconnect followed by real bytes now returns the budget, so a transfer that keeps moving keeps going while a host that answers the range request and then sends nothing still stops after five. There is a ceiling of a hundred reconnects for each retry in the budget, which nothing real comes close to and which exists only so that a host handing out a few bytes per connection cannot hold a fetch open forever.
