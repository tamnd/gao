...
//
// It wraps a [kho.Writer] with two things the corpus store does not do: it
...
// afford.
func NewWriter(w io.Writer, sample float64, opts ...kho.WriterOption) (*Writer, error) {
	...
	// and it still runs.
	seg, err := kho.NewWriter[*Reject](w, append([]kho.WriterOption{kho.Unvalidated()}, opts...)...)
	if err != nil {
		return nil, err
	}
...
func (w *Writer) Index() kho.Index { return w.seg.Index() }

// Segment is a reject store segment opened for random access.
type Segment = kho.Segment[*Reject, Reject]

// Open opens a reject store segment.
func Open(path string) (*Segment, error) { return kho.Open[*Reject](path) }

// Reader reads a reject store segment in order.
type Reader = kho.Reader[*Reject, Reject]

// NewReader returns a Reader over the zstd stream in r.
func NewReader(r io.Reader) (*Reader, error) { return kho.NewReader[*Reject](r) }
