// Package chot fixes the evaluation harness before any result exists.
//
...
func (h Harness) Against(r nhat.Roster) []string {
	var faults []string
...
// nameable.
func (h Harness) Unpinned(r nhat.Roster) []string {
	...
	var out []string
	for _, t := range h.Tasks {
		if v, ok := version[t.Benchmark]; ok && v == nhat.Unpinned {
