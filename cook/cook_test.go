...
func TestThePlanHoldsTogether(t *testing.T) {
	for _, p := range cook.Check() {
		t.Error(p)
	}
	for _, p := range cook.CheckArms() {
		t.Error(p)
	}
}
...
func TestTheQualityTiersAreNotCountedAsNewText(t *testing.T) {
	...
	for _, c := range cook.Budget() {
		if c.Kind == cook.Natural {
			all += c.Unique
		}
	}
	if cook.NaturalUnique() >= all {
		t.Fatalf("natural Vietnamese counts %d unique tokens, which is every line added up, so the extra pass lines are being asked for twice", cook.NaturalUnique())
	}
	if want := int64(309_000_000_000); cook.NaturalUnique() != want {
		t.Errorf("the corpus target is %d unique natural tokens, and the plan is built on %d", cook.NaturalUnique(), want)
	}
}
...
func TestTheAnchorsBuyReasoningAndDoNotTakeOver(t *testing.T) {
	if v := cook.VietnameseShare(); math.Round(v) != 66 {
		t.Errorf("the mixture is %.1f%% Vietnamese, and the plan argues for 66", v)
	}
	if a := cook.KindShare(cook.Anchor); math.Round(a) != 34 {
		t.Errorf("the anchors are %.1f%% of the mixture, and the plan argues for 34", a)
	}
	if cook.KindShare(cook.Synthetic) >= cook.KindShare(cook.Natural) {
...
func TestEveryComponentIsCountedUnderExactlyOneKind(t *testing.T) {
	var total float64
	for _, k := range []cook.Kind{cook.Natural, cook.Synthetic, cook.Translated, cook.Anchor} {
		total += cook.KindShare(k)
	}
	if math.Abs(total-100) > 0.05 {
		t.Errorf("the kinds cover %.2f%% of the run", total)
	}
	if cook.Anchor.Vietnamese() {
		t.Error("the anchor languages are counted as Vietnamese")
	}
	for _, k := range []cook.Kind{cook.Natural, cook.Synthetic, cook.Translated} {
		if !k.Vietnamese() {
...
func TestEveryPhaseSpendsItselfAndThePhasesSpendTheRun(t *testing.T) {
	...
	if off := math.Abs(float64(whole-cook.Instances())) / float64(cook.Instances()); off > 0.001 {
		t.Errorf("the phases spend %d tokens and the budget buys %d", whole, cook.Instances())
	}
}
...
func TestEveryGapBetweenTheTablesBelongsToSomebody(t *testing.T) {
	...
	for _, g := range cook.Reconcile() {
		if math.Abs(g.Off()) <= cook.Tolerance && g.Spends > 0 {
			continue
		}
...
func TestTheFleetIsNowhereNearBigEnoughToTrainOn(t *testing.T) {
	...
	if !strings.Contains(cook.Fleet(), "gamingpc") {
...
func TestThePlanReadsLikeSomebodyWroteIt(t *testing.T) {
	text := make([]string, 0, len(cook.Budget())+2*len(cook.Curriculum())+len(cook.Questions())+len(cook.Arms())+3)
	for _, c := range cook.Budget() {
...
func find(name string) (cook.Component, bool) {
	for _, c := range cook.Budget() {
		if c.Name == name {
			return c, true
		}
	}
	return cook.Component{}, false
}
