...
func part(path string, bytes int64, text, meta int64) kho.PartWeight {
	return kho.PartWeight{
		Path:   path,
		Bytes:  bytes,
		Footer: 8 << 10,
		Rows:   100_000,
		Columns: []kho.ColumnWeight{
...
func TestARepoThatWithholdsTextIsNotWeighedWithOneThatShipsIt(t *testing.T) {
	...
	extra.Parts[1].Columns = append(extra.Parts[1].Columns, kho.ColumnWeight{Name: "tokens", Compressed: 10, Uncompressed: 20})
	refuses(t, extra, "carries tokens and data/00000.parquet does not")

	both := release(2, TargetShard, 0.08)
	both.Parts[1].Columns = []kho.ColumnWeight{{Name: "url", Compressed: 10, Uncompressed: 20}}
...
func TestWeighingRealPartsOffDisk(t *testing.T) {
	dataset, ok := kho.Lookup("vietnamese-web-text")
	if !ok {
		t.Fatal("vietnamese-web-text is not in the dataset table")
	}
	stamp := kho.Stamp{Snapshot: "gao-v1.0", Stage: "kho@0.1.0", Box: "server3"}

	dir := t.TempDir()
	paths := make([]string, 0, 3)
	for file := range 3 {
		rel := kho.StagePath(stamp.Snapshot, file, 0)
		p, err := kho.CreatePart(dir, rel, dataset, stamp)
		if err != nil {
			t.Fatal(err)
		}
		for i := range 50 {
