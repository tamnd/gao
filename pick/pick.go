// Package pick is decontamination: it finds the documents that hold the text of
...
// text here. That is the same key [mill.Key] builds, on purpose: a decontamination
...
func Grams(text string) func(func(uint64) bool) {
	return func(yield func(uint64) bool) {
		s := strings.Fields(mill.Key(text))
