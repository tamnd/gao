// Package layers reads an estimate that was taken layer by layer and says what
...
func (l Layer) Blocking() []string {
	...
	if r := l.Rate(); l.Text > 0 && l.Tokens > 0 && (r < estimate.MinRate || r > estimate.MaxRate) {
		why = append(why, fmt.Sprintf("%s reads %.2f tokens a byte of text, outside %.2f to %.2f, so it is not Vietnamese read with %s",
			l.Name, r, estimate.MinRate, estimate.MaxRate, l.Tokenizer))
	}
	return why
}
