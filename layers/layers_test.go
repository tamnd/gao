...
func layer(b bucket, read int64) layers.Layer {
	l := layers.Layer{
...
func hplt(read ...int) layers.Source {
	want := map[int]bool{}
	for _, r := range read {
		want[r] = true
	}
	s := layers.Source{Source: "hplt-v3 vie_Latn"}
...
func sampled() layers.Source { return hplt(5, 7, 8, 9, 10) }
...
func find(t *testing.T, s *layers.Source, name string) *layers.Layer {
	t.Helper()
...
func TestAReadingThatIsNotAStratifiedSampleIsRefused(t *testing.T) {
	for _, tc := range []struct {
		name  string
		spoil func(s *layers.Source)
		want  string
	}{
		{"no layers", func(s *layers.Source) { s.Layers = nil }, "published in layers and none of them are here"},
		{"no source", func(s *layers.Source) { s.Source = "" }, "does not say what it is a reading of"},
		{"a layer twice", func(s *layers.Source) { s.Layers[0].Name = "bucket 2" }, "appears twice"},
		{"two layers at one rank", func(s *layers.Source) { s.Layers[0].Rank = 2 }, "both sit at rank 2"},
		{"no rank", func(s *layers.Source) { s.Layers[0].Rank = 0 }, "has no place in the ordering"},
		{"a layer holding nothing", func(s *layers.Source) { s.Layers[0].Stored = 0 }, "says it holds nothing on disk"},
		{"more read than held", func(s *layers.Source) { s.Layers[4].Read = s.Layers[4].Stored + 1 }, "bytes out of the"},
		{"tokens with nothing read", func(s *layers.Source) { s.Layers[0].Tokens = 1_000_000 }, "counted tokens without reading anything"},
		{"no tokenizer", func(s *layers.Source) { s.Layers[4].Tokenizer = "" }, "names no tokenizer"},
		{"two tokenizers", func(s *layers.Source) { s.Layers[6].Tokenizer = "gemma-3" }, "two tokenizers are two units"},
		{"a rate that is not Vietnamese", func(s *layers.Source) { s.Layers[4].Tokens = s.Layers[4].Text }, "outside 0.15 to 0.45"},
		{"nothing read at all", func(s *layers.Source) { *s = hplt() }, "no layer was read"},
	} {
...
// The real reading. layers/testdata holds the layer file gao taste wrote against
...
func real(t *testing.T) layers.Source {
	t.Helper()
	read, err := layers.ReadLayers("testdata/hplt3-vie_Latn-s1.jsonl")
	if err != nil {
		t.Fatal(err)
	}
	return layers.Source{Source: "hplt3", Layers: read}
}
...
func TestTheFaultsAboutUnreadLayersAreSilentWhenNothingIsUnread(t *testing.T) {
	...
	if lo, hi := s.Packing(); hi/lo <= layers.MaxPackSpread {
		t.Fatalf("the real reading packs at %.2f to %.2f, inside the %.2f limit, so this test proves nothing", lo, hi, layers.MaxPackSpread)
	}
	lo, hi := math.Inf(1), 0.0
	for _, l := range s.Lit() {
		lo, hi = math.Min(lo, l.Yield()), math.Max(hi, l.Yield())
	}
	if hi/lo <= layers.MaxYieldSpread {
		t.Fatalf("the real reading reads at %.3f to %.3f, inside the %.2f limit, so this test proves nothing", lo, hi, layers.MaxYieldSpread)
	}
...
func TestTheRealReadingIsAPrefixOfEveryLayerAndSaysSo(t *testing.T) {
	...
	part := s.Partial()
	if len(part) != 5 {
		t.Fatalf("%d layers were read over under %.0f%% of themselves, want 5", len(part), layers.MinShare*100)
	}
...
func heldBack(t *testing.T) layers.Source {
	t.Helper()
	read, err := layers.ReadLayers("testdata/hplt3-vie_Latn-s1-top3.jsonl")
	if err != nil {
		t.Fatal(err)
	}
	return layers.Source{Source: "hplt3", Layers: read, Quoted: 176_000_000_000}
}
