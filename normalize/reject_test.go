...
func TestADocumentOfKeystrokesIsRejected(t *testing.T) {
	...
	if reason != reject.ReasonResidue {
		t.Errorf("rejected as %q, want %q", reason, reject.ReasonResidue)
	}
}
...
func TestABinaryThatWasSniffedAsTextIsRejected(t *testing.T) {
	...
	if reason != reject.ReasonControl {
		t.Errorf("rejected as %q, want %q", reason, reject.ReasonControl)
	}
}
...
func TestABinaryIsRejectedForBeingABinary(t *testing.T) {
	...
	if reason, _ := Reject(r); reason != reject.ReasonControl {
		t.Errorf("rejected as %q, want %q", reason, reject.ReasonControl)
	}
}
