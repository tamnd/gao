...
func TestARepositorySaysWhoItIs(t *testing.T) {
	s := serve(t, standard)
	r, err := seed.Identify(context.Background(), s.Client, s.Server.URL)
	if err != nil {
...
func TestWhatARecordCarriesThatIsWorthKeeping(t *testing.T) {
	...
	items, err := seed.Records(context.Background(), s.Client, r, seed.Harvest{})
	if err != nil {
...
func TestAnEmptyTokenIsTheEndAndNotAToken(t *testing.T) {
	...
	r := seed.Repository{Base: s.Server.URL}
	if _, err := seed.Records(context.Background(), s.Client, r, seed.Harvest{}); err != nil {
		t.Fatalf("records: %v", err)
	}
	if asks != 1 {
...
func TestNothingInTheRangeIsAnAnswer(t *testing.T) {
	...
	_, err = seed.Records(context.Background(), s.Client, r, seed.Harvest{From: time.Now()})
	if !errors.Is(err, seed.ErrNoRecords) {
		t.Errorf("nothing in the range came back as %v", err)
	}
	// And it does not make the repository broken, which is what P03-6 counts.
	if _, err := seed.Works(context.Background(), s.Client, s.Server.URL); err != nil {
...
func TestTheDateIsWrittenTheWayTheRepositoryAsked(t *testing.T) {
	for _, tc := range []struct {
		...
		if _, err := seed.Records(context.Background(), s.Client, r, seed.Harvest{From: from}); err != nil {
...
` + recordXML("oai:x:1", "2021-01-01T00:00:00Z", "Mot") + `
	...
	items, err := seed.Records(context.Background(), s.Client, seed.Repository{Base: s.Server.URL}, seed.Harvest{})
	if err != nil {
		t.Fatalf("records: %v", err)
	}
	if len(items) != 2 {
...
func TestARepositoryThatNeverEnds(t *testing.T) {
	...
	_, err := seed.Records(context.Background(), s.Client, seed.Repository{Base: s.Server.URL}, seed.Harvest{})
	if err == nil {
...
func TestAHarvestCanBeCutShort(t *testing.T) {
	...
	items, err := seed.Records(context.Background(), s.Client, seed.Repository{Base: s.Server.URL}, seed.Harvest{Max: 2})
	if err != nil {
		t.Fatalf("records: %v", err)
	}
	if len(items) != 2 {
...
func TestWhatCountsAsWorking(t *testing.T) {
	...
	if _, err := seed.Works(context.Background(), working.Client, working.Server.URL); err != nil {
	...
	if _, err := seed.Works(context.Background(), page.Client, page.Server.URL); err == nil {
...
func TestARepositoryThatIsDownIsNotARepositoryThatSaidNo(t *testing.T) {
	...
	_, err := seed.Identify(context.Background(), s.Client(), s.URL)
	if err == nil {
...
func TestABaseURLThatAlreadyHasAQuery(t *testing.T) {
	...
	if _, err := seed.Identify(context.Background(), s.Client(), s.URL+"/index.php?option=oai"); err != nil {
		t.Fatalf("identify: %v", err)
	}
