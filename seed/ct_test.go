...
func hosts(t *testing.T, body, suffix string) []seed.Host {
	t.Helper()
	got, err := seed.Hosts(strings.NewReader(body), suffix)
	if err != nil {
...
func names(hs []seed.Host) []string {
...
func has(hs []seed.Host, name string) bool {
	for _, h := range hs {
		if h.Name == name {
			return true
		}
...
func TestSomethingThatIsNotASearchResult(t *testing.T) {
	if _, err := seed.Hosts(strings.NewReader("<html>rate limited</html>"), "vn"); err == nil {
...
func TestWhatIsNewIsWhatThisIsFor(t *testing.T) {
	...
	got := seed.New(found, []string{"vnexpress.vn", "www.tuoitre.vn"})
...
func TestASeedSpelledDifferentlyIsStillTheSameSeed(t *testing.T) {
	...
	if got := seed.New(found, []string{"VnExpress.vn."}); len(got) != 0 {
...
func TestSearchAsksForTheSuffixAndReadsTheAnswer(t *testing.T) {
	...
	got, err := seed.Search(context.Background(), s.Client(), s.URL, ".vn")
	if err != nil {
