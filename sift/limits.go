...
func (l Limits) Reject(r Result) (reject.Reason, string, bool) {
	if r.Syllables < l.MinSyllables {
		return reject.ReasonShort, fmt.Sprintf("%d syllables, under %d", r.Syllables, l.MinSyllables), true
	}

	if rate := r.AlphaRate(); rate < l.MinAlphaRate {
		return reject.ReasonBoilerplate, fmt.Sprintf("%.2f of tokens hold a letter, under %.2f", rate, l.MinAlphaRate), true
	}
	if rate := r.BulletRate(); rate > l.MaxBulletRate {
		return reject.ReasonBoilerplate, fmt.Sprintf("%.2f of lines are bullets, over %.2f", rate, l.MaxBulletRate), true
	}
	if rate := r.EllipsisRate(); rate > l.MaxEllipsisRate {
		return reject.ReasonBoilerplate, fmt.Sprintf("%.2f of lines trail off, over %.2f", rate, l.MaxEllipsisRate), true
	}
	if rate := r.SymbolRate(); rate > l.MaxSymbolRate {
		return reject.ReasonBoilerplate, fmt.Sprintf("symbol rate %.3f, over %.2f", rate, l.MaxSymbolRate), true
	}

	if mean := r.MeanSyllable(); mean < l.MinMeanSyllable || mean > l.MaxMeanSyllable {
		return reject.ReasonLanguage, fmt.Sprintf("mean syllable %.2f letters, outside %.1f to %.1f", mean, l.MinMeanSyllable, l.MaxMeanSyllable), true
	}
	if r.StopWords < l.MinStopWords {
		return reject.ReasonLanguage, fmt.Sprintf("%d distinct function words, under %d", r.StopWords, l.MinStopWords), true
	}
	if l.Identify && !r.Language.Vietnamese() {
		return reject.ReasonLanguage, languageDetail(r.Language), true
	}

	if rate := r.DuplicateLineRate(); rate > l.MaxDuplicateLineRate {
		return reject.ReasonRepetition, fmt.Sprintf("%.2f of lines had appeared before, over %.2f", rate, l.MaxDuplicateLineRate), true
	}
	if rate := r.DuplicateLineRuneRate(); rate > l.MaxDuplicateLineRuneRate {
		return reject.ReasonRepetition, fmt.Sprintf("%.2f of the text is repeated lines, over %.2f", rate, l.MaxDuplicateLineRuneRate), true
	}
	for i, n := range TopGramSizes {
		if r.Top[i] > l.MaxTop[i] {
			return reject.ReasonRepetition, fmt.Sprintf("%.2f of the text is one %d syllable gram, over %.2f", r.Top[i], n, l.MaxTop[i]), true
		}
	}
	for i, n := range RepeatGramSizes {
		if r.Repeat[i] > l.MaxRepeat[i] {
			return reject.ReasonRepetition, fmt.Sprintf("%.2f of the text is repeated %d syllable grams, over %.2f", r.Repeat[i], n, l.MaxRepeat[i]), true
		}
	}
	return "", "", false
}
...
type Tally struct {
	...
	Rejected  map[reject.Reason]int64 `json:"rejected,omitempty"`
...
func (t *Tally) Add(l Limits, r Result) {
	...
	if reason, _, ok := l.Reject(r); ok {
		if t.Rejected == nil {
			t.Rejected = map[reject.Reason]int64{}
		}
		t.Rejected[reason]++
		return
	}
