...
func crawl(b *frontier.Budget, format string, r frontier.Result, limit int) (int, string) {
	for i := range limit {
...
func TestATemplateThatProducesTextKeepsGoing(t *testing.T) {
	b := frontier.NewBudget(small())
	got, why := crawl(b, "https://diendan.vn/thread/%d", frontier.New, 500)
	if got != 500 {
...
func TestATemplateThatProducesNothingStops(t *testing.T) {
	b := frontier.NewBudget(small())
	got, why := crawl(b, "https://lich.vn/su-kien/%d", frontier.Empty, 500)
	if got >= 500 {
...
func TestPagesWeAlreadyHaveEarnNothing(t *testing.T) {
	b := frontier.NewBudget(small())
	got, why := crawl(b, "https://vnexpress.net/in/%d", frontier.Repeat, 500)
	if got >= 500 {
...
func TestADeepForumGetsDepthAndAShallowShopDoesNot(t *testing.T) {
	forum := frontier.NewBudget(small())
	for _, section := range []string{"tin-tuc", "the-thao", "am-nhac", "xe-co", "nha-dat"} {
		got, why := crawl(forum, "https://diendan.vn/"+section+"/thread/%d", frontier.New, 100)
		if got != 100 {
	...
	shop := frontier.NewBudget(small())
	got, _ := crawl(shop, "https://shop.vn/san-pham/%d", frontier.Repeat, 40000)
	if got > 20 {
...
func TestAnArchiveAndACalendarLookTheSameAndEndDifferently(t *testing.T) {
	archive := frontier.NewBudget(small())
	got, why := crawl(archive, "https://vnexpress.net/luu-tru/2024-03-%02d", frontier.New, 30)
	if got != 30 {
		t.Fatalf("a real archive stopped after %d days: %s", got, why)
	}

	calendar := frontier.NewBudget(small())
	got, why = crawl(calendar, "https://truong.edu.vn/lich/2024-03-%02d", frontier.Empty, 30)
	if got >= 30 {
...
func TestATemplateWithADateInItIsJudgedSooner(t *testing.T) {
	...
	plain, _ := crawl(b, "https://truong.edu.vn/bai/%d", frontier.Empty, 500)
	dated, _ := crawl(b, "https://truong.edu.vn/lich/2024-03-%02d", frontier.Empty, 500)
...
func TestOneHostRunningOutDoesNotTouchAnother(t *testing.T) {
	b := frontier.NewBudget(small())
	crawl(b, "https://mot.vn/thread/%d", frontier.Empty, 100)
...
func TestTheHeaviestSpenderIsWhatGetsReported(t *testing.T) {
	b := frontier.NewBudget(small())
	crawl(b, "https://diendan.vn/thread/%d", frontier.New, 40)
	crawl(b, "https://diendan.vn/lich/2024-03-%02d", frontier.Empty, 20)
...
func TestWhyAParticularURLWillNotBeAskedFor(t *testing.T) {
	b := frontier.NewBudget(small())
	crawl(b, "https://lich.vn/ngay/%d", frontier.Empty, 100)
...
func TestEveryRefusalSaysWhy(t *testing.T) {
	...
	_, why := crawl(b, "https://lich.vn/ngay/%d", frontier.Empty, 100)
	if strings.TrimSpace(why) == "" {
