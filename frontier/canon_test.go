...
func TestATrailingSlashIsTheServersBusiness(t *testing.T) {
	if frontier.Same("https://vnexpress.net/tin-tuc", "https://vnexpress.net/tin-tuc/") {
	...
	if !frontier.Same("https://vnexpress.net", "https://vnexpress.net/") {
...
func TestAVietnameseDomainAndItsPunycodeAreOneHost(t *testing.T) {
	if !frontier.Same("https://cà-phê.vn/tin-tuc", "https://xn--c-ph-0na7d.vn/tin-tuc") {
		a, _ := frontier.Canon("https://cà-phê.vn/tin-tuc")
...
func TestWhatWillNotCanonicalize(t *testing.T) {
	...
	for _, rawurl := range bad {
		if got, err := frontier.Canon(rawurl); err == nil {
...
func TestAURLThatWillNotParseIsNotTheSameAsAnything(t *testing.T) {
	if frontier.Same("javascript:void(0)", "javascript:void(0)") {
		t.Error("two copies of something that is not a URL were called one page")
	}
	if frontier.Same("https://vnexpress.net/a", "javascript:void(0)") {
