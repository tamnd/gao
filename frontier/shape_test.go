...
func shape(t *testing.T, rawurl string) frontier.Shape {
	t.Helper()
	s, err := frontier.Of(rawurl)
	if err != nil {
...
func TestTheSlugIsWhatMakesAPageAPage(t *testing.T) {
	...
	// would be two shapes.
	if !strings.Contains(shapeOf(t, "https://vnexpress.net/tin-tuc/12345"), frontier.KindNumber) {
...
func TestADateIsNotJustANumber(t *testing.T) {
	for _, dated := range []string{
		...
		s := shape(t, dated)
		if !strings.Contains(s.Path, frontier.KindDate) {
...
func TestAYearOnItsOwnIsANumber(t *testing.T) {
	...
	if !strings.Contains(s.Path, frontier.KindNumber) {
...
func TestAnOpaqueIdentifierCollapses(t *testing.T) {
	for _, rawurl := range []string{
		...
		s := shape(t, rawurl)
		if !strings.Contains(s.Path, frontier.KindID) {
...
func TestWhatCanonicalizesTogetherShapesTogether(t *testing.T) {
	...
	for _, pair := range pairs {
		if !frontier.Same(pair[0], pair[1]) {
...
func TestSomethingThatIsNotAURLHasNoShape(t *testing.T) {
	for _, rawurl := range []string{"javascript:void(0)", "/no-host", "mailto:a@b.vn"} {
		if s, err := frontier.Of(rawurl); err == nil {
