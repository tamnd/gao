...
func TestARejectedDocumentArrivesInTheRejectStoreWithItsReason(t *testing.T) {
	...
	// pass cannot afford.
	rejects, err := vo.NewWriter(&buf, 1)
	if err != nil {
		t.Fatalf("vo.NewWriter: %v", err)
	}
	...
	r, err := vo.NewReader(bytes.NewReader(buf.Bytes()))
	if err != nil {
		t.Fatalf("vo.NewReader: %v", err)
	}
	rec, err := r.Next()
	if err != nil {
		t.Fatalf("Next: %v", err)
	...
	if rec.Reason != vo.ReasonContract {
		t.Errorf("reject_reason is %q, want %q", rec.Reason, vo.ReasonContract)
	}
...
func TestTheCountsComeOutWithNoRejectStoreAtAll(t *testing.T) {
	...
	if got := d.Reasons()[vo.ReasonContract]; got != 2 {
...
func TestAMissingLanguageScoreIsNotALanguageFinding(t *testing.T) {
	for _, tc := range []struct {
		...
		{"no score published", doc.Document{}, vo.ReasonContract},
		{"a score the producer was unsure of", scored("vie", 0.2), vo.ReasonLanguage},
		{"a score the producer stood behind", scored("vie", 0.9), vo.ReasonContract},
		{"a language that is not the one being ingested", scored("eng", 0.99), vo.ReasonLanguage},
		{"a language with no score", scored("tha", 0), vo.ReasonLanguage},
	} {
...
func TestTheReasonsAreHandedOutAsACopy(t *testing.T) {
	...
	if held[vo.ReasonContract] != 1 {
		t.Errorf("the map moved under the caller: %v", held)
	}
	if d.Reasons()[vo.ReasonContract] != 2 {
