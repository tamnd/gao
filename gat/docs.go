...
func (d *Docs) tally(admitted, rejected int64, reason vo.Reason) {
	d.mu.Lock()
	defer d.mu.Unlock()
	d.admitted += admitted
	d.rejected += rejected
	...
	if d.reasons == nil {
		d.reasons = make(map[vo.Reason]int64)
	}
	d.reasons[reason]++
}
...
func reasonFor(rec *doc.Document) vo.Reason {
	switch {
	case rec.Lang != "" && rec.Lang != "vie":
		return vo.ReasonLanguage
	case rec.LangScore > 0 && rec.LangScore < minLangScore:
		return vo.ReasonLanguage
	}
	return vo.ReasonContract
}
...
func (d *Docs) Reasons() map[vo.Reason]int64 {
	d.mu.Lock()
	defer d.mu.Unlock()
	out := make(map[vo.Reason]int64, len(d.reasons))
