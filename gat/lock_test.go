...
func lockFile(dir string) string { return filepath.Join(dir, gat.LockName) }

// writeLock plants a lock file the way a previous run would have left one.
func writeLock(t *testing.T, dir string, h gat.Holder) {
	t.Helper()
...
func TestATruncatedLockFileStillLocksTheDirectory(t *testing.T) {
	...
	if !strings.Contains(err.Error(), gat.LockName) {
...
func TestReadHolderOnADirectoryWithNoLockIsNotAnError(t *testing.T) {
	h, err := gat.ReadHolder(t.TempDir())
	if err != nil {
