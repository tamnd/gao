// Package giao is to hand over: which box fetches which file of the ingest, and
...
func Divide(readings []Reading) Split {
	...
	for _, l := range readings {
		b, ok := may.Lookup(l.Box)
		switch {
		case !ok:
			continue
		case !may.HoldsCorpus(b):
...
func Room(box string) int64 {
	...
	if n := may.Scratch(b) - may.PeakBytes(b); n > 0 {
		return n
	}
	return 0
}
