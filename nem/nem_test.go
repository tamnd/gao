...
func sample(t *testing.T) nem.Sample {
	t.Helper()
...
// rather than the run: a layer the sample has no reading for comes through with
...
func TestTheLayerFileCarriesEveryLayerAndFillsInTheOnesThatWereRead(t *testing.T) {
	s := sample(t)
	all, err := tang.ReadLayers("../mau/testdata/hplt3-vie_Latn-layers.jsonl")
	if err != nil {
		t.Fatal(err)
	}
	...
	kept := s.Layers(append(all[:len(all):len(all)], tang.Layer{Name: "bucket 4", Rank: 4, Stored: 9}))
...
func TestTheRefusals(t *testing.T) {
	...
	for _, c := range []struct {
		name string
		of   func(nem.Sample) nem.Sample
		want string
	}{
		{"no source", func(s nem.Sample) nem.Sample { s.Source = ""; return s },
			"does not say what source it read"},
		{"no seed", func(s nem.Sample) nem.Sample { s.Seed = ""; return s },
			"names no seed"},
		{"nothing read", func(s nem.Sample) nem.Sample { s.Readings = nil; return s },
			"no layer was read"},
		{"a layer that gave nothing", func(s nem.Sample) nem.Sample {
			one := s.Readings[0]
			one.Documents = 0
			s.Readings = append([]nem.Reading{one}, s.Readings[1:]...)
			return s
...
// The digest is the plan's, not one this works out for itself, because a sample
...
func TestTheSampleCarriesTheDigestOfThePlanItPerformed(t *testing.T) {
	layers, err := tang.ReadLayers("../mau/testdata/hplt3-vie_Latn-layers.jsonl")
	if err != nil {
		t.Fatal(err)
	}
	files, err := mau.ReadFiles("../mau/testdata/hplt3-vie_Latn-listing.jsonl")
	if err != nil {
		t.Fatal(err)
	}

	p := mau.ReadPlan("hplt3", "s1", mau.Want, layers, files)
	if got, want := nem.Of(p, "", nil).Digest, p.Digest.String()[:16]; got != want {
		t.Errorf("the sample carries digest %s and the plan it ran is %s", got, want)
	}
	if got := sample(t).Digest; got != p.Digest.String()[:16] {
