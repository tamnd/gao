// Package nem reads the sample a plan drew and says what a stored byte of each
// layer holds.
//
...
func Of(p mau.Plan, tokenizer string, takes []Take) Sample {
	s := Sample{
		Source:    p.Source,
		Seed:      p.Seed,
...
// bounds.
func (s Sample) Layers(all []tang.Layer) []tang.Layer {
	got := make(map[string]Reading, len(s.Readings))
	for _, r := range s.Readings {
		got[r.Layer] = r
	}

	out := make([]tang.Layer, 0, len(all))
	for _, l := range all {
		r, ok := got[l.Name]
		if !ok || r.Read == 0 {
			out = append(out, l)
...
func (s Sample) Faults() []string {
	...
	var small []Reading
	for _, r := range s.Readings {
		if r.Read > 0 && r.Read < tang.MinRead {
	...
	case n == 1:
		out = append(out, fmt.Sprintf(
			...
			small[0].Layer, size(small[0].Read), size(tang.MinRead)))
	case n > 1:
		out = append(out, fmt.Sprintf(
			"%d layers were read off under %s each, starting with %s at %s, so tang will not scale anything by them",
			n, size(tang.MinRead), small[0].Layer, size(small[0].Read)))
	}
	...
	case n == 1:
		out = append(out, fmt.Sprintf(
			...
			narrow[0].Layer, plural(narrow[0].Drawn, "shard"), mau.MinFiles))
	case n > 1:
		out = append(out, fmt.Sprintf(
			...
			n, mau.MinFiles, narrow[0].Layer, narrow[0].Drawn))
	}
...
// in its verdict.
func size(n int64) string { return may.Size(n) }
