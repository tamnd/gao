...
func cardBody(b *strings.Builder, d Dataset, m *Manifest, x []Indexed) {
	...
	fmt.Fprintf(b, "This card is generated by `gao store card`, from the snapshot manifest where there is one and from `%s` where there is not. Editing it by hand works until the next run overwrites it.\n", IndexName)
}
...
func cardTagline(d Dataset, m *Manifest, x []Indexed) string {
	...
	if by := BySource(x); len(by) > 0 {
		...
		return fmt.Sprintf("%s %s from %s, %s of Parquet, one schema",
			cardCommas(n), what, corpora, fleet.Size(size))
	}
	return fmt.Sprintf("%s %s, %s of Parquet, one schema", cardCommas(n), what, fleet.Size(size))
}
...
func cardParts(b *strings.Builder, x []Indexed) {
	...
	for _, s := range by {
		fmt.Fprintf(b, "| `%s` | %s | %d | %d | %s | `%s` |\n",
			s.Source, cardCommas(s.Documents), s.Parts, s.Files, fleet.Size(s.Bytes),
			strings.Join(s.Snapshots, "`, `"))
	}
	fmt.Fprintf(b, "| **total** | **%s** | **%d** | | **%s** | |\n\n",
		cardCommas(documents), len(x), fleet.Size(bytes))
	...
	fmt.Fprintf(b, "The repo grows while ingests run, so these numbers are the ones from the last time `gao store index` was run against it rather than a sealed total. The counts in `%s` and the counts here always agree, because they are generated together.\n\n", IndexName)
}
...
func cardSnapshot(b *strings.Builder, m *Manifest) {
	...
	b.WriteString("\n")
	b.WriteString("The root commits to every file in the snapshot, and `gao store verify` checks the files against it and the root against the signature. A snapshot is immutable: a document removed after this one was sealed is a tombstone in a later snapshot rather than an edit to this one.\n\n")
}
