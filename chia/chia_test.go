...
func TestABornDigitalPageIsExtractedDirectly(t *testing.T) {
	r := chia.Read(pdf(t, page(show(vietnamese)), false))
	if r.Route != chia.Text {
...
func TestALegacyFontEncodingIsCaughtBeforeAnythingReadsIt(t *testing.T) {
	r := chia.Read(pdf(t, page(show(tcvn3(vietnamese))), false))
	if r.Route != chia.Legacy {
...
func TestAPageImageWithNoTextOnItGoesToOCR(t *testing.T) {
	r := chia.Read(pdf(t, page("BT (Trang 1) Tj ET")+imageObject(400_000), false))
	if r.Route != chia.Scan {
...
func TestFurnitureOnAScanDoesNotMakeItATextLayer(t *testing.T) {
	r := chia.Read(pdf(t, page("BT (Cong ty co phan ABC   Trang 4 / 12) Tj ET")+imageObject(200_000), false))
	if r.Route != chia.Scan {
...
func TestDensityIsMeasuredPerPageRatherThanPerDocument(t *testing.T) {
	...
	one := chia.Read(pdf(t, page(show(vietnamese)), false))
	if one.Pages != 1 {
		t.Fatalf("the scan found %d pages in a one page document", one.Pages)
	}
	if one.GlyphsPerPage() < chia.MinGlyphsPerPage {
...
func TestACompressedContentStreamIsRead(t *testing.T) {
	plain := chia.Read(pdf(t, page(show(vietnamese)), false))
	packed := chia.Read(pdf(t, page(show(vietnamese)), true))
...
func TestEveryTextShowingOperatorCounts(t *testing.T) {
	for _, tc := range []struct{ name, body string }{
		...
		t.Run(tc.name, func(t *testing.T) {
			r := chia.Read(pdf(t, page(tc.body), false))
			if r.Route != chia.Text {
...
func TestAHexStringIsReadAsTheBytesItHolds(t *testing.T) {
	...
	r := chia.Read(pdf(t, page("BT <"+hex.String()+"> Tj ET"), false))
	if r.Route != chia.Legacy {
...
func TestAnOctalEscapeIsTheByteItNames(t *testing.T) {
	...
	r := chia.Read(pdf(t, page(body), false))
	if r.Route != chia.Legacy {
...
func TestTheRouteLettersAreTheOnesTheCostTablesUse(t *testing.T) {
	want := map[chia.Route]string{chia.Text: "T", chia.Legacy: "L", chia.Scan: "O", chia.Unroutable: "-"}
...
func TestTheDistributionCarriesTheBoxItWasCountedOn(t *testing.T) {
	d := chia.NewDistribution("gamingpc")
	d.Add(chia.Reading{Route: chia.Text, Pages: 10})
	d.Add(chia.Reading{Route: chia.Text, Pages: 4})
	d.Add(chia.Reading{Route: chia.Scan, Pages: 200})
	d.Add(chia.Reading{Route: chia.Legacy, Pages: 6, Charset: "TCVN3"})

	if d.Total() != 4 {
		t.Errorf("the distribution counted %d documents", d.Total())
	}
	if got := d.Share(chia.Text); got != 50 {
	...
	if d.Pages(chia.Scan) != 200 {
		t.Errorf("the scan route holds %d pages", d.Pages(chia.Scan))
	}
...
func TestAnEmptyDistributionReportsNothingRatherThanZeroPercent(t *testing.T) {
	d := chia.NewDistribution("server3")
	if d.Total() != 0 {
		t.Errorf("a fresh distribution holds %d documents", d.Total())
	}
	for _, r := range chia.Routes {
		if d.Share(r) != 0 {
...
func tcvn3(s string) string {
	var set *phoi.Charset
	for _, c := range phoi.Charsets() {
		if c.Name() == "TCVN3" {
			set = c
		}
	}
