// Package tieng measures what a syllable-atomic tokenizer would govern, and what
...
func Read(source string, slots int, docs []Doc) Reading {
	r := Reading{Source: source, Slots: slots, docs: docs, Inventory: len(sang.Syllables())}
	...
	for _, d := range docs {
		...
		if !sang.Identify(d.Text).Vietnamese() {
			r.Refused++
...
func measure(r *Reading, name, text string, perDoc map[string]int64) [][]string {
	...
	for _, raw := range strings.Fields(text) {
		...
		case word == "":
			...
			continue
		case sang.Syllable(word), sang.BareSyllable(word):
			if sang.Syllable(word) {
				r.Marked++
			} else {
				r.Bare++
			}
