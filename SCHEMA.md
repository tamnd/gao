...
<!-- Generated by `gao store schema -md`. The meanings live in store/schema.go, beside the type they describe, and a test fails when this file falls behind them. -->
...
| column | type | filled by | meaning |
| --- | --- | --- | --- |
| `pii_spans.start` | `uint32` | `cover` | byte offset into the text where the identifier begins |
| `pii_spans.len` | `uint32` | `cover` | how many bytes long it is, counted in bytes rather than runes because a reader slicing the text has bytes |
| `pii_spans.type` | `string` | `cover` | which kind of identifier it is, from the same set pii_types draws on |
