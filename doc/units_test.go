...
func TestReferenceConversion(t *testing.T) {
	// One gigabyte of extracted Vietnamese text is roughly 769 million
	// characters, 162 million syllables, and 235 million gao tokens. Every size
	// in this project traces back to this row, so it gets a test.
	const gb = int64(BytesPerGB)

	if got := CharsPerGB / 1e6; math.Abs(got-769) > 2 {
		t.Errorf("characters per GB is %.1f million, want about 769", got)
	}
	if got := float64(EstimateTokens(gb)) / 1e6; math.Abs(got-235) > 2 {
		t.Errorf("tokens per GB is %.1f million, want about 235", got)
	}
	if got := float64(EstimateSyllables(gb)) / 1e6; math.Abs(got-162) > 2 {
		t.Errorf("syllables per GB is %.1f million, want about 162", got)
	}
}
...
func TestHPLTEstimateMatchesThePublishedNumber(t *testing.T) {
	...
	if math.Abs(got-55.0) > 1 {
		t.Errorf("naive conversion of %.1f GB gives %.1fB tokens, want about 55.0B", gbOfHPLT, got)
	}
	// The gap between 55B and 176B is the point: the 234.5 GB figure is
