...
type Result struct {
	...
	// worth quoting off it is [soi.Score.DER], the share of the page's marks
	// that did not come back.
	Score soi.Score `json:"score"`
...
func Grade(it Item, answer string) Result {
	r := Result{
		DocID:    it.DocID,
		Score:    soi.Measure(it.Answer, answer),
		Faithful: phoi.Bare(answer) == it.Prompt,
	}
