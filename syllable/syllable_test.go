...
func docs(n int, body string) []syllable.Doc {
	out := make([]syllable.Doc, 0, n)
	for i := range n {
		out = append(out, syllable.Doc{Name: fmt.Sprintf("bai-%03d.txt", i), Text: body})
	}
	return out
}
...
func read(t *testing.T, d []syllable.Doc) syllable.Reading {
	t.Helper()
	r := syllable.Read("a sample", syllable.Slots, d)
...
func TestARunThatRepeatsOftenEnoughTakesASlot(t *testing.T) {
	r := read(t, docs(1, repeat(syllable.MinCount+10, "việt nam")))
	...
	if got := r.Runs[0].Count; got != syllable.MinCount+10 {
		t.Errorf("the run was counted %d times, want %d", got, syllable.MinCount+10)
	}
	if r.Cost <= 0 {
...
func TestARunThatDoesNotRepeatOftenEnoughTakesNothing(t *testing.T) {
	r := read(t, docs(1, repeat(syllable.MinCount-1, "việt nam")))

	for _, run := range r.Runs {
		if run.Run == "việt nam" {
			t.Fatalf("a run seen %d times took a slot, and the floor is %d", run.Count, syllable.MinCount)
		}
	}
...
func TestALongerRunPaysForItselfOutOfTheShorterOnesInsideIt(t *testing.T) {
	r := read(t, docs(1, repeat(syllable.MinCount*4, "thành phố hồ chí")))
...
func TestAMergeDoesNotCrossPunctuation(t *testing.T) {
	r := read(t, docs(1, repeat(syllable.MinCount*2, "việt nam, việt nam")))
...
func TestTheSavingIsWalkedRatherThanAddedUp(t *testing.T) {
	r := read(t, docs(1, repeat(syllable.MinCount*3, "thành phố hồ chí")))
...
func TestADocumentTheIdentifierDoesNotCallVietnameseIsLeftOutAndNamed(t *testing.T) {
	sample := docs(3, repeat(syllable.MinCount, "việt nam"))
	sample = append(sample, syllable.Doc{
		Name: "english.txt",
...
func TestOneDocumentCarryingTheTableIsNamed(t *testing.T) {
	...
	sample = append(sample, syllable.Doc{Name: "dai.txt", Text: repeat(200, "việt nam")})
...
func TestTextTheRuleMostlyDoesNotGovernIsNamedAsThat(t *testing.T) {
	...
	if r.Governed >= syllable.MinGoverned {
...
func TestAnArmGivenEverySlotItAskedForIsNamedAsACeiling(t *testing.T) {
	body := repeat(syllable.MinCount+5, "việt nam") + repeat(syllable.MinCount+5, "hà nội")

	r := syllable.Read("a sample", 1, docs(1, body))
	...
	wide := syllable.Read("a sample", syllable.Slots, docs(1, body))
...
func TestTheSameTextReadTwiceGivesTheSameTable(t *testing.T) {
	body := repeat(syllable.MinCount+3, "việt nam") + repeat(syllable.MinCount+7, "hà nội") + repeat(syllable.MinCount+1, "thành phố")
...
func TestARefusalIsAboutTheReadingRatherThanAboutTheText(t *testing.T) {
	good := docs(1, repeat(syllable.MinCount, "việt nam"))
	...
	for _, c := range []struct {
		...
		{"no source", "", syllable.Slots, good, "does not say what text it was taken over"},
		{"no slots", "a sample", 0, good, "allowed no vocabulary slots"},
		{"no documents", "a sample", syllable.Slots, nil, "no text was read"},
		{"a document with no name", "a sample", syllable.Slots,
			[]syllable.Doc{{Text: line(0)}}, "arrived with no name"},
		{"two documents with no name", "a sample", syllable.Slots,
			[]syllable.Doc{{Text: line(0)}, {Text: line(1)}}, "2 documents arrived with no name"},
		{"the same document twice", "a sample", syllable.Slots,
			append(append([]syllable.Doc{}, good...), good...), "was read twice"},
		{"a document with no text", "a sample", syllable.Slots,
			[]syllable.Doc{{Name: "trong.txt"}}, "trong.txt holds no text"},
		{"nothing the identifier calls Vietnamese", "a sample", syllable.Slots,
			[]syllable.Doc{{Name: "en.txt", Text: "The quarterly report is attached to this message."}},
			"called none of these documents Vietnamese"},
	} {
		t.Run(c.name, func(t *testing.T) {
			r := syllable.Read(c.source, c.slots, c.docs)
...
func TestTheVerdictSaysWhatTheRuleCostsAndWhatItDidNotMeasure(t *testing.T) {
	r := read(t, docs(1, repeat(syllable.MinCount*2, "việt nam")))
...
func TestTheInventoryIsOnTheReadingBecauseItIsWhatMakesTheFloorReachable(t *testing.T) {
	r := read(t, docs(1, repeat(syllable.MinCount, "việt nam")))
