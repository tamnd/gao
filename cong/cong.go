// Package cong adds up a release without letting the addition decide anything.
//
...
func (g Group) Blocking() []string {
	...
	if !g.Class.Valid() {
		why = append(why, fmt.Sprintf("%s carries no license class, so whether it ships is undecided and the publishable subset cannot be stated", g.Source))
	}
	if g.Snapshot == "" {
