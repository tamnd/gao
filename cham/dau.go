...
// A Dau verifies diacritic restoration.
//
...
// NewDau returns a verifier with nothing in its key yet.
func NewDau() *Dau { return &Dau{key: map[string]string{}} }
...
func (d *Dau) Learn(page string) bool {
...
func (d *Dau) Items() int { return len(d.key) }

// Specialist is the arm this verifies.
func (d *Dau) Specialist() string { return "dau" }
...
func (d *Dau) Verify(prompt, answer string) Verdict {
	...
	bare := phoi.Bare(answer)
	switch {
	case bare == prompt:
		r := dau.Grade(dau.NewItem(doc.SumString(page), page), answer)
