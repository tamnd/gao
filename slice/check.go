...
func (s *Slice) Against(m *store.Manifest) []string {
	var faults []string
	...
	shards := make(map[int]store.Shard, len(m.Shards))
...
func (s *Slice) Fresh(head *store.Manifest) error {
...
func (s *Slice) Repo() (store.Dataset, bool) { return store.Lookup(s.Dataset) }
