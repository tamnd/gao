...
func TestARuleAddressedToUsIsMatchedByTheTokenAndNotByTheHeader(t *testing.T) {
	...
	if r.Allows(harvest.Bot, "/tin-tuc/") {
		t.Error("a block addressed to us by name did not apply to us")
	}
	if !r.Allows(harvest.Agent("1.0"), "/tin-tuc/") {
...
func TestTheTokenIsMatchedWhateverCaseTheSiteWroteItIn(t *testing.T) {
	for _, written := range []string{"gaobot", "GAOBOT", "GaoBot"} {
		r := harvest.ReadRobots([]byte("User-agent: " + written + "\nDisallow: /\n"))
		if r.Allows(harvest.Bot, "/") {
...
func TestTheTokenIsOneWordAPersonCanType(t *testing.T) {
	if harvest.Bot != strings.ToLower(harvest.Bot) {
		t.Errorf("the token is %q and a site owner will type it in lower case", harvest.Bot)
	}
	if strings.ContainsAny(harvest.Bot, " \t/:()") {
		t.Errorf("the token is %q, and a robots.txt parser splits on some of that", harvest.Bot)
	}
}
...
func TestEveryRequestCarriesSomewhereToComplainTo(t *testing.T) {
	agent := harvest.Agent("0.4.1")

	if !strings.HasPrefix(agent, harvest.Bot+"/0.4.1 ") {
		t.Errorf("the header is %q, and a log analyzer reads the token and the version off the front", agent)
	}
	if !strings.Contains(agent, "(+"+harvest.Contact+")") {
		t.Errorf("the header is %q and does not carry %q", agent, harvest.Contact)
	}
	if !strings.HasPrefix(harvest.Contact, "https://") {
		t.Errorf("the contact address is %q, and a webmaster pastes it into a browser", harvest.Contact)
	}
}
...
func TestThereIsOneAgentAndItDoesNotVary(t *testing.T) {
	first, second := harvest.Agent("1.2.3"), harvest.Agent("1.2.3")
	if first != second {
	...
	if harvest.Agent("1.2.3") == harvest.Agent("1.2.4") {
...
func TestAnUnstampedBuildStillSaysWhoItIs(t *testing.T) {
	agent := harvest.Agent("dev")

	if !strings.HasPrefix(agent, harvest.Bot+"/dev ") {
