...
func visit() *harvest.Visit {
	return &harvest.Visit{
		...
		Robots: harvest.Decision{Allowed: true, Why: harvest.RobotsAllow, Rule: "Allow: /tin-tuc"},
	}
}

func write(t *testing.T, gz bool, records ...*harvest.Record) []byte {
	t.Helper()
	var buf bytes.Buffer
	w := harvest.NewWARCWriter(&buf, gz)
...
func read(t *testing.T, b []byte) []*harvest.Record {
	t.Helper()
	r, err := harvest.NewWARCReader(bytes.NewReader(b))
	if err != nil {
...
func TestAPageComesBackOutWithoutRefetchingIt(t *testing.T) {
	...
	for _, gz := range []bool{false, true} {
		records := read(t, write(t, gz, harvest.VisitRecords(v, when(), "gaobot/1.0")...))
		if len(records) != 2 {
...
func TestVietnameseSurvivesTheRoundTripByteForByte(t *testing.T) {
	...
	records := read(t, write(t, true, harvest.VisitRecords(v, when(), "gaobot/1.0")...))
	resp, err := records[1].Response()
	if err != nil {
		t.Fatal(err)
	}
...
func TestManyRecordsInOneFile(t *testing.T) {
	records := make([]*harvest.Record, 0, 8)
	records = append(records, harvest.Info(harvest.WARCInfo{
	...
	for i, path := range []string{"/a", "/b", "/c"} {
		...
		records = append(records, harvest.VisitRecords(v, when(), "gaobot/1.0")...)
	}
...
func TestEveryRecordIsItsOwnGzipMember(t *testing.T) {
	v := visit()
	var buf bytes.Buffer
	w := harvest.NewWARCWriter(&buf, true)

	offsets := make([]int64, 0, 2)
	lengths := make([]int64, 0, 2)
	for _, r := range harvest.VisitRecords(v, when(), "gaobot/1.0") {
		off, n, err := w.Write(r)
		if err != nil {
			t.Fatal(err)
		}
...
func TestTheSameCrawlWrittenTwiceIsTheSameBytes(t *testing.T) {
	first := write(t, true, harvest.VisitRecords(visit(), when(), "gaobot/1.0")...)
	second := write(t, true, harvest.VisitRecords(visit(), when(), "gaobot/1.0")...)
...
func TestTwoDifferentPagesGetDifferentIdentities(t *testing.T) {
	...
	ra := harvest.VisitRecords(a, when(), "gaobot/1.0")[1]
	rb := harvest.VisitRecords(b, when(), "gaobot/1.0")[1]
	...
	idc, _ := harvest.VisitRecords(c, when(), "gaobot/1.0")[1].Get("WARC-Record-ID")
	if idc == ida {
...
func TestTheRequestAndTheResponseAreJoined(t *testing.T) {
	records := harvest.VisitRecords(visit(), when(), "gaobot/1.0")
...
func TestWhatTheSiteSaidAboutMiningIsInTheRecord(t *testing.T) {
	v := visit()
	v.Reserve = harvest.Reservation{
		NoTrain: true,
		Policy:  "https://vnexpress.net/dieu-khoan",
		Said:    []string{"tdm-reservation: 1", "X-Robots-Tag: noai"},
	}

	rec := read(t, write(t, true, harvest.VisitRecords(v, when(), "gaobot/1.0")...))[1]
...
func TestAPageThatReservedNothingSaysNothing(t *testing.T) {
	rec := read(t, write(t, true, harvest.VisitRecords(visit(), when(), "gaobot/1.0")...))[1]
...
func TestTheDigestCoversTheBytes(t *testing.T) {
	a := harvest.VisitRecords(visit(), when(), "gaobot/1.0")[1]

	changed := visit()
	changed.Body = append(changed.Body, ' ')
	b := harvest.VisitRecords(changed, when(), "gaobot/1.0")[1]
...
func TestTheFileSaysWhoWroteItAndWhereToComplain(t *testing.T) {
	rec := read(t, write(t, true, harvest.Info(harvest.WARCInfo{
		Filename:  "gao-crawl-2026-03-15-00000.warc.gz",
		Software:  "gao/1.0",
...
func TestATruncatedFileFailsWhereItStops(t *testing.T) {
	whole := write(t, false,
		harvest.VisitRecords(visit(), when(), "gaobot/1.0")[1],
		harvest.VisitRecords(visit(), when(), "gaobot/1.0")[0],
	)
	cut := whole[:len(whole)-40]

	r, err := harvest.NewWARCReader(bytes.NewReader(cut))
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = r.Next()
	if err == nil || errors.Is(err, harvest.ErrDone) {
...
func TestSomethingThatIsNotAWARCSaysSo(t *testing.T) {
	r, err := harvest.NewWARCReader(strings.NewReader("GET / HTTP/1.1\r\n\r\n"))
	if err != nil {
		t.Fatal(err)
	}
	if _, err := r.Next(); err == nil {
...
func TestAnEmptyFileIsJustFinished(t *testing.T) {
	r, err := harvest.NewWARCReader(bytes.NewReader(nil))
	if err != nil {
		t.Fatal(err)
	}
	if _, err := r.Next(); !errors.Is(err, harvest.ErrDone) {
