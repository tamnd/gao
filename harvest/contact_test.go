...
func TestTheBlockTheContactPageTellsPeopleToWriteStopsUs(t *testing.T) {
	...
	stop := harvest.ReadRobots([]byte(examples[0]))
	for _, path := range []string{"/", "/tin-tuc/", "/dien-dan/chu-de-123", "/robots.txt"} {
		if stop.Allows(harvest.Bot, path) {
...
func TestTheContactPageDescribesTheAgentWeActuallySend(t *testing.T) {
	...
	if !strings.Contains(md, harvest.Contact) {
		t.Errorf("the page does not carry %q, which is the URL every request points at", harvest.Contact)
	}
	if !strings.Contains(md, harvest.Bot+"/") {
		t.Errorf("the page does not show the %q header a site owner is looking for in a log", harvest.Bot)
	}
