...
func (b build) pairs() []compare.Pair {
	if b.raters == 0 {
		b.raters = 8
	}
	if b.pick == nil {
	...
	out := make([]compare.Pair, 0, b.items+b.items/5)
	for i := range b.items {
		...
		for k := range reads {
			...
			case left:
				choice = compare.Left
			case right:
				choice = compare.Right
			}

			out = append(out, compare.Pair{
...
func read(b build) compare.Reading { return compare.Read(b.pairs()) }
...
func TestPickingTheLeftHandAnswerIsCaughtBeforeTheWinRateIsBelieved(t *testing.T) {
	...
	for i := range pairs {
		pairs[i].Choice = compare.Left
	}

	r := compare.Read(pairs)
...
func TestTooFewSecondOpinionsIsAFaultRatherThanARefusal(t *testing.T) {
	...
	kept := make([]compare.Pair, 0, len(pairs))
...
func TestAnEvaluationThatCannotBeReadIsRefused(t *testing.T) {
	for _, tc := range []struct {
		name  string
		spoil func(pairs []compare.Pair) []compare.Pair
		want  string
	}{
		{"no judgements", func([]compare.Pair) []compare.Pair { return nil }, "holds no judgements"},
		{"too few judgements", func(p []compare.Pair) []compare.Pair { return p[:100] }, "under the 200 this reading needs"},
		{"a judgement with no item", func(p []compare.Pair) []compare.Pair { p[3].Item = ""; return p }, "no item on it"},
		{"a judgement with no rater", func(p []compare.Pair) []compare.Pair { p[3].Rater = ""; return p }, "does not say who made it"},
		{"one person reading a pair twice", func(p []compare.Pair) []compare.Pair {
			p[3].Item, p[3].Rater = p[2].Item, p[2].Rater
			return p
		}, "is one person reading the same pair twice"},
		{"a system against itself", func(p []compare.Pair) []compare.Pair { p[3].Right = p[3].Left; return p }, "on both sides"},
		{"a choice that is not a choice", func(p []compare.Pair) []compare.Pair { p[3].Choice = "better"; return p }, `"better"`},
		{"a third system", func(p []compare.Pair) []compare.Pair { p[3].Right = "com-8b-sft-mixed"; return p }, "two system protocol"},
		{"every judgement a tie", func(p []compare.Pair) []compare.Pair {
			for i := range p {
				p[i].Choice = compare.Tie
			}
			return p
		}, "no win rate to report"},
		{"nobody read anything twice", func(p []compare.Pair) []compare.Pair {
			out := make([]compare.Pair, 0, len(p))
			seen := map[string]bool{}
			for _, one := range p {
				if seen[one.Item] {
					continue
		...
		t.Run(tc.name, func(t *testing.T) {
			r := compare.Read(tc.spoil(build{items: 400}.pairs()))
