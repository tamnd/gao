...
func TestARealLogIsReadRowForRow(t *testing.T) {
	steps, err := spike.ReadSteps("testdata/on-dinh.jsonl")
	if err != nil {
...
`)
	...
	steps, err := spike.ReadSteps(path)
	if err != nil {
...
func TestARowWithNothingToReadIsRefusedAndNamed(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(c.name, func(t *testing.T) {
			_, err := spike.ReadSteps(write(t, "hong.jsonl", c.body))
			if err == nil {
...
func TestALogThatIsNotThereSaysSo(t *testing.T) {
	_, err := spike.ReadSteps(filepath.Join(t.TempDir(), "khong-co.jsonl"))
