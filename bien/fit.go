...
func Available(b may.Box) int64 {
...
func (c Cost) Fits(b may.Box) bool { return c.Total <= Available(b) }

// Headroom is the share of the box's frontier memory the plan does not use.
func (c Cost) Headroom(b may.Box) float64 {
	a := Available(b)
	if a <= 0 {
		return 0
	}
...
func (c Cost) Blocking(b may.Box) []string {
	var out []string
	a := Available(b)
	if a <= 0 {
