...
func healthy() []repeat.Doc {
	r := rand.New(rand.NewSource(1))
	docs := make([]repeat.Doc, 0, 400)
	for i := range 400 {
		docs = append(docs, repeat.Doc{
...
func set(docs []repeat.Doc) repeat.Set { return repeat.Read("gao-synth-1.0", docs) }
...
func TestASetThatCannotBeMeasuredIsRefused(t *testing.T) {
	for _, tc := range []struct {
		name  string
		spoil func(docs []repeat.Doc) []repeat.Doc
		want  string
	}{
		{"no documents", func([]repeat.Doc) []repeat.Doc { return nil }, "holds no documents"},
		{"too few documents", func(docs []repeat.Doc) []repeat.Doc { return docs[:100] }, "under the 200 this measure needs"},
		{"a document with no identity", func(docs []repeat.Doc) []repeat.Doc { docs[3].ID = ""; return docs }, "no identity"},
		{"a document twice", func(docs []repeat.Doc) []repeat.Doc { docs[3].ID = docs[2].ID; return docs }, "appears twice"},
		{"a document with no prompt", func(docs []repeat.Doc) []repeat.Doc { docs[3].Prompt = ""; return docs }, "does not say which prompt made it"},
		{"a kept document with no text", func(docs []repeat.Doc) []repeat.Doc { docs[3].Text = "  "; return docs }, "was kept and holds no text"},
		{"everything rejected", func(docs []repeat.Doc) []repeat.Doc {
			for i := range docs {
				docs[i].Kept = false
			}
			return docs
