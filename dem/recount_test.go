...
// what the columns in the store should add up to.
...
func TestTheCorpusRecountsOutOfItsColumns(t *testing.T) {
	...
	got, err := RecountOf(t.Context(), s.store(), snapshot, t.TempDir(), nil)
	if err != nil {
...
func TestRecountingDoesNotMoveTheText(t *testing.T) {
	...
	if _, err := RecountOf(t.Context(), s.store(), snapshot, t.TempDir(), nil); err != nil {
		t.Fatalf("RecountOf: %v", err)
	}
...
func TestARecountResumesAtThePart(t *testing.T) {
	...
	work := t.TempDir()
	first, err := RecountOf(t.Context(), s.store(), snapshot, work, nil)
	if err != nil {
		t.Fatalf("RecountOf: %v", err)
	}
	...
	second, err := RecountOf(t.Context(), s.store(), snapshot, work, nil)
	if err != nil {
...
func TestARecountThrowsAwayTheLineItWasWritingWhenItDied(t *testing.T) {
	...
	work := t.TempDir()
	want, err := RecountOf(t.Context(), s.store(), snapshot, work, nil)
	if err != nil {
		t.Fatalf("RecountOf: %v", err)
	}
	...
	got, err := RecountOf(t.Context(), s.store(), snapshot, work, nil)
	if err != nil {
...
func TestARecountRefusesAResumeFileItDidNotWrite(t *testing.T) {
	...
	_, err := RecountOf(t.Context(), s.store(), snapshot, work, nil)
	if err == nil {
...
func TestARecountOfASnapshotThatIsNotThereSaysSo(t *testing.T) {
	...
	_, err := RecountOf(t.Context(), s.store(), "hplt-v3-0000000000", t.TempDir(), nil)
	if err == nil {
		t.Fatal("recounting a snapshot the store does not hold succeeded")
	}
}
...
func TestAReportThatMatchesTheStoreAgrees(t *testing.T) {
	...
	if !Agree(diffs) {
		t.Errorf("a report that matches the store was reported as a difference: %+v", diffs)
	}
}
...
func TestAReportShortOfTheStoreIsCaughtAndTheColumnIsNamed(t *testing.T) {
	...
	if Agree(diffs) {
		t.Fatal("a report ten documents short of the store agreed with it")
	}
	off := diffs[0].Off()
...
// A source the store holds and the report never mentions is the difference worth
...
// onlyColumns is the counts as the store can know them, which is everything but
