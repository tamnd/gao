...
type Store struct {
	...
	// API overrides [kho.HubAPI]. It is for tests.
	API string
...
func (s *Store) pusher() *kho.Pusher {
	return &kho.Pusher{Repo: s.Repo, Token: s.Token, API: s.API}
}

func (s *Store) fetcher() *gat.Fetcher {
	return &gat.Fetcher{Token: s.Token}
}
...
func (s *Store) Snapshots(ctx context.Context) ([]string, error) {
	files, err := s.pusher().List(ctx, kho.DataDir)
	if err != nil {
		return nil, err
	}
	...
	for _, f := range files {
		...
		if snapshot, _, _, ok := kho.ParseStagePath(f.Path); ok {
			seen[snapshot] = true
		}
	}
...
// what it finds.
func keysOfPart(ctx context.Context, s *Store, part kho.Stored, path string, moved int64) (Keys, int64, error) {
...
func partKeys(path string) string {
	if _, file, part, ok := kho.ParseStagePath(path); ok {
		return fmt.Sprintf("f%05d-p%05d%s", file, part, KeysExt)
	}
	name := strings.TrimSuffix(filepath.Base(path), kho.ParquetExt)
	return name + KeysExt
}
