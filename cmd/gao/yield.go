...
func runYield(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	r, err := yield.ReadRun(*crawl, fs.Arg(0))
	if err != nil {
	...
	if *asJSON {
		...
		printYield(stdout, p, report)
	}
	if len(report.Faults) > 0 {
		return 1
	}
	if report.Verdict.Call == yield.Stop {
		return 2
	}
	return 0
}
...
type yieldReport struct {
	...
	Verdict yield.Verdict `json:"verdict"`
	...
	Budget *yield.Budget `json:"budget,omitempty"`

	Faults []string `json:"faults,omitempty"`
}

func printYield(w io.Writer, p yield.Point, r yieldReport) {
	...
	if holding, forum, news := p.Holding(); holding {
		...
		forum, news := p.By[yield.Forum].Tokens, p.By[yield.News].Tokens
...
func printYieldBudget(w io.Writer, b yield.Budget) {
