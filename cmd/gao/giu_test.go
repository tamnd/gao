...
// giuPanel writes a retention file where every specialist gained twelve points,
// the distillation kept the given share of that, and merging the same
// checkpoints kept two thirds.
func giuPanel(t *testing.T, kept, merged float64) string {
	t.Helper()
	names := [][2]string{
...
func TestRetentionIsReportedOneSpecialistAtATime(t *testing.T) {
	out, errOut, code := exec(t, "giu", giuPanel(t, 0.93, 0.66))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestAPanelThatMergingMatchesExitsNonZero(t *testing.T) {
	out, errOut, code := exec(t, "giu", giuPanel(t, 0.93, 0.89))
	if code != 2 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestAPanelThatCannotCarryARetentionIsRefused(t *testing.T) {
	...
	out, errOut, code := exec(t, "giu", path)
	if code != 1 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestTheRetentionIsAlsoMachineReadable(t *testing.T) {
	out, errOut, code := exec(t, "giu", "-json", giuPanel(t, 0.93, 0.66))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
	var got struct {
...
func TestGiuRefusesWhatItCannotRead(t *testing.T) {
	if _, _, code := exec(t, "giu"); code != 2 {
		t.Errorf("no argument exited %d", code)
	}
	if _, _, code := exec(t, "giu", "a.jsonl", "b.jsonl"); code != 2 {
		t.Errorf("two arguments exited %d", code)
	}
	if _, _, code := exec(t, "giu", filepath.Join(t.TempDir(), "missing.jsonl")); code != 1 {
