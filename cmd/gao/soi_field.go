...
`)
	...
	report := soiFieldReport{
		...
		Budget: soi.Budget(), Blocking: f.Blocking(), Verdict: f.Verdict(),
	}
	if l, ok := f.Leads(); ok {
	...
	for _, c := range f.Ranked() {
		report.Results = append(report.Results, soiResult{
...
// soiResult is one engine as the table carries it, which is rates and the card
// they were measured on rather than the counts underneath.
type soiResult struct {
...
type soiFieldReport struct {
	Card      string   `json:"card"`
	CardBytes int64    `json:"card_bytes"`
	Pages     int64    `json:"pages"`
	Gate      soi.Gate `json:"gate"`

	Engines int         `json:"engines"`
	Losers  int         `json:"losers"`
	Results []soiResult `json:"results"`
...
func printSoiField(w io.Writer, f soi.Field, b may.Box) {
	...
	fmt.Fprintf(w, "Hours are for the %.1fM pages the plan routes to OCR, against the %.0f OCR has of the extraction stage's %.0f.\n",
		float64(f.Pages)/1e6, soi.Budget(), soi.Hours)
