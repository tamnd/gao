...
func TestLuatPrintsEveryQuestionWithSomethingToActOn(t *testing.T) {
	...
	if code != 0 {
		t.Fatalf("gao luat: exit %d, want 0", code)
	}
	if !strings.Contains(out, luat.FiledOn) {
		t.Error("gao luat did not print the filing date, so an old agenda reads as a current one")
	}
	for _, q := range luat.Questions() {
		if !strings.Contains(out, q.ID) {
			t.Errorf("gao luat did not print %s", q.ID)
		}
		if !strings.Contains(out, q.Position()) {
			t.Errorf("gao luat printed %s without the position gao acts on", q.ID)
		}
	}
}
...
func TestLuatPrintsEveryDeterminationAndWhatShips(t *testing.T) {
	...
	for _, d := range luat.Determinations() {
		if !strings.Contains(out, d.Subject) {
			t.Errorf("gao luat did not print the determination for %s", d.Subject)
		}
	}
	for _, p := range luat.Publications() {
		if !strings.Contains(out, p.Class.String()) {
			t.Errorf("gao luat did not print what ships for %s", p.Class)
		}
	}
	// Both numbers, which is the whole rule about the headline.
	if !strings.Contains(out, "210B") || !strings.Contains(out, "300B") {
		t.Error("gao luat printed one of the two token numbers rather than both")
	}
}
...
func TestLuatPrintsTheFallbackWithoutBeingAsked(t *testing.T) {
	...
	if !strings.Contains(out, luat.RecipeOnly.Then) {
		t.Error("gao luat did not print the fallback")
	}
	for _, s := range luat.RecipeOnly.Publishes {
		if !strings.Contains(out, s) {
			t.Errorf("gao luat did not print that the fallback still ships %q", s)
		}
	}
	if !strings.Contains(out, luat.RecipeOnly.Withholds) {
		t.Error("gao luat did not print what the fallback withholds")
	}
}
...
func TestLuatVerboseAddsTheEvidenceAndTheStakes(t *testing.T) {
	...
	if code != 0 {
		t.Fatalf("gao luat -v: exit %d, want 0", code)
	}
	if len(full) <= len(plain) {
		t.Error("gao luat -v printed no more than the plain listing")
	}
	for _, d := range luat.Determinations() {
		if !strings.Contains(full, d.Evidence) {
			t.Errorf("gao luat -v did not print the evidence for %s", d.Subject)
		}
		if strings.Contains(plain, d.Evidence) {
			t.Errorf("gao luat printed the evidence for %s without -v", d.Subject)
		}
	}
	q, _ := luat.Ask("Q5")
	if !strings.Contains(full, q.Stakes) {
		t.Error("gao luat -v did not print what Q5 changes")
	}
}
...
func TestLuatPrintsOneQuestion(t *testing.T) {
	...
	if code != 0 {
		t.Fatalf("gao luat -q Q5: exit %d, want 0", code)
	}
	q, ok := luat.Ask("Q5")
	if !ok {
...
func TestLuatRejectsAQuestionThatDoesNotExist(t *testing.T) {
	...
	if code != 2 {
		t.Fatalf("gao luat -q Q99: exit %d, want 2", code)
	}
	if out != "" {
		t.Errorf("gao luat -q Q99 printed to stdout: %q", out)
	}
...
func TestLuatPrintsOnePath(t *testing.T) {
	...
	if !strings.Contains(out, "open") || !strings.Contains(out, "restricted") {
		t.Error("gao luat -source printed one license class for the crawl")
	}
}
...
func TestLuatRejectsAPathThatDoesNotExist(t *testing.T) {
	...
	if code != 2 {
		t.Fatalf("gao luat -source gao-vibes: exit %d, want 2", code)
	}
...
func TestLuatRejectsAnUnknownFlag(t *testing.T) {
	if _, _, code := exec(t, "luat", "-nope"); code != 2 {
		t.Errorf("gao luat -nope: exit %d, want 2", code)
	}
}
