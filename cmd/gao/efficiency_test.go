...
func TestTheArchitectureIsPrintedWithItsArithmetic(t *testing.T) {
	out, errOut, code := exec(t, "efficiency", "model")
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestTheModelIsAvailableAsJSON(t *testing.T) {
	out, errOut, code := exec(t, "efficiency", "model", "-json")
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
	var got efficiencyModelReport
...
func TestThePlanIsAPurchaseOrder(t *testing.T) {
	out, errOut, code := exec(t, "efficiency", "plan")
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestHalfTheUtilizationIsTwiceTheCompute(t *testing.T) {
	at := func(mfu string) efficiencyPlanReport {
		t.Helper()
		out, errOut, code := exec(t, "efficiency", "plan", "-json", "-mfu", mfu)
		if code != 0 {
			t.Fatalf("exit %d: %s", code, errOut)
		}
		var got efficiencyPlanReport
		if err := json.Unmarshal([]byte(out), &got); err != nil {
			t.Fatal(err)
		}
		return got
...
func TestPlanningFP8OntoHardwareWithoutItFails(t *testing.T) {
	_, errOut, code := exec(t, "efficiency", "plan", "-instance", "a100-sxm")
	if code != 1 {
	...
	_, errOut, code = exec(t, "efficiency", "plan", "-instance", "a100-sxm", "-precision", "bf16")
	if code != 0 {
...
func TestHardwareNobodyPricedIsAUsageError(t *testing.T) {
	_, errOut, code := exec(t, "efficiency", "plan", "-instance", "tpu-v6")
	if code != 2 {
...
func stepLog(t *testing.T, at ...float64) string {
	t.Helper()
	m := efficiency.Com()
	h, _ := efficiency.Lookup("h100-sxm")
	peak, _ := h.Peak(efficiency.FP8)
...
func TestARunThatHoldsItsNumberReadsClean(t *testing.T) {
	out, errOut, code := exec(t, "efficiency", "read", stepLog(t, held(0.44, 100)...))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestARunThatIsDyingExitsNonZero(t *testing.T) {
	...
	out, _, code := exec(t, "efficiency", "read", stepLog(t, decline...))
	if code != 1 {
...
func TestAStepWithNoHardwareOnItFailsTheRun(t *testing.T) {
	...
	out, _, code := exec(t, "efficiency", "read", path)
	if code != 1 {
		t.Fatalf("exit %d, want 1:\n%s", code, out)
	}
...
func TestTheRunIsAvailableAsJSON(t *testing.T) {
	out, errOut, code := exec(t, "efficiency", "read", "-json", stepLog(t, held(0.44, 50)...))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
	var got efficiencyReadReport
	if err := json.Unmarshal([]byte(out), &got); err != nil {
		t.Fatal(err)
	}
	if !got.Passes || got.Steps != 50 || len(got.Windows) != efficiency.WindowCount {
...
func TestATrainingLogThatIsNotThereIsAFailure(t *testing.T) {
	_, errOut, code := exec(t, "efficiency", "read", filepath.Join(t.TempDir(), "nothing.jsonl"))
	if code != 1 {
		t.Fatalf("exit %d, want 1: %s", code, errOut)
	}
...
func TestEfficiencyWithoutASubcommandPrintsUsage(t *testing.T) {
	_, errOut, code := exec(t, "efficiency")
	if code != 2 {
		t.Fatalf("exit %d, want 2: %s", code, errOut)
	}
...
func TestAnUnknownEfficiencySubcommandSaysSo(t *testing.T) {
	_, errOut, code := exec(t, "efficiency", "mfu")
	if code != 2 {
		t.Fatalf("exit %d, want 2: %s", code, errOut)
	}
...
func TestEfficiencyHelpGoesToStdout(t *testing.T) {
	out, _, code := exec(t, "efficiency", "help")
	if code != 0 {
...
func TestEfficiencyReadWantsExactlyOneLog(t *testing.T) {
	_, errOut, code := exec(t, "efficiency", "read")
	if code != 2 {
		t.Fatalf("exit %d, want 2: %s", code, errOut)
	}
}
...
func TestTheCheckpointIntervalIsComputedRatherThanChosen(t *testing.T) {
	out, errOut, code := exec(t, "efficiency", "spot")
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestCapacityTakenBackFasterThanASaveIsNotAnIntervalProblem(t *testing.T) {
	out, _, code := exec(t, "efficiency", "spot", "-mean", "90s")
	if code != 1 {
		t.Fatalf("exit %d, want 1:\n%s", code, out)
	}
...
func TestAPublishableCheckpointIsNotAResumableOne(t *testing.T) {
	out, _, code := exec(t, "efficiency", "spot", "-weights")
	if code != 0 {
		t.Fatalf("exit %d:\n%s", code, out)
	}
...
func TestTheSpotPlanIsAlsoMachineReadable(t *testing.T) {
	out, _, code := exec(t, "efficiency", "spot", "-json")
	if code != 0 {
		t.Fatalf("exit %d:\n%s", code, out)
	}
	var report struct {
...
func TestSpotRefusesNumbersItCannotComputeFrom(t *testing.T) {
	if _, _, code := exec(t, "efficiency", "spot", "-rate", "0"); code != 2 {
		t.Error("a write rate of zero did not read as a usage error")
	}
	if _, _, code := exec(t, "efficiency", "spot", "extra"); code != 2 {
