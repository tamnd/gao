...
func TestNauCheckIsTheOneThatCanFail(t *testing.T) {
	...
	if code != 0 {
		t.Fatalf("gao nau check: exit %d, want 0\n%s", code, out)
	}
	if strings.TrimSpace(out) == "" {
		t.Error("gao nau check said nothing, so a passing plan and a silent one look the same")
	}
}
...
func TestNauBudgetPrintsEveryComponentAndWhatItAddsUpTo(t *testing.T) {
	...
	if code != 0 {
		t.Fatalf("gao nau budget: exit %d, want 0", code)
	}
	for _, c := range nau.Budget() {
		if !strings.Contains(out, c.Name) {
			t.Errorf("gao nau budget did not print %s", c.Name)
		}
	}
	for _, want := range []string{"66% Vietnamese", "34% anchor", "309B"} {
		if !strings.Contains(out, want) {
			t.Errorf("gao nau budget did not print %q, which is the line the mixture is argued from", want)
		}
	}
}
...
func TestNauBudgetDoesNotLeaveTheCorpusTargetToBeAddedUp(t *testing.T) {
	...
	if strings.Contains(out, "379B") {
		t.Error("gao nau budget printed the sum of every unique count, which asks the crawl for the quality tiers twice")
	}
}
...
func TestNauCurriculumPrintsEveryPhaseAndWhatItReads(t *testing.T) {
	...
	if code != 0 {
		t.Fatalf("gao nau curriculum: exit %d, want 0", code)
	}
	for _, p := range nau.Curriculum() {
		if !strings.Contains(out, p.Name) {
			t.Errorf("gao nau curriculum did not print the %s phase", p.Name)
		}
		if !strings.Contains(out, p.Why) {
			t.Errorf("gao nau curriculum printed %s without the argument for it", p.Name)
		}
		for _, s := range p.Mix {
			if !strings.Contains(out, s.Component) {
				t.Errorf("gao nau curriculum did not print %s in the %s phase", s.Component, p.Name)
			}
		}
	}
}
...
func TestNauReconcilePrintsBothSidesAndTheDifference(t *testing.T) {
	...
	if code != 0 {
		t.Fatalf("gao nau reconcile: exit %d, want 0", code)
	}
	for _, want := range []string{"buys", "spends", "off", "epochs"} {
		if !strings.Contains(out, want) {
			t.Errorf("gao nau reconcile has no %q column", want)
		}
	}
	if !strings.Contains(out, "gao-web") {
		t.Error("gao nau reconcile did not print the component with the widest gap")
	}
}
...
func TestNauQuestionsPrintsWhatIsStillOpen(t *testing.T) {
	...
	if code != 0 {
		t.Fatalf("gao nau questions: exit %d, want 0", code)
	}
	for _, q := range nau.Questions() {
		if !strings.Contains(out, q.ID) {
			t.Errorf("gao nau questions did not print %s", q.ID)
		}
		if !strings.Contains(out, q.Ask) {
			t.Errorf("gao nau questions printed %s without what it asks", q.ID)
		}
	}
}
...
func TestNauArmsPrintsTheRecipeOnceForAllThree(t *testing.T) {
	...
	if code != 0 {
		t.Fatalf("gao nau arms: exit %d, want 0", code)
	}
	for _, a := range nau.Arms() {
		if !strings.Contains(out, a.ID) {
			t.Errorf("gao nau arms did not print %s", a.ID)
		}
	}
	r := nau.Matched()
	...
	if !strings.Contains(out, r.Gate) {
		t.Error("gao nau arms did not print the gate, so a result has nothing to fail against")
	}
}
...
func TestNauFleetSaysTheRunDoesNotGoOnTheBoxesWeOwn(t *testing.T) {
	...
	if code != 0 {
		t.Fatalf("gao nau fleet: exit %d, want 0", code)
	}
	if !strings.Contains(out, "gamingpc") {
		t.Error("gao nau fleet did not name the box with the only GPU on it")
	}
	if !strings.Contains(out, "times short") {
		t.Error("gao nau fleet did not print how far short the fleet is, which is the number that ends the argument")
	}
}
...
func TestNauRefusesASubcommandItDoesNotHave(t *testing.T) {
	...
	if code != 2 {
		t.Errorf("gao nau train: exit %d, want 2", code)
	}
	if !strings.Contains(errOut, "train") {
		t.Error("gao nau did not say which subcommand it did not recognize")
	}
}
