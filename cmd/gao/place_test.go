...
import (
	...
	"github.com/tamnd/gao/place"
	"github.com/zeebo/blake3"
)

// placePilot writes the published frame drawn small enough to label in a test,
// which is what a pilot is for outside a test as well.
func placePilot(t *testing.T) (place.Frame, string) {
	t.Helper()
	f := place.Fixed()
	f.Size = 240
...
// placeLabels writes what a labeling pass that went well returns, with change
// applied first so a test can break exactly one thing.
func placeLabels(t *testing.T, f place.Frame, change func([]place.Label) []place.Label) string {
	t.Helper()
	digest := f.Digest()
	var labels []place.Label
	n := 0
	for _, sl := range f.Slices {
		for range sl.Wanted(f.Size) {
			band := place.Bands[n%len(place.Bands)]
			id := doc.Hash(blake3.Sum256(fmt.Appendf(nil, "doc-%d", n)))
			labels = append(labels, place.Label{Doc: id, Source: sl.Source, By: "an", Band: band, Frame: digest})
			if n%8 == 0 {
				second := band
				if n%32 == 0 {
					i := slices.Index(place.Bands, band)
					if i+1 < len(place.Bands) {
						second = place.Bands[i+1]
					} else {
						second = place.Bands[i-1]
					}
				}
				labels = append(labels, place.Label{Doc: id, Source: sl.Source, By: "binh", Band: second, Frame: digest})
			}
			n++
		}
	}
...
func TestTheFramePrintsTheDrawAndTheScale(t *testing.T) {
	out, errOut, code := exec(t, "place", "frame")
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestTheFrameSpeaksJSON(t *testing.T) {
	out, _, code := exec(t, "place", "frame", "-json")
	if code != 0 {
		t.Fatalf("exit %d", code)
	}
	var report struct {
		Frame  place.Frame `json:"frame"`
	...
	if len(report.Frame.Slices) != len(place.Fixed().Slices) {
...
func TestARubricWithNoBoundaryOnItIsReported(t *testing.T) {
	...
	out, _, code := exec(t, "place", "frame", "-frame", path)
	if code != 1 {
...
func TestALabelingThatMetTheGatePasses(t *testing.T) {
	f, frame := placePilot(t)
	out, errOut, code := exec(t, "place", "read", "-frame", frame, placeLabels(t, f, nil))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestARubricPeopleDoNotAgreeOnFailsTheGate(t *testing.T) {
	f, frame := placePilot(t)
	path := placeLabels(t, f, func(ls []place.Label) []place.Label {
		for i, l := range ls {
			if l.By == "binh" {
				j := slices.Index(place.Bands, l.Band)
				if j+1 < len(place.Bands) {
					ls[i].Band = place.Bands[j+1]
				} else {
					ls[i].Band = place.Bands[j-1]
				}
			}
	...
	out, _, code := exec(t, "place", "read", "-frame", frame, path)
	if code != 1 {
		t.Fatalf("exit %d, want 1", code)
	}
...
func TestADocumentFromOutsideTheDrawIsReported(t *testing.T) {
	...
	path := placeLabels(t, f, func(ls []place.Label) []place.Label {
		...
		return append(ls, place.Label{Doc: id, Source: "madlad400", By: "an", Band: place.Plain})
	})
	out, _, code := exec(t, "place", "read", "-frame", frame, path)
	if code != 1 {
		t.Fatalf("exit %d, want 1", code)
...
func TestTheLabelingSpeaksJSONToo(t *testing.T) {
	f, frame := placePilot(t)
	out, _, code := exec(t, "place", "read", "-json", "-frame", frame, placeLabels(t, f, nil))
	if code != 0 {
		t.Fatalf("exit %d", code)
	}
	var score struct {
	...
	if len(score.ByBand) != len(place.Bands) {
...
func TestNoLabelFileAsksForTheUsage(t *testing.T) {
	_, errOut, code := exec(t, "place", "read")
	if code != 2 {
...
func TestALabelFileThatIsNotThereSaysSo(t *testing.T) {
	_, errOut, code := exec(t, "place", "read", filepath.Join(t.TempDir(), "nope.jsonl"))
	if code != 1 {
...
func TestAFrameFileThatIsNotThereSaysSoToo(t *testing.T) {
	_, errOut, code := exec(t, "place", "frame", "-frame", filepath.Join(t.TempDir(), "nope.json"))
	if code != 1 {
		t.Errorf("exit %d, want 1", code)
	}
	if !strings.Contains(errOut, "gao place:") {
...
func TestNoPlaceSubcommandAsksForTheUsage(t *testing.T) {
	_, errOut, code := exec(t, "place")
	if code != 2 {
		t.Errorf("exit %d, want 2", code)
	}
...
func TestAnUnknownPlaceSubcommandIsNamed(t *testing.T) {
	_, errOut, code := exec(t, "place", "label")
	if code != 2 {
		t.Errorf("exit %d, want 2", code)
	}
...
// placeAgreed writes a labeling where the second opinions are on the documents
...
func placeAgreed(t *testing.T, f place.Frame, bands [][2]place.Band) string {
	t.Helper()
	digest := f.Digest()
	var labels []place.Label
	n, seen := 0, 0
	for _, sl := range f.Slices {
		for range sl.Wanted(f.Size) {
			...
			n++
			if !f.Doubled(id) {
				labels = append(labels, place.Label{Doc: id, Source: sl.Source, By: "an", Band: place.Plain, Frame: digest})
				continue
			}
			pair := bands[seen%len(bands)]
			seen++
			labels = append(labels,
				place.Label{Doc: id, Source: sl.Source, By: "an", Band: pair[0], Frame: digest},
				place.Label{Doc: id, Source: sl.Source, By: "binh", Band: pair[1], Frame: digest},
			)
		}
	}
...
func TestPerfectAgreementOnOneBandIsReportedAsChance(t *testing.T) {
	f, frame := placePilot(t)
	labels := placeAgreed(t, f, [][2]place.Band{{place.Plain, place.Plain}})

	out, _, code := exec(t, "place", "agree", "-frame", frame, labels)
	if code != 1 {
...
func TestTheReportNamesTheLineTheRubricFailsOn(t *testing.T) {
	f, frame := placePilot(t)
	labels := placeAgreed(t, f, [][2]place.Band{
		{place.Rich, place.Rich}, {place.Plain, place.Plain}, {place.Thin, place.Thin}, {place.Unusable, place.Unusable},
		{place.Plain, place.Thin}, {place.Rich, place.Rich}, {place.Unusable, place.Unusable}, {place.Rich, place.Rich},
	})
	...
	r, ok := f.Rule(place.Plain)
	if !ok || r.Confused != place.Thin {
...
func TestSecondOpinionsNobodyDrewAreReportedByTheCommand(t *testing.T) {
	f, frame := placePilot(t)
	out, _, code := exec(t, "place", "agree", "-frame", frame, placeLabels(t, f, nil))
	if code != 1 {
		t.Fatalf("exit %d, want 1:\n%s", code, out)
	}
...
func TestTheAgreementNumberSpeaksJSON(t *testing.T) {
	...
	out, _, code := exec(t, "place", "agree", "-json", "-frame", frame, labels)
	if code != 0 {
		t.Fatalf("exit %d:\n%s", code, out)
	}
	var report struct {
...
func TestAgreeAsksForALabelFile(t *testing.T) {
	if _, _, code := exec(t, "place", "agree"); code != 2 {
		t.Error("no label file did not read as a usage error")
	}
	if _, errOut, code := exec(t, "place", "agree", filepath.Join(t.TempDir(), "nowhere.jsonl")); code != 1 || !strings.Contains(errOut, "nowhere.jsonl") {
