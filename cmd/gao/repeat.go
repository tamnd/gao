...
func runRepeat(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	docs, err := repeat.ReadDocs(fs.Arg(0))
	if err != nil {
		fmt.Fprintf(stderr, "gao repeat: %v\n", err)
		return 1
	}

	s := repeat.Read(*generator, docs)
	report := repeatReport{
...
type repeatReport struct {
	...
	Shapes  []repeat.Shape `json:"shapes,omitempty"`
	Prompts []repeat.Shape `json:"prompts,omitempty"`
...
func printRepeat(w io.Writer, r repeatReport) {
	if len(r.Blocking) > 0 {
	...
	fmt.Fprintf(w, "%s wrote %s and its own filter kept %d of them, which is %s rejected.\n",
		r.Generator, plural(r.Docs, "document"), r.Kept, percent(r.RejectRate))
...
func printShapes(w io.Writer, shapes []repeat.Shape) {
