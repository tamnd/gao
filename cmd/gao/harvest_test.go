...
func TestHarvestPinsPrintsEverySourceAndItsRevision(t *testing.T) {
	out, _, code := exec(t, "harvest", "pins")
	if code != 0 {
	...
	if !strings.Contains(out, harvest.PinnedOn()) {
		t.Error("gao harvest pins does not say when the manifest was pinned")
	}
	if !strings.Contains(out, fleet.GB(harvest.TotalBytes())) {
...
func TestHarvestPinsSaysWhichSourceIsGated(t *testing.T) {
	out, _, code := exec(t, "harvest", "pins")
	if code != 0 {
		t.Fatalf("gao harvest pins: exit %d, want 0", code)
	}
...
func TestHarvestPinsPrintsOneSourceInFull(t *testing.T) {
	out, _, code := exec(t, "harvest", "pins", "-source", "hplt3")
	if code != 0 {
		t.Fatalf("gao harvest pins -source hplt3: exit %d, want 0", code)
	}
	p, ok := harvest.Pin("hplt3")
	if !ok {
	...
	_, errOut, code := exec(t, "harvest", "pins", "-source", "nothing")
	if code == 0 {
...
func TestHarvestPinsListsEveryFileOnRequest(t *testing.T) {
	out, _, code := exec(t, "harvest", "pins", "-files")
	if code != 0 {
	...
	if missing > 0 {
		t.Errorf("gao harvest pins -files omitted %d of %d files", missing, harvest.Files())
	}
...
func TestHarvestRejectsWhatItCannotDo(t *testing.T) {
...
func TestHarvestIsInTheTopLevelHelp(t *testing.T) {
...
func TestDriftReportsMovedLoudlyAndUnreachableSeparately(t *testing.T) {
	unchanged := driftResult{
		...
		Drift:  harvest.Drift{Source: doc.SourceHPLT3, Pinned: "sha256:aaaa", Current: "sha256:aaaa"},
	}
	moved := driftResult{
...
func TestPinsShowsADroppedSourceAndSaysWhy(t *testing.T) {
	...
	if !strings.Contains(out, fleet.GB(harvest.DroppedBytes())) {
		t.Errorf("the header does not say how much is pinned and not fetched:\n%s", out)
	}

	out, _, code = exec(t, "harvest", "pins", "-source", "madlad400")
	if code != 0 {
		t.Fatalf("exit %d, want 0", code)
	}
...
func TestHFRefusesADroppedSourceWithTheReason(t *testing.T) {
	_, errOut, code := exec(t, "harvest", "hf", "-dir", t.TempDir(), "-source", "madlad400", "-plan")
	if code != 1 {
...
func TestTheDefaultPlanLeavesOutADroppedSource(t *testing.T) {
	out, _, code := exec(t, "harvest", "hf", "-dir", t.TempDir(), "-plan")
	if code != 0 {
		t.Fatalf("exit %d, want 0", code)
	}
	if !strings.Contains(out, fleet.GB(harvest.TotalBytes())) {
	...
	if strings.Contains(out, fleet.GB(harvest.TotalBytes()+harvest.DroppedBytes())) {
...
func TestHarvestAgentPrintsTheTokenAndTheHeaderSeparately(t *testing.T) {
	out, _, code := exec(t, "harvest", "agent")
	if code != 0 {
		t.Fatalf("gao harvest agent: exit %d\n%s", code, out)
	}

	for _, want := range []string{harvest.Bot, harvest.Contact, "User-agent: " + harvest.Bot, "Disallow: /"} {
		if !strings.Contains(out, want) {
	...
	if !strings.Contains(out, harvest.Agent(version)) {
		t.Errorf("gao harvest agent does not print the header it sends:\n%s", out)
	}
}

func TestHarvestAgentTakesNoArguments(t *testing.T) {
	if _, _, code := exec(t, "harvest", "agent", "extra"); code != 2 {
		t.Errorf("gao harvest agent with an argument: exit %d, want 2", code)
	}
}

func TestHarvestAgentIsInTheSubcommandList(t *testing.T) {
	out, _, code := exec(t, "harvest", "help")
	if code != 0 {
...
func TestHarvestFetchPrintsTheDecisionAndNotJustThePage(t *testing.T) {
	s := fetchSite(t, map[string]http.HandlerFunc{
		"/robots.txt": html("User-agent: *\nAllow: /\n"),
		"/bai-viet":   html("<p>xin chào</p>"),
	})

	out, errOut, code := exec(t, "harvest", "fetch", "-delay", "1ms", s.URL+"/bai-viet")
	if code != 0 {
...
func TestHarvestFetchRefusesWhatRobotsRefusesAndSaysWhy(t *testing.T) {
	s := fetchSite(t, map[string]http.HandlerFunc{
		"/robots.txt":       html("User-agent: gaobot\nDisallow: /thanh-vien/\n"),
		"/thanh-vien/nguoi": html("<p>ho so</p>"),
	})

	out, errOut, code := exec(t, "harvest", "fetch", "-delay", "1ms", s.URL+"/thanh-vien/nguoi")
	if code != 1 {
...
func TestHarvestFetchWithBodyWritesTheBytesAndNothingElse(t *testing.T) {
	...
	out, _, code := exec(t, "harvest", "fetch", "-delay", "1ms", "-body", s.URL+"/bai-viet")
	if code != 0 {
...
func TestHarvestFetchNeedsSomethingToFetch(t *testing.T) {
	if _, _, code := exec(t, "harvest", "fetch"); code != 2 {
		t.Errorf("gao harvest fetch with no URL: exit %d, want 2", code)
	}
}

func TestHarvestFetchIsInTheSubcommandList(t *testing.T) {
	out, _, code := exec(t, "harvest", "help")
	if code != 0 {
		t.Fatalf("gao harvest help: exit %d", code)
	}
...
func TestAFetchWritesAWARCThatComesBackOut(t *testing.T) {
	...
	out, errOut, code := exec(t, "harvest", "fetch", "-delay", "1ms", "-warc", path, s.URL+"/bai-viet")
	if code != 0 {
		t.Fatalf("gao harvest fetch -warc: exit %d\n%s\n%s", code, out, errOut)
	}

	out, errOut, code = exec(t, "harvest", "warc", "-uri", s.URL+"/bai-viet", path)
	if code != 0 {
...
func TestTheListingSaysWhatIsInTheFile(t *testing.T) {
	...
	if _, errOut, code := exec(t, "harvest", "fetch", "-delay", "1ms", "-warc", path, s.URL+"/mot", s.URL+"/hai"); code != 0 {
		t.Fatalf("gao harvest fetch -warc: exit %d\n%s", code, errOut)
	}

	out, _, code := exec(t, "harvest", "warc", path)
	if code != 0 {
...
func TestTheFieldsCarryWhatTheSummaryWouldHaveLost(t *testing.T) {
	...
	if _, errOut, code := exec(t, "harvest", "fetch", "-delay", "1ms", "-warc", path, s.URL+"/tin/mot"); code != 0 {
		t.Fatalf("gao harvest fetch -warc: exit %d\n%s", code, errOut)
	}

	out, _, code := exec(t, "harvest", "warc", "-fields", path)
	if code != 0 {
...
func TestAskingForAPageTheCrawlNeverFetched(t *testing.T) {
	...
	if _, errOut, code := exec(t, "harvest", "fetch", "-delay", "1ms", "-warc", path, s.URL+"/mot"); code != 0 {
		t.Fatalf("gao harvest fetch -warc: exit %d\n%s", code, errOut)
	}

	out, errOut, code := exec(t, "harvest", "warc", "-uri", s.URL+"/hai", path)
	if code == 0 {
...
func TestReadingSomethingThatIsNotAWARC(t *testing.T) {
	...
	_, errOut, code := exec(t, "harvest", "warc", path)
	if code == 0 {
...
func TestHarvestWARCNeedsAFile(t *testing.T) {
	if _, _, code := exec(t, "harvest", "warc"); code != 2 {
		t.Error("gao harvest warc with no file did not report the usage")
	}
	if _, errOut, code := exec(t, "harvest", "warc", filepath.Join(t.TempDir(), "missing.warc")); code != 1 {
		t.Errorf("a file that does not exist: exit %d, want 1: %s", code, errOut)
	}
}

func TestHarvestWARCIsInTheSubcommandList(t *testing.T) {
	out, _, code := exec(t, "harvest", "help")
	if code != 0 {
		t.Fatalf("gao harvest help: exit %d", code)
	}
