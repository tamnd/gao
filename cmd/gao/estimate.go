...
import (
	...
	"github.com/tamnd/gao/estimate"
)

func runEstimate(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	s, err := estimate.ReadSource(*source, *snapshot, *seed, *parts, *bytes, fs.Arg(0))
	if err != nil {
	...
	report := estimateReport{
		...
		High:      s.High(),
		Width:     s.Width(),
		Line:      estimate.MaxWidth,
		Floor:     estimate.MinSample,
		Needed:    s.Needed(estimate.MaxWidth),
		Holds:     s.Holds(),
		Blocking:  s.Blocking(),
		Verdict:   s.Verdict(),
	}
...
func printEstimate(w io.Writer, s estimate.Source, r estimateReport) {
	...
	if r.Needed > 0 {
		fmt.Fprintf(w, "Reaching the %s line takes %s more, which is the fourfold cost of halving an interval rather than a proportional one.\n",
			percent(r.Line), plural(int(r.Needed), "part"))
	}
	fmt.Fprintf(w, "\n%s\n", r.Verdict)
	if r.Against != "" {
		fmt.Fprintf(w, "%s\n", r.Against)
