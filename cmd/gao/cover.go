...
func runCover(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	if *recall {
		...
		return runCoverRecall(stdout, stderr, *asJSON)
	}
	lv, ok := cover.ParseLevel(*level)
	if !ok {
	...
	for _, name := range files {
		if !*report {
			...
			covered, found := cover.Redact(string(text), lv)
			tally.Add(lv, found)
		...
		for i, text := range texts {
			found := cover.Find(text)
			tally.Add(lv, found)
			rows = append(rows, coverRowOf(lv, label(name, i, len(texts)), found, *spans))
		}
	}
	if !*report {
		return 0
	...
	if *asJSON {
		return printJSON(stdout, stderr, coverReport{Level: lv.String(), Documents: rows, Total: coverTotalOf(tally)})
	}
	printCover(stdout, lv, rows, tally)
	return 0
}

// runCoverRecall prints how much of a set of hand marked documents the detectors
...
func runCoverRecall(stdout, stderr io.Writer, asJSON bool) int {
	set, err := cover.Labeled()
	if err != nil {
		fmt.Fprintf(stderr, "gao cover: %v\n", err)
		return 1
	}
	...
	printCoverRecall(stdout, s)
	return 0
}

func printCoverRecall(w io.Writer, s cover.Score) {
...
type coverRow struct {
	...
	ByKind   map[cover.Kind]int `json:"by_kind,omitempty"`
	Found    []cover.Span       `json:"found,omitempty"`
}

func coverRowOf(level cover.Level, name string, found []cover.Span, withSpans bool) coverRow {
	row := coverRow{Document: name, Spans: int64(len(found)), ByKind: map[cover.Kind]int{}}
	for _, s := range found {
		row.ByKind[s.Kind]++
		if s.Cued {
			row.Cued++
...
type coverTotal struct {
	...
	Covered   int64                `json:"covered"`
	Cued      int64                `json:"cued"`
	ByKind    map[cover.Kind]int64 `json:"by_kind,omitempty"`
}

func coverTotalOf(t cover.Tally) coverTotal {
	return coverTotal{
...
type coverReport struct {
	...
	Documents []coverRow `json:"documents"`
	Total     coverTotal `json:"total"`
}

func printCover(w io.Writer, level cover.Level, rows []coverRow, t cover.Tally) {
	kinds := cover.Kinds()
	tw := tabwriter.NewWriter(w, 0, 0, 2, ' ', 0)
