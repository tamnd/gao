...
func runSpike(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	steps, err := spike.ReadSteps(fs.Arg(0))
	if err != nil {
		fmt.Fprintf(stderr, "gao spike: %v\n", err)
		return 1
	}

	c := spike.ReadCurve(*run, *total, *every, steps)
	report := spikeReport{
		Curve:    c,
		Window:   spike.Window,
		Rise:     spike.Rise,
		Scatter:  spike.Scatter,
		Faults:   c.Faults(),
		Blocking: c.Blocking(),
		Holds:    c.Holds(),
		Verdict:  c.Verdict(),
...
func printSpike(w io.Writer, r spikeReport, top int) {
	if len(r.Blocking) > 0 {
...
func back(s spike.Spike) string {
	if !s.Recovered() {
		return "never"
	}
