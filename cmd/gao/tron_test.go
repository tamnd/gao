...
// tronRows is the composed set, at the slate's shares, with a translated arm
...
func tronLine(source, capability, origin string, n int64, held bool) string {
	audit := ""
	if origin == "native" {
...
// tronSet writes the composed set, with each row's counts run through change
// before it is written, so a test moving one number moves one number.
func tronSet(t *testing.T, change func(capability string, native, translated, holdout int64) (int64, int64, int64)) string {
	t.Helper()
	var lines []string
	for _, r := range tronRows {
		...
		if native > 0 {
			lines = append(lines, tronLine("nguoi-viet-"+r.capability, r.capability, "native", native, false))
		}
		if translated > 0 {
			lines = append(lines, tronLine("dich-may-"+r.capability, r.capability, "translated", translated, false))
		}
		if holdout > 0 {
			lines = append(lines, tronLine("arm-"+r.capability, r.capability, "translated", holdout, true))
		}
	}
...
func TestTronReportsTheOriginsApartRatherThanAsATotal(t *testing.T) {
	out, errOut, code := exec(t, "tron", tronSet(t, nil))
	if code != 0 {
...
func TestTronPrintsTheSlateBeforeAnySetExists(t *testing.T) {
	out, _, code := exec(t, "tron", "-slate")
	if code != 0 {
	...
	if _, _, code := exec(t, "tron", "-slate", "somewhere.jsonl"); code != 2 {
		t.Error("the slate took a file argument")
	}
}

func TestTronExitsTwoWhenTheArmsWouldNotMeasureOrigin(t *testing.T) {
	// The arm composed the way somebody composes it when translated writing data
	// is the easiest thing to get, which it is.
	skewed := tronSet(t, func(capability string, native, translated, holdout int64) (int64, int64, int64) {
		if capability == "viet" {
			return native, translated, 90_000
		}
		return native, translated, holdout / 2
	})
	out, _, code := exec(t, "tron", skewed)
	if code != 2 {
	...
	small := tronSet(t, func(_ string, native, translated, holdout int64) (int64, int64, int64) {
		return native, translated, holdout / 6
	})
	out, _, code = exec(t, "tron", small)
	if code != 2 {
...
func TestTronRefusesASetWhoseWritingIsMostlyTranslated(t *testing.T) {
	path := tronSet(t, func(capability string, native, translated, holdout int64) (int64, int64, int64) {
		if capability == "viet" {
			return 48_000, 96_000, holdout
		}
		return native, translated, holdout
	})
	out, _, code := exec(t, "tron", path)
	if code != 1 {
...
func TestTronRefusesASetMissingACapabilityTheSlateNames(t *testing.T) {
	path := tronSet(t, func(capability string, native, translated, holdout int64) (int64, int64, int64) {
...
func TestTronJSONCarriesTheArmsAndTheComposition(t *testing.T) {
	out, _, code := exec(t, "tron", "-json", tronSet(t, nil))
	if code != 0 {
		t.Fatalf("exit %d\n%s", code, out)
	}
...
func TestTronWithoutASetIsAUsageError(t *testing.T) {
	if _, _, code := exec(t, "tron"); code != 2 {
		t.Error("tron with no argument did not exit 2")
	}
	if _, _, code := exec(t, "tron", filepath.Join(t.TempDir(), "nothing.jsonl")); code != 1 {
