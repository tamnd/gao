...
`)
	...
	lines := make([]bocLine, 0, len(pages))
	...
	if *asJSON {
		return printJSON(stdout, stderr, bocReport{Pages: lines, Threads: threadsIn(lines)})
	}
	printBoc(stdout, lines, *text, *furniture)
	return 0
}

// bocLine is one page and what came out of it.
type bocLine struct {
	Page string `json:"page"`
	boc.Thread
}

type bocReport struct {
	Pages []bocLine `json:"pages"`
...
func threadsIn(lines []bocLine) int {
	var n int
	for _, l := range lines {
		if l.Ok() {
			n++
...
func printBoc(w io.Writer, lines []bocLine, text, furniture bool) {
