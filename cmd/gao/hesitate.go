...
func runHesitate(stdout, stderr io.Writer, args []string) int {
	if len(args) == 0 {
		hesitateUsage(stderr)
		return 2
	}
	switch args[0] {
	case "items":
		return runHesitateItems(stdout, stderr, args[1:])
	case "grade":
		return runHesitateGrade(stdout, stderr, args[1:])
	case "help", "-h", "--help":
		hesitateUsage(stdout)
		return 0
	default:
...
func hesitateUsage(w io.Writer) {
...
func runHesitateItems(stdout, stderr io.Writer, args []string) int {
	...
	s, code := readHesitateSet(stderr, *path)
	if code != 0 {
		return code
	}
	faults := s.Faults()
	...
	switch {
	case *asJSON:
		if code := printJSON(stdout, stderr, hesitateItemsReport{Set: s, Digest: s.Digest().String(), Faults: faults}); code != 0 {
			return code
		}
	case *pairs:
		printHesitatePairs(stdout, s)
	default:
		printHesitateTopics(stdout, s)
	}
...
func runHesitateGrade(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	replies, err := hesitate.ReadReplies(fs.Arg(0))
	if err != nil {
	...
	out := hesitateGradeReport{Score: score, Faults: append(s.Faults(), score.Publishable()...)}
	if *asJSON {
		if code := printJSON(stdout, stderr, out); code != 0 {
			return code
		}
	} else {
		printHesitateScore(stdout, s, out)
	}
...
type hesitateItemsReport struct {
	Set    hesitate.Set `json:"set"`
	Digest string       `json:"digest"`
	Faults []string     `json:"faults,omitempty"`
}

type hesitateGradeReport struct {
	hesitate.Score
	Faults []string `json:"faults,omitempty"`
}

func readHesitateSet(stderr io.Writer, path string) (hesitate.Set, int) {
	if path == "" {
		return hesitate.Fixed(), 0
	}
	s, err := hesitate.ReadSet(path)
	if err != nil {
		fmt.Fprintf(stderr, "gao hesitate: %v\n", err)
		return hesitate.Set{}, 1
	}
	return s, 0
}

func printHesitateTopics(w io.Writer, s hesitate.Set) {
	fmt.Fprintf(w, "%s\n\n", s.Describe())
	...
	for _, topic := range s.Topics() {
		...
		for _, it := range s.Items {
			if it.Topic == topic && it.Kind == hesitate.Answer {
				n++
				if why == "" {
					why = it.Why
				}
	...
	fmt.Fprintf(w, "\ndigest %s, published as %s\n", s.Digest(), hesitate.Repo)
	fmt.Fprintf(w, "Run 'gao hesitate items -pairs' for the prompts themselves.\n")
}

func printHesitatePairs(w io.Writer, s hesitate.Set) {
	fmt.Fprintf(w, "%s\n", s.Describe())
	for _, it := range s.Items {
		if it.Kind != hesitate.Answer {
			continue
		}
...
func printHesitateScore(w io.Writer, s hesitate.Set, out hesitateGradeReport) {
	...
	if sc.Passed {
		...
		fmt.Fprintf(w, "fail: the model declined %.1f%% of what it should answer and %.1f%% of what it should not, against a ceiling of %.0f%% and a floor of %.0f%%.\n",
			100*sc.OverRefusal, 100*sc.HarmRefusal, 100*hesitate.MaxOverRefusal, 100*hesitate.MinHarmRefusal)
	}
