...
const (
	inspectPage      = "Tiếng Việt có sáu thanh điệu, và đó là chỗ mọi thứ hỏng.\n"
	inspectNoTones   = "Tiêng Viêt co sau thanh diêu, va do la chô moi thư hong.\n"
	inspectNoMarks   = "Tieng Viet co sau thanh dieu, va do la cho moi thu hong.\n"
	inspectOneWrong  = "Tiếng Việt có sáu thanh điệu, và đó là chổ mọi thứ hỏng.\n"
	inspectSecondRef = "Cộng hòa xã hội chủ nghĩa Việt Nam, độc lập, tự do, hạnh phúc.\n"
)
...
func TestInspectReportsBothRates(t *testing.T) {
	dir := t.TempDir()
	page := writeText(t, dir, "page.txt", inspectPage)
	read := writeText(t, dir, "read.txt", inspectNoMarks)

	var stdout, stderr bytes.Buffer
	if code := runInspect(&stdout, &stderr, []string{page, read}); code != 0 {
...
func TestInspectGateFailsALossyReading(t *testing.T) {
	dir := t.TempDir()
	page := writeText(t, dir, "page.txt", inspectPage)
	bad := writeText(t, dir, "bad.txt", inspectNoMarks)
	good := writeText(t, dir, "good.txt", inspectPage)

	var stdout, stderr bytes.Buffer
	if code := runInspect(&stdout, &stderr, []string{"-gate", page, bad}); code != 1 {
	...
	stdout.Reset()
	stderr.Reset()
	if code := runInspect(&stdout, &stderr, []string{"-gate", page, good}); code != 0 {
	...
	stdout.Reset()
	stderr.Reset()
	if code := runInspect(&stdout, &stderr, []string{page, bad}); code != 0 {
...
func TestInspectPrintsTheToneConfusionMatrix(t *testing.T) {
	dir := t.TempDir()
	page := writeText(t, dir, "page.txt", inspectPage)
	read := writeText(t, dir, "read.txt", inspectOneWrong)

	var stdout, stderr bytes.Buffer
	if code := runInspect(&stdout, &stderr, []string{"-matrix", page, read}); code != 0 {
	...
	if code := runInspect(&stdoutNoMatrix, &stderr, []string{page, read}); code != 0 {
...
func TestInspectScoresASetAsOneNumber(t *testing.T) {
	dir := t.TempDir()
	one := writeText(t, dir, "one.txt", inspectPage)
	oneRead := writeText(t, dir, "one-read.txt", inspectNoMarks)
	two := writeText(t, dir, "two.txt", inspectSecondRef)
	twoRead := writeText(t, dir, "two-read.txt", inspectSecondRef)

	var stdout, stderr bytes.Buffer
	if code := runInspect(&stdout, &stderr, []string{"-json", one, oneRead, two, twoRead}); code != 0 {
...
func TestInspectWantsPairs(t *testing.T) {
	dir := t.TempDir()
	page := writeText(t, dir, "page.txt", inspectPage)
	...
	for _, args := range [][]string{
		...
		var stdout, stderr bytes.Buffer
		if code := runInspect(&stdout, &stderr, args); code != 2 {
	...
	var stdout, stderr bytes.Buffer
	if code := runInspect(&stdout, &stderr, nil); code != 2 {
...
func TestInspectOnAFileThatIsNotThere(t *testing.T) {
	dir := t.TempDir()
	page := writeText(t, dir, "page.txt", inspectPage)

	var stdout, stderr bytes.Buffer
	if code := runInspect(&stdout, &stderr, []string{page, dir + "/nowhere.txt"}); code != 1 {
...
func TestInspectSeparatesTheDamageThatShowsFromTheDamageThatDoesNot(t *testing.T) {
	dir := t.TempDir()
	page := writeText(t, dir, "page.txt", inspectPage)
	tonesOnly := writeText(t, dir, "tones.txt", inspectNoTones)

	var stdout, stderr bytes.Buffer
	if code := runInspect(&stdout, &stderr, []string{page, tonesOnly}); code != 0 {
		t.Fatalf("gao inspect = %d, want 0\n%s", code, stderr.String())
	}
	out := stdout.String()
