...
// nhipStages writes the four stages of the pipeline, all of them publishable
// unless a test hands in something else.
func nhipStages(t *testing.T, lines ...string) string {
	t.Helper()
	if len(lines) == 0 {
		lines = []string{
...
func TestThePipelinePutsTheBoxOnEveryRate(t *testing.T) {
	out, errOut, code := exec(t, "nhip", nhipStages(t))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
	...
	counted, _, code := exec(t, "nhip", "-counted", "-docs", "412000000", nhipStages(t))
	if code != 0 {
...
func TestAStageThatOnlyLooksBusyExitsOne(t *testing.T) {
	...
	out, _, code := exec(t, "nhip", nhipStages(t,
		stageLine("normalize", 1400, 1<<30),
		half,
		stageLine("classify", 210, 2<<30),
		stageLine("dedup", 620, 1900<<20),
...
func TestThePipelineIsAlsoMachineReadable(t *testing.T) {
	out, errOut, code := exec(t, "nhip", "-json", nhipStages(t))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
	var got struct {
...
func TestNhipRefusesWhatItCannotRead(t *testing.T) {
	if _, _, code := exec(t, "nhip"); code != 2 {
		t.Errorf("no argument exited %d", code)
	}
	if _, _, code := exec(t, "nhip", "a.jsonl", "b.jsonl"); code != 2 {
		t.Errorf("two arguments exited %d", code)
	}
	if _, _, code := exec(t, "nhip", filepath.Join(t.TempDir(), "missing.jsonl")); code != 1 {
