...
import (
	...
	"github.com/tamnd/gao/bang"
	"github.com/tamnd/gao/nhat"
)

func runBang(stdout, stderr io.Writer, args []string) int {
	if len(args) == 0 {
		bangUsage(stderr)
		return 2
	}
	switch args[0] {
	case "board":
		return runBangBoard(stdout, stderr, args[1:], false)
	case "rows":
		return runBangBoard(stdout, stderr, args[1:], true)
	case "help", "-h", "--help":
		bangUsage(stdout)
		return 0
	default:
...
type bangReport struct {
	Roster   string    `json:"roster"`
	Arms     []bangArm `json:"arms"`
...
func runBangBoard(stdout, stderr io.Writer, args []string, byRow bool) int {
	name := "bang board"
	if byRow {
		name = "bang rows"
	}
	...
	for _, a := range board.Arms {
		report.Arms = append(report.Arms, bangArm{
	...
	case byRow:
		printBangRows(stdout, report)
	default:
		printBang(stdout, report)
	}
...
func printBang(w io.Writer, r bangReport) {
	if len(r.Blocking) > 0 {
		printBangRefusal(w, r)
		return
	}
	...
	for _, a := range r.Arms {
		fmt.Fprintf(tw, "%s\t%d\t%.1f\t%s\t%s\n",
			bangArmName(a.Origin), a.Benchmarks, a.Mean, bangMargin(a.Margin), bangDecided(a))
	}
	_ = tw.Flush()
	...
	fmt.Fprintf(w, "On the benchmarks gao built the model is %s, on everybody else's it is %s.\n",
		bangMargin(r.Own), bangMargin(r.Others))
...
func printBangRows(w io.Writer, r bangReport) {
	if len(r.Blocking) > 0 {
		printBangRefusal(w, r)
		return
	}
	...
	for _, a := range r.board.Arms {
		for _, s := range a.Scores {
			fmt.Fprintf(tw, "%s\t%s\t%s\t%.1f\t%.1f\t%s\t%d\t%.1f\t%s\n",
				s.Benchmark, bangArmName(a.Origin), bangBuiltBy(s.Benchmark), s.Score, s.Baseline,
				bangMargin(s.Margin()), s.Runs, s.Spread, yesno(s.Decided()))
		}
	}
	_ = tw.Flush()
...
func printBangRefusal(w io.Writer, r bangReport) {
...
// bangBuiltBy says who made the benchmark, which is the column that keeps the
// builder's own instruments visible in the table rather than in a footnote.
func bangBuiltBy(name string) string {
	if bang.Built(name) {
		return "gao"
	}
	return "elsewhere"
...
// bangMargin writes a margin the way it is read out loud, because a minus sign
// in a column of small numbers is easy to miss.
func bangMargin(f float64) string {
	switch {
	case f > 0:
...
// bangDecided is how much of an arm sits outside its own noise, which is the
// number that says how much of the arm is evidence.
func bangDecided(a bangArm) string {
