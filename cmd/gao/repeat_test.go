...
var repeatVocab = strings.Fields(`
...
func repeatText(r *rand.Rand, n int) string {
	out := make([]string, 0, n)
	for range n {
		out = append(out, repeatVocab[r.Intn(len(repeatVocab))])
	}
	return strings.Join(out, " ")
}

// repeatRun writes a generated run of 400 documents. Each body comes back from
// body, which is what the test is varying, and a tenth of the run is rejected.
func repeatRun(t *testing.T, body func(r *rand.Rand, i int) string) string {
	t.Helper()
...
func varied(t *testing.T) string {
	return repeatRun(t, func(r *rand.Rand, _ int) string { return repeatText(r, 120) })
}
...
func collapsed(t *testing.T) string {
	...
	for range 20 {
		sentences = append(sentences, repeatText(r, 14))
	}
	return repeatRun(t, func(_ *rand.Rand, i int) string {
		out := make([]string, 0, 4)
		for j := range 4 {
...
func TestRepeatPassesARunThatKeepsProducingNewMaterial(t *testing.T) {
	out, errOut, code := exec(t, "repeat", "-generator", "gao-synth-1.0", varied(t))
...
func TestRepeatCatchesASetThatHasRunOutOfThingsToSay(t *testing.T) {
	out, errOut, code := exec(t, "repeat", "-generator", "gao-synth-1.0", collapsed(t))
...
func TestRepeatNamesThePromptThatCarriedTheSet(t *testing.T) {
	...
	out, _, code := exec(t, "repeat", "-generator", "gao-synth-1.0", path)
	if code != 2 {
...
func TestRepeatPrintsTheSameReadingAsJSON(t *testing.T) {
	out, _, code := exec(t, "repeat", "-generator", "gao-synth-1.0", "-json", collapsed(t))
	if code != 2 {
...
func TestRepeatRefusesASetWithNoGenerator(t *testing.T) {
	out, errOut, code := exec(t, "repeat", varied(t))
...
func TestRepeatRefusesASetTooSmallToReadACurveOff(t *testing.T) {
	path := filepath.Join(t.TempDir(), "run.jsonl")
	lines := make([]string, 0, 40)
	r := rand.New(rand.NewSource(10))
	for i := range 40 {
		lines = append(lines, fmt.Sprintf(`{"id":"synth-%04d","prompt":"p01","text":%q,"kept":true}`, i, repeatText(r, 120)))
	}
	if err := os.WriteFile(path, []byte(strings.Join(lines, "\n")+"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestRepeatSaysWhichLineOfTheRunIsWrong(t *testing.T) {
	path := filepath.Join(t.TempDir(), "run.jsonl")
...
`), 0o600); err != nil {
	...
	_, errOut, code := exec(t, "repeat", "-generator", "gao-synth-1.0", path)
	if code != 1 {
...
func TestRepeatUsageErrors(t *testing.T) {
	if _, _, code := exec(t, "repeat"); code != 2 {
		t.Errorf("no file: exit %d, want 2", code)
	}

	_, errOut, code := exec(t, "repeat", "-h")
	if code != 2 {
...
func TestRepeatIsInTheCommandList(t *testing.T) {
