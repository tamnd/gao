...
func writeRuns(t *testing.T, runs []mill.Ablation) string {
	t.Helper()
...
func ablation(threshold, retention, score float64) mill.Ablation {
	return mill.Ablation{
		Threshold: threshold,
		Retention: retention,
		Score:     score,
		Noise:     0.3,
...
func TestMillChoosesTheThresholdFromTheRuns(t *testing.T) {
	path := writeRuns(t, []mill.Ablation{
	...
	if code := runMill(&stdout, &stderr, []string{"-choose", path, "-json"}); code != 0 {
...
func TestMillReportsACurveThatDidNotSeparate(t *testing.T) {
	path := writeRuns(t, []mill.Ablation{
	...
	var stdout, stderr bytes.Buffer
	if code := runMill(&stdout, &stderr, []string{"-choose", path}); code != 0 {
		t.Fatalf("gao mill -choose = %d, want 0\n%s", code, stderr.String())
	}
	out := stdout.String()
...
func TestMillExitsNonZeroOnRunsThatCannotChoose(t *testing.T) {
	path := writeRuns(t, []mill.Ablation{
		ablation(0.60, 0.71, 41.0),
		ablation(0.80, 0.84, 46.0),
	})
	...
	var stdout, stderr bytes.Buffer
	if code := runMill(&stdout, &stderr, []string{"-choose", path}); code != 1 {
...
func TestMillChooseSaysWhichFileItCouldNotRead(t *testing.T) {
	missing := filepath.Join(t.TempDir(), "khong-co.json")

	var stdout, stderr bytes.Buffer
	if code := runMill(&stdout, &stderr, []string{"-choose", missing}); code != 1 {
...
func TestMillChoosePrintsTheRunsUnderTheAnswer(t *testing.T) {
	path := writeRuns(t, []mill.Ablation{
		ablation(0.60, 0.71, 41.0),
		ablation(0.70, 0.79, 42.0),
		ablation(0.80, 0.84, 46.0),
