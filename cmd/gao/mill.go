...
func runMill(stdout, stderr io.Writer, args []string) int {
	fs := flag.NewFlagSet("mill", flag.ContinueOnError)
	fs.SetOutput(stderr)
	threshold := fs.Float64("threshold", mill.DefaultThreshold, "the similarity at which two documents are copies of each other")
...
`)
	...
	if *choose != "" {
		return runMillChoose(stdout, stderr, *choose, *asJSON)
	}
	if *hosts < 1 {
	...
	banding := mill.Default()
	if *curve {
		banding = mill.Wide()
	}
	index, err := mill.New(banding)
	if err != nil {
	...
	if *curve {
		reports := index.Curve(mill.CurveThresholds...)
		if *asJSON {
			return printJSON(stdout, stderr, millCurve{Banding: bandingOf(banding), Curve: reports})
		}
	...
	if *asJSON {
		return printJSON(stdout, stderr, millRun{Banding: bandingOf(banding), Report: report})
	}
...
func runOverlap(stdout, stderr io.Writer, files []string, threshold float64, asJSON bool) int {
	...
	o, err := mill.NewOverlap(mill.Wide())
	if err != nil {
		fmt.Fprintf(stderr, "gao mill: %v\n", err)
		return 1
	}
	for _, name := range files {
		...
		err := store.ScanPart(name, func(r store.Row) error {
			source := r.Source
			if source == "" {
				source = "unnamed"
			}
...
// runMillChoose applies the rule to a set of ablation runs.
//
...
func runMillChoose(stdout, stderr io.Writer, path string, asJSON bool) int {
	b, err := os.ReadFile(path)
	if err != nil {
		fmt.Fprintf(stderr, "gao mill: %v\n", err)
		return 1
	}
	var runs []mill.Ablation
	if err := json.Unmarshal(b, &runs); err != nil {
		fmt.Fprintf(stderr, "gao mill: %s: %v\n", path, err)
		return 1
	}
	c, err := mill.Choose(runs)
	if err != nil {
		for _, problem := range mill.CheckAblations(runs) {
...
func runBoiler(stdout, stderr io.Writer, files []string, hosts int, asJSON bool) int {
	...
	b := mill.NewBoiler(mill.DefaultFurniture())
	for _, name := range files {
		if err := store.ScanPart(name, func(r store.Row) error {
	...
	run.Hosts = b.Hosts()
	run.Furniture = mill.DefaultFurniture()
...
type boilerRun struct {
	Furniture mill.Furniture    `json:"furniture"`
	Hosts     int               `json:"hosts"`
	Documents int               `json:"documents"`
	Lines     int               `json:"lines"`
	Removed   int               `json:"removed"`
	Emptied   int               `json:"emptied"`
	Sites     []mill.HostReport `json:"sites,omitempty"`
}
...
func printBoiler(w io.Writer, run boilerRun, reports []mill.HostReport, top int) {
...
type millBanding struct {
	Bands int     `json:"bands"`
	Rows  int     `json:"rows"`
	Knee  float64 `json:"knee"`
}

func bandingOf(b mill.Banding) millBanding {
	return millBanding{Bands: b.Bands, Rows: b.Rows, Knee: b.Knee()}
}

type millRun struct {
	Banding millBanding `json:"banding"`
	Report  mill.Report `json:"report"`
}

type millCurve struct {
	Banding millBanding   `json:"banding"`
	Curve   []mill.Report `json:"curve"`
}
...
func readInto(x *mill.Index, name string) error {
	if filepath.Ext(name) != ".parquet" {
		text, err := os.ReadFile(name)
		if err != nil {
			return err
	...
	rows, err := store.ReadPart(name)
	if err != nil {
		return err
	}
	for _, r := range rows {
		x.Add(r.DocID, len([]rune(r.Text)), mill.Sign(r.Text))
	}
	return nil
}

func printDedup(w io.Writer, b mill.Banding, r mill.Report) {
	tw := tabwriter.NewWriter(w, 0, 0, 2, ' ', 0)
...
func printCurve(w io.Writer, reports []mill.Report) {
	tw := tabwriter.NewWriter(w, 0, 0, 2, ' ', 0)
