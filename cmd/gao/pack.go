...
`)
	...
	report := packReport{
		...
		Metadata: r.Metadata(),
		Share:    r.Share(),
		Max:      pack.MaxMetadata,
		Ceiling:  pack.Ceiling,
		Ratio:    r.Ratio(),
		Target:   pack.TargetShard,
		Holds:    r.Holds(),
		Blocking: r.Blocking(),
		Verdict:  r.Verdict(),
	}
	for _, c := range r.Columns() {
		report.Columns = append(report.Columns, packColumn{
			Name:         c.Name,
	...
	for _, p := range r.Loose() {
		report.Loose = append(report.Loose, packShard{Path: p.Path, Bytes: p.Bytes, Rows: p.Rows})
	}
	...
	if *asJSON {
		...
		printPack(stdout, report, *all, *columns)
	}
...
// packColumns is how many columns the table prints before folding the rest into
// one row.
const packColumns = 10

type packColumn struct {
...
type packReport struct {
	...
	Columns []packColumn `json:"columns"`
	...
	Loose  []packShard `json:"loose,omitempty"`
...
func printPack(w io.Writer, r packReport, all, columns bool) {
	...
	shown := r.Columns
	if !columns && len(shown) > packColumns {
		shown = shown[:packColumns]
	}
	...
	if server1, ok := fleet.Lookup("server1"); ok {
