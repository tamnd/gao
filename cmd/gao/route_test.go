...
func TestRouteRoutesEachDocumentAndCountsThem(t *testing.T) {
	dir := t.TempDir()
	a := writePDF(t, dir, "born-digital.pdf", pdfProse)
	b := writePDF(t, dir, "scan.pdf", "BT (Trang 1) Tj ET")

	out, _, code := exec(t, "route", a, b)
	if code != 0 {
...
func TestRouteLabelsTheDistributionWithTheBoxItRanOn(t *testing.T) {
	dir := t.TempDir()
	out, _, code := exec(t, "route", "-box", "gamingpc", writePDF(t, dir, "a.pdf", pdfProse))
	if code != 0 {
...
func TestRouteExplainsItselfWhenAsked(t *testing.T) {
	dir := t.TempDir()
	out, _, code := exec(t, "route", "-why", writePDF(t, dir, "a.pdf", pdfProse))
	if code != 0 {
...
func TestRouteFailsOnAFileItCannotRead(t *testing.T) {
	_, errOut, code := exec(t, "route", filepath.Join(t.TempDir(), "khong-co.pdf"))
	if code != 1 {
...
func TestRouteWithNoFilesSaysWhatItIsFor(t *testing.T) {
	_, errOut, code := exec(t, "route")
	if code != 2 {
