...
// listenSet writes a set the decoder got right unless a test says otherwise.
func listenSet(t *testing.T, lines ...string) string {
	t.Helper()
	if len(lines) == 0 {
		lines = []string{
...
func TestTheTranscriptReportLeadsWithTheTrackNearestAGate(t *testing.T) {
	out, errOut, code := exec(t, "listen", listenSet(t))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestADecoderLoopingOnSilenceExitsTwo(t *testing.T) {
	out, _, code := exec(t, "listen", listenSet(t,
...
func TestADecodeOnABoxWithNoCardInItExitsOne(t *testing.T) {
	...
	out, _, code := exec(t, "listen", listenSet(t, nowhere, trackLine("vov-phong-van-2", 640, 620, 1, 15400)))
	if code != 1 {
		t.Fatalf("exit %d: %s", code, out)
	}
	...
	out, _, code = exec(t, "listen", listenSet(t, cpu, trackLine("vov-phong-van-2", 640, 620, 1, 15400)))
	if code != 1 {
		t.Fatalf("exit %d: %s", code, out)
	}
	...
	out, _, code = exec(t, "listen", listenSet(t, big, trackLine("vov-phong-van-2", 640, 620, 1, 15400)))
	if code != 0 {
...
func TestTheTranscriptSetIsAlsoMachineReadable(t *testing.T) {
	out, errOut, code := exec(t, "listen", "-json", listenSet(t))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
	var got struct {
...
func TestListenRefusesWhatItCannotRead(t *testing.T) {
	if _, _, code := exec(t, "listen"); code != 2 {
		t.Errorf("no argument exited %d", code)
	}
	if _, _, code := exec(t, "listen", "a.jsonl", "b.jsonl"); code != 2 {
		t.Errorf("two arguments exited %d", code)
	}
	if _, _, code := exec(t, "listen", filepath.Join(t.TempDir(), "missing.jsonl")); code != 1 {
