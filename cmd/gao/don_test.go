...
func TestTheRotationTheCrawlIsPlannedAgainstWorks(t *testing.T) {
	out, errOut, code := exec(t, "don", "fit")
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestARotationThatDoesNotKeepUpExitsNonZero(t *testing.T) {
	out, _, code := exec(t, "don", "fit", "-uplink", "1500000")
	if code != 1 {
...
func TestEveryNumberCanBeArguedWith(t *testing.T) {
	out, _, code := exec(t, "don", "fit", "-confirm", "12h")
	if code != 1 {
		t.Fatalf("exit %d, want 1:\n%s", code, out)
	}
	...
	out, _, code = exec(t, "don", "fit", "-fetches", "50")
	if code != 0 {
...
func TestABoxWithNoRoomSaysSoRatherThanPrintingAPlan(t *testing.T) {
	out, _, code := exec(t, "don", "fit", "-box", "server2")
	if code != 1 {
		t.Fatalf("exit %d, want 1:\n%s", code, out)
	}
...
func TestABoxThatIsNotOnTheFleetIsAUsageError(t *testing.T) {
	_, errOut, code := exec(t, "don", "fit", "-box", "server9")
	if code != 2 {
...
func TestTheArithmeticIsAvailableAsJSON(t *testing.T) {
	out, errOut, code := exec(t, "don", "fit", "-json")
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
	var got donFitReport
...
func TestARotationThatTookEveryStepReadsClean(t *testing.T) {
	...
	out, errOut, code := exec(t, "don", "read", path)
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestALogThatDeletedSomethingUnconfirmedFails(t *testing.T) {
	...
	out, _, code := exec(t, "don", "read", path)
	if code != 1 {
		t.Fatalf("exit %d, want 1:\n%s", code, out)
	}
...
func TestTheOldestThingStillOnTheBoxIsNamed(t *testing.T) {
	...
	out, _, code := exec(t, "don", "read", "-files", path)
	if code != 0 {
		t.Fatalf("exit %d:\n%s", code, out)
	}
...
func TestTheLedgerIsAvailableAsJSON(t *testing.T) {
	...
	out, errOut, code := exec(t, "don", "read", "-json", path)
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
	var got donReadReport
	if err := json.Unmarshal([]byte(out), &got); err != nil {
		t.Fatal(err)
	}
...
func TestALogThatIsNotThereIsAFailure(t *testing.T) {
	_, errOut, code := exec(t, "don", "read", filepath.Join(t.TempDir(), "nothing.jsonl"))
	if code != 1 {
...
func TestDonWithoutASubcommandPrintsUsage(t *testing.T) {
	_, errOut, code := exec(t, "don")
	if code != 2 {
		t.Fatalf("exit %d, want 2: %s", code, errOut)
	}
...
func TestAnUnknownDonSubcommandSaysSo(t *testing.T) {
	_, errOut, code := exec(t, "don", "clean")
	if code != 2 {
		t.Fatalf("exit %d, want 2: %s", code, errOut)
	}
...
func TestDonHelpGoesToStdout(t *testing.T) {
	out, _, code := exec(t, "don", "help")
	if code != 0 {
...
func TestDonReadWantsExactlyOneLog(t *testing.T) {
	_, errOut, code := exec(t, "don", "read")
	if code != 2 {
		t.Fatalf("exit %d, want 2: %s", code, errOut)
	}
}
