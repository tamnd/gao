...
import (
	...
	"github.com/tamnd/gao/wait"
)

func runWait(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	r, err := wait.ReadRun(*crawl, fs.Arg(0))
	if err != nil {
	...
	for _, h := range r.Hosts {
		if _, ok := fleet.Lookup(h.Box); !ok && h.Box != "" {
	...
	report := waitReport{
		...
		Asked:   len(r.Asked()),
		MinLoad: wait.MinLoad, MaxErrors: wait.MaxErrors,
	...
	for _, h := range r.Ranked() {
		report.Readings = append(report.Readings, waitReading{
...
// waitReading is one host as the table carries it.
...
type waitReport struct {
	...
	Readings []waitReading `json:"readings"`
...
func printWait(w io.Writer, r wait.Run, claims []string) {
