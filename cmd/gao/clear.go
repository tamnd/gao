...
import (
	...
	"github.com/tamnd/gao/clear"
	"github.com/tamnd/gao/fleet"
)

func runClear(stdout, stderr io.Writer, args []string) int {
	if len(args) == 0 {
		clearUsage(stderr)
		return 2
	}
	switch args[0] {
	case "fit":
		return runClearFit(stdout, stderr, args[1:])
	case "read":
		return runClearRead(stdout, stderr, args[1:])
	case "help":
		clearUsage(stdout)
		return 0
	default:
...
func runClearFit(stdout, stderr io.Writer, args []string) int {
	r := clear.Target()
	...
	fs.Usage = func() { clearUsage(stderr); fs.PrintDefaults() }
...
// clearFitReport is the arithmetic in a form something else can read. Durations
...
func runClearRead(stdout, stderr io.Writer, args []string) int {
	...
	events, err := clear.ReadLog(fs.Arg(0))
	if err != nil {
...
func printClearRead(w io.Writer, l clear.Ledger, files bool) {
	tw := tabwriter.NewWriter(w, 0, 0, 2, ' ', 0)
	fmt.Fprint(tw, "state\tfiles\tbytes\n")
	for _, s := range []clear.State{clear.Resident, clear.Pushed, clear.Verified, clear.Reclaimed} {
		fmt.Fprintf(tw, "%s\t%d\t%s\n", s, l.Count(s), fleet.Size(bytesAt(l, s)))
	}
	fmt.Fprintf(tw, "\nall\t%d\t%s\n", len(l.Files), fleet.Size(l.Bytes()))
	_ = tw.Flush()
	...
	if files {
		...
		for _, f := range l.Files {
			fmt.Fprintf(tw, "%s\t%s\t%s\t%s\t%s\n", f.Name, f.Reached, fleet.Size(f.Bytes), span(f.Held()), f.Path)
		}
		_ = tw.Flush()
	}
...
func bytesAt(l clear.Ledger, s clear.State) int64 {
	var n int64
	for _, f := range l.Files {
		if f.Reached == s {
			n += f.Bytes
