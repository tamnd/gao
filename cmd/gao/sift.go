...
func runSift(stdout, stderr io.Writer, args []string) int {
	fs := flag.NewFlagSet("sift", flag.ContinueOnError)
	fs.SetOutput(stderr)
	minSyllables := fs.Int("min-syllables", sift.Default().MinSyllables, "the shortest thing that counts as a document")
...
`)
	...
	for _, name := range files {
		...
		for i, text := range texts {
			r := sift.Measure(text)
			tally.Add(limits, r)
			rows = append(rows, siftRowOf(limits, label(name, i, len(texts)), r))
		}
	}

	if *asJSON {
		return printJSON(stdout, stderr, siftReport{Documents: rows, Total: siftTotalOf(tally)})
	}
	printSift(stdout, rows, tally)
	return 0
}
...
func eachDocument(name string, fn func(text string) error) error {
	...
	return store.ScanPart(name, func(r store.Row) error { return fn(r.Text) })
}
...
func siftRowOf(l sift.Limits, name string, r sift.Result) siftRow {
	row := siftRow{
		Document:   name,
...
func vietnameseRate(r siftRow) float64 {
	if r.MarkRate < sift.MinMarkRate {
		return r.BareRate
	}
	return r.Vietnamese
}
...
func siftTotalOf(t sift.Tally) siftTotal {
	total := siftTotal{
...
type siftReport struct {
	Documents []siftRow `json:"documents"`
	Total     siftTotal `json:"total"`
}

func printSift(w io.Writer, rows []siftRow, t sift.Tally) {
...
var checkOrder = []reject.Reason{
	reject.ReasonShort, reject.ReasonBoilerplate, reject.ReasonLanguage, reject.ReasonRepetition,
}

// sortedReasons puts the counts in a fixed order, so that two runs of the same
// report list them the same way round.
func sortedReasons(counted map[reject.Reason]int64) []reject.Reason {
	order := map[reject.Reason]int{}
	for i, r := range checkOrder {
		order[r] = i
	}
	out := make([]reject.Reason, 0, len(counted))
	for r := range counted {
		out = append(out, r)
	}
