...
import (
	...
	"github.com/tamnd/gao/gian"
	"github.com/tamnd/gao/may"
)

func runGian(stdout, stderr io.Writer, args []string) int {
	if len(args) == 0 {
		gianUsage(stderr)
		return 2
	}
	switch args[0] {
	case "ladder":
		return runGianLadder(stdout, stderr, args[1:])
	case "pool":
		return runGianPool(stdout, stderr, args[1:])
	case "help", "-h", "--help":
		gianUsage(stdout)
		return 0
	default:
...
type gianRungReport struct {
	gian.Rung
	Demanded string `json:"demanded"`
}

type gianLadderReport struct {
	Rungs    []gianRungReport `json:"rungs"`
...
func runGianLadder(stdout, stderr io.Writer, args []string) int {
	...
	report := gianLadderReport{Blocking: gian.CheckLadder()}
	for _, r := range gian.Ladder() {
		report.Rungs = append(report.Rungs, gianRungReport{Rung: r, Demanded: share(r.Demand, r.Tokens)})
	}
	...
	if *asJSON {
		...
		for _, r := range report.Rungs {
			fmt.Fprintf(tw, "%d %s\t%d\t%s\t%s\t%s (%s)\t%s\n",
				r.Stage, r.Name, r.Window, gianFloor(r.Floor), gianTokens(r.Tokens), gianTokens(r.Demand), r.Demanded, r.Method)
		}
		_ = tw.Flush()
...
type gianPoolReport struct {
	gian.Pool
	Box      string           `json:"box"`
	Bands    []gianBandReport `json:"bands"`
...
func runGianPool(stdout, stderr io.Writer, args []string) int {
	...
	for _, b := range pool.Extending() {
		largest := b.Largest()
		report.Bands = append(report.Bands, gianBandReport{
...
func printGian(w io.Writer, r gianPoolReport) {
	if len(r.Blocking) > 0 {
	...
	for _, b := range r.Bands {
		fmt.Fprintf(tw, "%d\t%s\t%s\t%s\t%s\t%.1f\t%s %s\n",
			b.Window, thousands(b.Documents), gianTokens(b.Tokens), thousands(b.Mean), percent(b.Reach), b.Passes, b.Source, percent(b.Share))
	}
	_ = tw.Flush()

	fmt.Fprintf(w, "\n%s over %s of Parquet, read on %s.\n", count(r.Parts, "part"), disk(r.Bytes), r.Box)
	if r.Box == "unmeasured" {
	...
	fmt.Fprintf(w, "The longest document is %s tokens and %s of them are longer than the %d window, which the last rung reads in pieces.\n",
		thousands(int64(r.Longest)), thousands(r.Over), gian.Top())

	if len(r.Faults) > 0 {
		fmt.Fprintf(w, "\n%s the ladder cannot be climbed with:\n", count(len(r.Faults), "reading"))
...
// gianFloor writes the length a document has to reach to be in a band, and says
// so in words for the rung that has no floor under it.
func gianFloor(floor int) string {
	if floor == 0 {
		return "any length"
	}
...
// gianTokens writes a token count the way the training plan quotes one.
func gianTokens(n int64) string {
	switch {
	case n >= 1e9:
