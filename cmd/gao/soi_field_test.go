...
func TestTheFieldIsPublishedWithTheEnginesThatLost(t *testing.T) {
	out, errOut, code := exec(t, "soi", "field", engineField(t))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestABatchSizedToFillTheCardIsRefused(t *testing.T) {
	...
	out, errOut, code := exec(t, "soi", "field", path)
	if code != 1 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestAnEngineThatCannotReadTheSliceInTimeExitsNonZero(t *testing.T) {
	...
	both, _, code := exec(t, "soi", "field", split)
	if code != 0 {
	...
	cheap, _, code := exec(t, "soi", "field", "-pages", "1000000", path)
	if code != 0 {
...
func TestTheFieldIsAlsoMachineReadable(t *testing.T) {
	out, errOut, code := exec(t, "soi", "field", "-json", engineField(t))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
	var got struct {
...
func TestSoiFieldRefusesWhatItCannotRead(t *testing.T) {
	if _, _, code := exec(t, "soi", "field"); code != 2 {
		t.Errorf("no argument exited %d", code)
	}
	if _, _, code := exec(t, "soi", "field", "a.jsonl", "b.jsonl"); code != 2 {
		t.Errorf("two arguments exited %d", code)
	}
	if _, _, code := exec(t, "soi", "field", filepath.Join(t.TempDir(), "missing.jsonl")); code != 1 {
		t.Errorf("a file that is not there exited %d", code)
	}
	// server1 has no accelerator, and a field is a fact about a card.
	if _, _, code := exec(t, "soi", "field", "-box", "server1", engineField(t)); code != 2 {
