...
func claim(t *testing.T, id string) string {
	t.Helper()
	for _, p := range predict.Published().Predictions {
		if p.ID == id {
			return p.Claim
		}
	}
...
func predictResults(t *testing.T, lines ...string) string {
	t.Helper()
...
func predictResult(t *testing.T, id, state, reading, box string) string {
	t.Helper()
	return fmt.Sprintf(`{"id":%q,"claim":%q,"state":%q,"reading":%q,"by":"gao count counts","box":%q}`,
		id, claim(t, id), state, reading, box)
}

func TestPredictPrintsTheRegisterBeforeAnythingHasAResult(t *testing.T) {
	out, errOut, code := exec(t, "predict")
	if code != 0 {
...
func TestPredictPrintsOneSliceWhenAskedForOne(t *testing.T) {
	out, _, code := exec(t, "predict", "-slice", "S7")
	if code != 0 {
		t.Fatalf("exit %d\n%s", code, out)
	}
...
func TestPredictPrintsTheMissesWhateverElseWasAskedFor(t *testing.T) {
	path := predictResults(t,
		predictResult(t, "P07-5", "truot", "3.28 characters per token", "server3"),
		predictResult(t, "P05-5", "dung", "6.2% of documents changed", "server2"),
	)
	out, _, code := exec(t, "predict", "-results", path)
	if code != 0 {
...
func TestPredictRefusesAResultMeasuredAgainstAnEditedClaim(t *testing.T) {
	path := predictResults(t,
...
func TestPredictRefusesAMeasurementFromABoxThatIsNotInTheFleet(t *testing.T) {
	path := predictResults(t, predictResult(t, "P03-1", "dung", "181.4B tokens", "laptop"))
	out, _, code := exec(t, "predict", "-results", path)
	if code != 1 {
	...
	ok := predictResults(t, predictResult(t, "P03-1", "dung", "181.4B tokens", "gamingpc"))
	if _, _, code := exec(t, "predict", "-results", ok); code != 0 {
		t.Errorf("a result off gamingpc exited %d", code)
	}
}

func TestPredictExitsTwoWhenTheRegisterComesBackTooRightToBeWorthMaking(t *testing.T) {
	lines := make([]string, 0, 40)
	for _, p := range predict.Published().Predictions[:40] {
		lines = append(lines, predictResult(t, p.ID, "dung", "inside the claim", "server1"))
	}
	out, _, code := exec(t, "predict", "-results", predictResults(t, lines...))
	if code != 2 {
...
func TestPredictJSONCarriesTheDigestAndTheBand(t *testing.T) {
	out, _, code := exec(t, "predict", "-json")
	if code != 0 {
		t.Fatalf("exit %d\n%s", code, out)
	}
...
func TestPredictWithAnArgumentIsAUsageError(t *testing.T) {
	if _, _, code := exec(t, "predict", "S1"); code != 2 {
		t.Error("gao predict with a positional argument did not exit 2")
	}
	if _, _, code := exec(t, "predict", "-results", filepath.Join(t.TempDir(), "nothing.jsonl")); code != 1 {
