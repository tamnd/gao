...
// graftTrial writes a trial that paid for itself unless a test says otherwise.
func graftTrial(t *testing.T, lines ...string) string {
	t.Helper()
	if len(lines) == 0 {
		lines = []string{
...
func TestTheGraftIsCostedAgainstTheRunRatherThanTheTokenizer(t *testing.T) {
	out, errOut, code := exec(t, "graft", graftTrial(t))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestAGraftThatSpendsTheRunRecoveringExitsTwo(t *testing.T) {
	out, _, code := exec(t, "graft", graftTrial(t,
...
func TestTheTrialIsAlsoMachineReadable(t *testing.T) {
	out, errOut, code := exec(t, "graft", "-json", graftTrial(t))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
	var got struct {
...
func TestGraftRefusesWhatItCannotRead(t *testing.T) {
	if _, _, code := exec(t, "graft"); code != 2 {
		t.Errorf("no argument exited %d", code)
	}
	if _, _, code := exec(t, "graft", "a.jsonl", "b.jsonl"); code != 2 {
		t.Errorf("two arguments exited %d", code)
	}
	if _, _, code := exec(t, "graft", filepath.Join(t.TempDir(), "missing.jsonl")); code != 1 {
