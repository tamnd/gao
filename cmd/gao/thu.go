...
func runThu(stdout, stderr io.Writer, args []string) int {
	if len(args) == 0 {
		thuUsage(stderr)
		return 2
	}
	switch args[0] {
	case "slate":
		return runThuSlate(stdout, stderr, args[1:])
	case "read":
		return runThuRead(stdout, stderr, args[1:])
	case "help", "-h", "--help":
		thuUsage(stdout)
		return 0
	default:
...
func runThuSlate(stdout, stderr io.Writer, args []string) int {
	...
	s, code := readThuSlate(stderr, *path)
	if code != 0 {
		return code
	}
	faults := s.Faults()
	...
	switch {
	case *asJSON:
		if code := printJSON(stdout, stderr, thuSlateReport{Slate: s, Digest: s.Digest().String(), Faults: faults}); code != 0 {
			return code
		}
	case *knobs:
		printThuKnobs(stdout, s)
	default:
		printThuSlate(stdout, s)
	}
...
`)
	...
	out := thuReadReport{Report: report, Faults: append(s.Faults(), report.Publishable()...)}
	if *asJSON {
		if code := printJSON(stdout, stderr, out); code != 0 {
			return code
		}
	} else {
		printThuReport(stdout, s, out)
	}
	if len(out.Faults) > 0 {
		return 1
	}
...
type thuSlateReport struct {
	Slate  thu.Slate `json:"slate"`
	Digest string    `json:"digest"`
	Faults []string  `json:"faults,omitempty"`
}

type thuReadReport struct {
	thu.Report
	Faults []string `json:"faults,omitempty"`
}

func readThuSlate(stderr io.Writer, path string) (thu.Slate, int) {
	if path == "" {
		return thu.Fixed(), 0
	}
...
func printThuSlate(w io.Writer, s thu.Slate) {
	fmt.Fprintf(w, "%s\n\n", s.Describe())
...
func printThuKnobs(w io.Writer, s thu.Slate) {
	fmt.Fprintf(w, "%s\n\n", s.Describe())
	tw := tabwriter.NewWriter(w, 0, 0, 2, ' ', 0)
...
func printThuReport(w io.Writer, s thu.Slate, out thuReadReport) {
	r := out.Report
	fmt.Fprintf(w, "%s\n\n", s.Describe())
