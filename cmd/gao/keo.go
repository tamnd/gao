...
`)
	...
	for _, r := range d.Fleet() {
		if _, ok := may.Lookup(r.Source); !ok && r.Source != "" {
	...
	report := keoReport{
		...
		Unaffordable: len(d.Unaffordable()), Budget: keo.Budget,
	...
	for _, r := range d.Ranked() {
		report.Readings = append(report.Readings, keoReading{
...
// keoReading is one resume as the table carries it.
type keoReading struct {
	Step int `json:"step"`
...
type keoReport struct {
	...
	Readings []keoReading `json:"readings"`
...
func printKeo(w io.Writer, d keo.Drill, claims []string) {
	...
	for _, r := range d.Ranked() {
		fmt.Fprintf(tw, "%d\t%s\t%s\t%s\t%s/s\t%d of %d\t%s\t%s\t%s\t%.0f%%\t%+.4f\t%s\n",
			...
			r.ReadRanks, r.WroteRanks, keo.Duration(r.Provision), keo.Duration(r.Load),
			keo.Duration(r.Restart()), 100*r.Overhead(), r.Drift(), matched(r))
	}
	_ = tw.Flush()
	...
	if bad := d.Unaffordable(); len(bad) > 0 {
		...
		fmt.Fprintf(w, "The %s copy came back intact and costs %s to get back into, which is %.0f%% of a %s interval, so it is the copy that survives rather than the copy a live restart reads.\n",
			r.From, keo.Duration(r.Restart()), 100*r.Overhead(), keo.Duration(r.Interval))
	}
	fmt.Fprintf(w, "\n%s.\n", d.Verdict())
}
...
func matched(r keo.Resume) string {
	switch {
