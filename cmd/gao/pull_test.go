...
// pullDrill writes a drill that came back intact unless a test says otherwise.
func pullDrill(t *testing.T, lines ...string) string {
	t.Helper()
	if len(lines) == 0 {
		lines = []string{
...
func TestARestartDrillNamesTheCopyItCameBackFrom(t *testing.T) {
	out, errOut, code := exec(t, "pull", pullDrill(t))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestAResumeOffTheTrainingHostExitsOne(t *testing.T) {
	out, _, code := exec(t, "pull", pullDrill(t,
...
func TestTheDrillIsAlsoMachineReadable(t *testing.T) {
	out, errOut, code := exec(t, "pull", "-json", pullDrill(t))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
	var got struct {
...
func TestPullRefusesWhatItCannotRead(t *testing.T) {
	if _, _, code := exec(t, "pull"); code != 2 {
		t.Errorf("no argument exited %d", code)
	}
	if _, _, code := exec(t, "pull", "a.jsonl", "b.jsonl"); code != 2 {
		t.Errorf("two arguments exited %d", code)
	}
	if _, _, code := exec(t, "pull", filepath.Join(t.TempDir(), "missing.jsonl")); code != 1 {
