...
func TestMillBoilerCountsWhatEveryPageOfAHostCarries(t *testing.T) {
	var stdout, stderr bytes.Buffer
	if code := runMill(&stdout, &stderr, []string{"-boiler", "-json", boilerPart(t)}); code != 0 {
...
func TestMillBoilerSaysWhatItCountedAsFurniture(t *testing.T) {
	var stdout, stderr bytes.Buffer
	if code := runMill(&stdout, &stderr, []string{"-boiler", boilerPart(t)}); code != 0 {
		t.Fatalf("gao mill -boiler = %d, want 0\n%s", code, stderr.String())
	}
	out := stdout.String()
...
func TestMillBoilerReportsThePagesItEmptied(t *testing.T) {
	rows := make([]hostedRow, 0, len(bodyLines)+1)
	for _, body := range bodyLines {
		rows = append(rows, hostedRow{host: "vnbao.vn", text: hostedPage(body)})
	}
	...
	var stdout, stderr bytes.Buffer
	if code := runMill(&stdout, &stderr, []string{"-boiler", path}); code != 0 {
		t.Fatalf("gao mill -boiler = %d, want 0\n%s", code, stderr.String())
	}
...
// repeats.
func TestMillBoilerRefusesATextFile(t *testing.T) {
	dir := t.TempDir()
	path := writeText(t, dir, "a.txt", millArticle)

	var stdout, stderr bytes.Buffer
	if code := runMill(&stdout, &stderr, []string{"-boiler", path}); code != 2 {
...
func TestMillBoilerAndCurveAreNotRunTogether(t *testing.T) {
	var stdout, stderr bytes.Buffer
	if code := runMill(&stdout, &stderr, []string{"-boiler", "-curve", boilerPart(t)}); code != 2 {
...
// cut and says so.
func TestMillBoilerCutsTheTableAndSaysSo(t *testing.T) {
	var stdout, stderr bytes.Buffer
	if code := runMill(&stdout, &stderr, []string{"-boiler", "-hosts", "1", boilerPart(t)}); code != 0 {
