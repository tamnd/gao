...
// bocPage writes a XenForo shaped thread, which is what the large Vietnamese
...
func bocPage(t *testing.T, posts ...string) string {
	t.Helper()
...
func TestTheThreadComesOutAndThePageStaysBehind(t *testing.T) {
	out, errOut, code := exec(t, "boc", bocPage(t, bocPosts...))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestThePostsCanBePrinted(t *testing.T) {
	out, _, code := exec(t, "boc", "-text", bocPage(t, bocPosts...))
	if code != 0 {
...
func TestWhatWasDroppedCanBeRead(t *testing.T) {
	out, _, code := exec(t, "boc", "-furniture", bocPage(t, bocPosts...))
	if code != 0 {
		t.Fatalf("exit %d: %s", code, out)
	}
...
<div class="article-body"><p>Giá gạo xuất khẩu của Việt Nam tháng qua đã tăng lên mức cao nhất kể từ đầu năm nay.</p></div></body></html>`
	...
	out, errOut, code := exec(t, "boc", path)
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestTheWholeThreadIsAvailableAsJSON(t *testing.T) {
	out, errOut, code := exec(t, "boc", "-json", bocPage(t, bocPosts...))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
	var got bocReport
	...
	page := got.Pages[0]
	if len(page.Posts) != len(bocPosts) {
		t.Errorf("%d posts, want %d", len(page.Posts), len(bocPosts))
	}
...
func TestSeveralPagesAreOneTable(t *testing.T) {
	out, _, code := exec(t, "boc", bocPage(t, bocPosts...), bocPage(t, bocPosts...))
	if code != 0 {
		t.Fatalf("exit %d: %s", code, out)
	}
...
func TestBocWithoutAPageIsAUsageError(t *testing.T) {
	out, errOut, code := exec(t, "boc")
	if code != 2 {
...
func TestAMissingPageIsAFailure(t *testing.T) {
	_, errOut, code := exec(t, "boc", filepath.Join(t.TempDir(), "nothing.html"))
	if code != 1 {
