...
// stretchPart writes one part holding count documents of the given source, each
...
func stretchPart(t *testing.T, source doc.Source, ntokens uint32, count int) string {
	t.Helper()
...
func TestStretchLadderPrintsTheRungsWithWhatEachOneIsFor(t *testing.T) {
	out, errOut, code := exec(t, "stretch", "ladder")
	if code != 0 {
...
func TestStretchLadderQuotesTheLongDemandAsAShareOfTheStage(t *testing.T) {
	out, _, code := exec(t, "stretch", "ladder")
	if code != 0 {
...
func TestStretchPoolReadsAPartAndSaysWhatTheReadCost(t *testing.T) {
	path := stretchPart(t, doc.SourceFinePDFs, 70000, 300)
	out, errOut, code := exec(t, "stretch", "pool", "-name", "a slice", path)
	if code != 2 {
...
func TestStretchPoolRefusesAPartWithNoTokenCountsOnIt(t *testing.T) {
	path := stretchPart(t, doc.SourceCrawl, 0, 20)
	out, _, code := exec(t, "stretch", "pool", path)
	if code != 1 {
...
func TestStretchPoolRefusesAPartThatIsNotThere(t *testing.T) {
	missing := filepath.Join(t.TempDir(), "part-00000.parquet")
	_, errOut, code := exec(t, "stretch", "pool", missing)
	if code != 1 {
...
func TestStretchPrintsTheSameReadingAsJSON(t *testing.T) {
	path := stretchPart(t, doc.SourceMedia, 40000, 50)
	out, _, code := exec(t, "stretch", "pool", "-name", "a slice", "-json", path)
	if code != 2 {
		t.Fatalf("exit %d: %s", code, out)
	}
	var got stretchPoolReport
	...
	ladder, _, code := exec(t, "stretch", "ladder", "-json")
	if code != 0 {
		t.Fatalf("exit %d", code)
	}
	var rungs stretchLadderReport
...
func TestStretchSaysHowToBeAskedProperly(t *testing.T) {
	for _, args := range [][]string{
		{"stretch"},
		{"stretch", "pool"},
		{"stretch", "ladder", "extra"},
		{"stretch", "nothing"},
	} {
	...
	out, _, code := exec(t, "stretch", "help")
