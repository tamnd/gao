...
// sinkStep writes a step that lost nothing unless a test says otherwise.
func sinkStep(t *testing.T, lines ...string) string {
	t.Helper()
	if len(lines) == 0 {
		lines = []string{
...
func TestAStepThatKeptItsValuesPrintsWhatItChecked(t *testing.T) {
	out, errOut, code := exec(t, "sink", "-loss", "2.3141", "-bf16", "2.3139", sinkStep(t))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestTheCurveAgreeingAndTheGradientEmptyingExitsNonZero(t *testing.T) {
	...
	out, errOut, code := exec(t, "sink", "-loss", "2.3141", "-bf16", "2.3139", path)
	if code != 2 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestATensorWiderThanTheFormatIsMarkedInTheTable(t *testing.T) {
	...
	out, _, code := exec(t, "sink", "-loss", "2.3141", "-bf16", "2.3139", path)
	if code != 2 {
...
func TestAStepThatIsNotACheckExitsOne(t *testing.T) {
	...
	alone, _, code := exec(t, "sink", "-loss", "2.3141", sinkStep(t))
	if code != 1 {
...
func TestTheStepIsAlsoMachineReadable(t *testing.T) {
	...
	out, errOut, code := exec(t, "sink", "-json", "-loss", "2.3141", "-bf16", "2.3139", path)
	if code != 2 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
	var got struct {
...
func TestSinkRefusesWhatItCannotRead(t *testing.T) {
	if _, _, code := exec(t, "sink"); code != 2 {
		t.Errorf("no argument exited %d", code)
	}
	if _, _, code := exec(t, "sink", "a.jsonl", "b.jsonl"); code != 2 {
		t.Errorf("two arguments exited %d", code)
	}
	if _, _, code := exec(t, "sink", filepath.Join(t.TempDir(), "missing.jsonl")); code != 1 {
