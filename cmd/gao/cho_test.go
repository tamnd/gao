...
// choRun writes a run that was polite unless a test says otherwise.
func choRun(t *testing.T, lines ...string) string {
	t.Helper()
	if len(lines) == 0 {
		lines = []string{
...
func TestThePolitenessReportLeadsWithTheHostThatCameNearest(t *testing.T) {
	out, errOut, code := exec(t, "cho", choRun(t))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestADelayThatWasNotHeldExitsTwo(t *testing.T) {
	out, _, code := exec(t, "cho", choRun(t,
...
func TestAReadingOffAnIdleBoxExitsOne(t *testing.T) {
	...
	out, _, code := exec(t, "cho", choRun(t, idle, hostLine("tuoitre.vn", 4, 0, 4.4, 5.0, 2, 3)))
	if code != 1 {
		t.Fatalf("exit %d: %s", code, out)
	}
	...
	out, _, code = exec(t, "cho", choRun(t, elsewhere, hostLine("tuoitre.vn", 4, 0, 4.4, 5.0, 2, 3)))
	if code != 1 {
		t.Fatalf("exit %d: %s", code, out)
	}
...
func TestThePolitenessRunIsAlsoMachineReadable(t *testing.T) {
	out, errOut, code := exec(t, "cho", "-json", choRun(t))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
	var got struct {
...
func TestChoRefusesWhatItCannotRead(t *testing.T) {
	if _, _, code := exec(t, "cho"); code != 2 {
		t.Errorf("no argument exited %d", code)
	}
	if _, _, code := exec(t, "cho", "a.jsonl", "b.jsonl"); code != 2 {
		t.Errorf("two arguments exited %d", code)
	}
	if _, _, code := exec(t, "cho", filepath.Join(t.TempDir(), "missing.jsonl")); code != 1 {
