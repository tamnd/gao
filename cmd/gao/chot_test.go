...
func honest(t *testing.T, arm string, score float64) string {
	...
	b, err := json.Marshal(chot.Result{Harness: h.Digest(), Arm: arm, Scores: scores})
	if err != nil {
		t.Fatal(err)
	}
	return string(b)
...
func TestTheHarnessPrintsWhatItMeasures(t *testing.T) {
	out, errOut, code := exec(t, "chot", "harness")
	if code != 0 {
...
func TestTheHarnessNamesWhatIsNotYetReproducible(t *testing.T) {
	...
	out, _, code := exec(t, "chot", "harness")
	if code != 0 {
...
func TestThePromptsComeOutVerbatimWhenAsked(t *testing.T) {
	...
	// opening the file.
	plain, _, _ := exec(t, "chot", "harness")
	full, _, code := exec(t, "chot", "harness", "-prompts")
	if code != 0 {
		t.Fatalf("exit %d", code)
	}
...
func TestAnHonestRunAudits(t *testing.T) {
	...
	out, errOut, code := exec(t, "chot", "audit", path)
	if code != 0 {
...
func TestADroppedBenchmarkFailsTheAudit(t *testing.T) {
	...
	out, _, code := exec(t, "chot", "audit", path)
	if code != 1 {
...
func TestResultsFromAnotherHarnessFailTheAudit(t *testing.T) {
	...
	r.Harness = chot.Harness{Version: "made up"}.Digest()
	b, err := json.Marshal(r)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTheAuditSpeaksJSON(t *testing.T) {
	path := writeResults(t, honest(t, "com-8B-cpt-gao", 0.6))
	out, _, code := exec(t, "chot", "audit", "-json", path)
	if code != 1 {
		t.Fatalf("exit %d", code)
	}
	var a chot.Audit
...
func TestTheHarnessSpeaksJSON(t *testing.T) {
	out, _, code := exec(t, "chot", "harness", "-json")
	if code != 0 {
		t.Fatalf("exit %d", code)
	}
	var report struct {
...
func TestATypoInAResultFileIsAnErrorRatherThanADefault(t *testing.T) {
	...
	_, errOut, code := exec(t, "chot", "audit", path)
	if code != 1 {
...
func TestAHarnessFromAFileCanBeAudited(t *testing.T) {
	...
	body, err := os.ReadFile("../../chot/harness.json")
	if err != nil {
		t.Fatal(err)
	}
	...
	out, _, code := exec(t, "chot", "digest", "-harness", path)
	if code != 0 {
		t.Fatalf("exit %d", code)
	}
...
func TestASubcommandNobodyWroteIsNamed(t *testing.T) {
	_, errOut, code := exec(t, "chot", "kiem")
	if code != 2 {
		t.Errorf("exit %d, want 2", code)
	}
...
func TestAResultFileThatIsNotThereSaysSo(t *testing.T) {
	_, errOut, code := exec(t, "chot", "audit", filepath.Join(t.TempDir(), "khong-co.json"))
	if code != 1 {
