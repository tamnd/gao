...
func runSyllable(stdout, stderr io.Writer, args []string) int {
	fs := flag.NewFlagSet("syllable", flag.ContinueOnError)
	fs.SetOutput(stderr)
	asJSON := fs.Bool("json", false, "print JSON")
	source := fs.String("source", "", "the text this reading was taken over")
	slots := fs.Int("slots", syllable.Slots, "how many vocabulary slots the arm without the rule may spend on cross-syllable runs")
...
`)
	...
	docs, err := syllable.ReadDocs(fs.Args())
	if err != nil {
		fmt.Fprintf(stderr, "gao syllable: %v\n", err)
		return 1
	}

	r := syllable.Read(*source, *slots, docs)
	report := syllableReport{
		Reading:  r,
		Atomic:   syllable.Atomic,
		Faults:   r.Faults(),
		Blocking: r.Blocking(),
		Holds:    r.Holds(),
		Verdict:  r.Verdict(),
	...
	if *asJSON {
		...
		printSyllable(stdout, report, *top)
	}
...
func printSyllable(w io.Writer, r syllableReport, top int) {
	if len(r.Blocking) > 0 {
