...
func runInspect(stdout, stderr io.Writer, args []string) int {
	if len(args) > 0 && args[0] == "field" {
		return runInspectField(stdout, stderr, args[1:])
	}
...
`)
	...
	var total inspect.Score
	lines := make([]inspectLine, 0, len(files)/2)
	for i := 0; i < len(files); i += 2 {
		...
		s := inspect.Measure(string(ref), string(read))
		total.Add(s)
		lines = append(lines, inspectLine{Page: files[i], Reading: files[i+1], Score: s})
	}

	fails := inspect.S4.Check(total)
	if *asJSON {
		if code := printJSON(stdout, stderr, inspectReport{Readings: lines, Total: total, Fails: fails}); code != 0 {
			return code
		}
	} else {
		printInspect(stdout, lines, total, *matrix)
	}
...
// inspectLine is one page and the reading of it that was judged.
...
type inspectReport struct {
	Readings []inspectLine `json:"readings"`
	Total    inspect.Score `json:"total"`
...
func printInspect(w io.Writer, lines []inspectLine, total inspect.Score, matrix bool) {
	...
	for _, t := range inspect.Tones() {
		fmt.Fprintf(tw, "\t%s", t)
	}
	fmt.Fprint(tw, "\n")
	for _, from := range inspect.Tones() {
		fmt.Fprintf(tw, "%s", from)
		for _, to := range inspect.Tones() {
