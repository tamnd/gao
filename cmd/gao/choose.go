...
import (
	...
	"github.com/tamnd/gao/choose"
)

func runChoose(stdout, stderr io.Writer, args []string) int {
	if len(args) == 0 {
		chooseUsage(stderr)
		return 2
	}
	switch args[0] {
	case "criteria":
		return runChooseCriteria(stdout, stderr, args[1:])
	case "bases":
		return runChooseBases(stdout, stderr, args[1:])
	case "score":
		return runChooseScore(stdout, stderr, args[1:])
	case "help":
		chooseUsage(stdout)
		return 0
	default:
...
func runChooseCriteria(stdout, stderr io.Writer, args []string) int {
	fs := flag.NewFlagSet("choose criteria", flag.ContinueOnError)
	fs.SetOutput(stderr)
	asJSON := fs.Bool("json", false, "print JSON")
	fs.Usage = func() { chooseUsage(stderr); fs.PrintDefaults() }
...
func runChooseBases(stdout, stderr io.Writer, args []string) int {
...
func runChooseScore(stdout, stderr io.Writer, args []string) int {
	...
	readings, err := choose.ReadReadings(fs.Arg(0))
	if err != nil {
		fmt.Fprintf(stderr, "gao choose: %v\n", err)
		return 1
	}
	table := choose.Score(readings)

	report := chooseScoreReport{
		Bases: len(choose.Bases()), Band: choose.Band, Suites: table.Suites,
	...
	for _, r := range table.Ranked() {
		report.Ranked = append(report.Ranked, chooseRow{
...
type chooseScoreReport struct {
	Bases  int         `json:"bases"`
	Ranked []chooseRow `json:"ranked,omitempty"`
...
func printChooseScore(w io.Writer, table choose.Table) {
