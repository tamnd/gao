...
`)
	...
	report := hoiReport{
		...
		Heaviest:    heaviest,
		Leans:       leans,
		MaxPerDoc:   hoi.MaxPerDocument,
		Reach:       s.Reach(),
		MinReach:    hoi.MinReach,
		Ladder:      s.Ladder(),
	...
	for _, q := range s.Out() {
		report.Rejected = append(report.Rejected, reject{ID: q.ID, Why: q.Blocking()})
	}
	...
	if *asJSON {
		...
		printHoi(stdout, report, *rejects)
	}
...
type hoiReport struct {
	...
	Ladder      []hoi.Row `json:"ladder"`
	Composition []hoi.Row `json:"composition"`
...
func printHoi(w io.Writer, r hoiReport, rejects bool) {
