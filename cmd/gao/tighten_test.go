...
import (
	...
	"github.com/tamnd/gao/tighten"
)

// tightenLog writes a training log of n steps and returns the path. fix is handed
// each step before it is written, so a test can spoil one reading and leave the
// rest of the run alone.
func tightenLog(t *testing.T, n int, fix func(i int, s *tighten.Step)) string {
	t.Helper()
	...
	for i := range n {
		...
		s := tighten.Step{
			...
			Rollouts:  groups * tighten.Group,
			Truncated: groups * tighten.Group / 100,
			ClipLow:   0.031,
			ClipHigh:  0.004,
			Entropy:   0.92 - 0.22*f,
			Reward:    0.41 + 0.33*f,
...
func tightenConfigFile(t *testing.T, r tighten.Recipe) string {
	t.Helper()
...
func TestTightenRecipePrintsTheSettingsAndWhatTheyFix(t *testing.T) {
	out, errOut, code := exec(t, "tighten", "recipe")
	if code != 0 {
	...
	why, _, code := exec(t, "tighten", "recipe", "-why")
	if code != 0 {
...
func TestTightenRecipeRefusesAConfigurationThatIsNotWhatItSaysItIs(t *testing.T) {
	r := tighten.Plan()
	r.EpsHigh = r.EpsLow
	out, _, code := exec(t, "tighten", "recipe", "-config", tightenConfigFile(t, r))
	if code != 2 {
...
func TestTightenReadsACleanRunAsOne(t *testing.T) {
	out, errOut, code := exec(t, "tighten", "read", "-specialist", "dau", tightenLog(t, 400, nil))
	if code != 0 {
...
func TestTightenReadsAStalledRunAsTheThreeThingsThatStalledIt(t *testing.T) {
	path := tightenLog(t, 400, func(i int, s *tighten.Step) {
	...
	out, _, code := exec(t, "tighten", "read", "-specialist", "dau", path)
	if code != 2 {
...
func TestTightenNamesTheRunItWasNotToldTheSpecialistFor(t *testing.T) {
	out, _, code := exec(t, "tighten", "read", tightenLog(t, 40, nil))
	if code != 0 {
...
func TestTightenRefusesALogThatIsNotOneRun(t *testing.T) {
	path := tightenLog(t, 20, func(i int, s *tighten.Step) {
		if i == 7 {
			s.Box = "somewhere else"
		}
	})
	out, _, code := exec(t, "tighten", "read", path)
	if code != 1 {
...
func TestTightenReadsAMissingOrMalformedLogAsTheFileItIs(t *testing.T) {
	_, errOut, code := exec(t, "tighten", "read", filepath.Join(t.TempDir(), "nothing.jsonl"))
	if code != 1 {
	...
	_, errOut, code = exec(t, "tighten", "read", bad)
	if code != 1 {
...
func TestTightenPrintsJSONWithTheSameReadingsInIt(t *testing.T) {
	out, _, code := exec(t, "tighten", "read", "-specialist", "dau", "-json", tightenLog(t, 400, nil))
	if code != 0 {
	...
	recipe, _, code := exec(t, "tighten", "recipe", "-json")
	if code != 0 {
		t.Fatalf("recipe -json exits %d", code)
	}
	var conf struct {
		Recipe  tighten.Recipe `json:"recipe"`
	...
	if conf.Recipe != tighten.Plan() || conf.Context != tighten.Context || !conf.Holds {
		t.Errorf("the JSON recipe is not the one the plan fixed: %+v", conf)
	}
}

func TestTightenUsageIsPrintedForWhatItCannotDo(t *testing.T) {
	out, _, code := exec(t, "tighten", "help")
	if code != 0 {
	...
	for _, args := range [][]string{
		...
		{"tighten", "measure"},
		{"tighten", "read"},
		{"tighten", "read", "one.jsonl", "two.jsonl"},
		{"tighten", "recipe", "extra"},
	} {
	...
	_, errOut, code := exec(t, "tighten", "recipe", "-config", filepath.Join(t.TempDir(), "nothing.json"))
	if code != 1 {
...
func TestTightenReadsARunAgainstTheConfigurationItWasTakenUnder(t *testing.T) {
	r := tighten.Plan()
	r.Oversample = 8
	out, _, code := exec(t, "tighten", "read", "-config", tightenConfigFile(t, r), tightenLog(t, 40, func(i int, s *tighten.Step) {
		if i >= 30 {
			s.Kept = s.Groups / 5
		}
	}))
