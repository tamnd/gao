...
`)
	...
	report := latReport{Snapshot: parent.Snapshot, Documents: parent.Counts.Documents, Bytes: parent.Counts.Bytes}
...
func printLat(w io.Writer, r latReport) {
	...
	for _, line := range r.Slices {
		...
		fmt.Fprintf(tw, "%s\t%s\t%d\t%d\t%s\t%s\t%s\n",
			s.Name, s.Dataset, s.Shards(), c.Documents, share(c.Documents, r.Documents), latSize(c.Bytes), state)
	}
	_ = tw.Flush()
...
func sliceDocuments(r latReport) int64 {
	var n int64
	for _, line := range r.Slices {
...
// latSize prints bytes of text the way the rest of the project quotes a corpus
// size, which is never a file size.
func latSize(n int64) string {
	const unit = 1000
	if n < unit {
