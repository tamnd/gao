...
func runLayers(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	read, err := layers.ReadLayers(fs.Arg(0))
	if err != nil {
		fmt.Fprintf(stderr, "gao layers: %v\n", err)
		return 1
	}

	s := layers.Source{Source: *source, Quoted: *quoted, Layers: read}
	lo, hi := s.Packing()
	report := layersReport{
		Source:    s.Source,
		Quoted:    s.Quoted,
...
func printLayers(w io.Writer, r layersReport) {
	if len(r.Blocking) > 0 {
