...
func runWeigh(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	c, err := weigh.ReadComparison(*name, fs.Arg(0))
	if err != nil {
	...
	for _, r := range c.Runs {
		...
		if _, ok := fleet.Lookup(r.EvalBox); !ok {
	...
	for _, r := range c.Runs {
		...
		row.Score = r.Scores[weigh.Metric]
		row.Base = r.BaseScores[weigh.Metric]
...
func printWeigh(w io.Writer, r weighReport) {
	...
	if adherence := adherenceOf(r); adherence > 0 {
		fmt.Fprintf(w, "\nP10-2, vi-adherence on the gao arm before anything is done about it, reads %s against the %s it was predicted under.\n",
			percent(adherence), percent(weigh.Adherence))
	}
...
func adherenceOf(r weighReport) float64 {
	for _, a := range r.Arms {
