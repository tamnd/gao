...
func runStore(stdout, stderr io.Writer, args []string) int {
	if len(args) == 0 {
		storeUsage(stderr)
		return 2
	}
	switch args[0] {
	case "verify":
		return runStoreVerify(stdout, stderr, args[1:])
	case "remove":
		return runStoreRemove(stdout, stderr, args[1:])
	case "reproduce":
		return runStoreReproduce(stdout, stderr, args[1:])
	case "keygen":
		return runStoreKeygen(stdout, stderr, args[1:])
	case "datasets":
		return runStoreDatasets(stdout, stderr, args[1:])
	case "columns":
		return runStoreColumns(stdout, stderr, args[1:])
	case "schema":
		return runStoreSchema(stdout, stderr, args[1:])
	case "push":
		return runStorePush(stdout, stderr, args[1:])
	case "order":
		return runStoreOrder(stdout, stderr, args[1:])
	case "card":
		return runStoreCard(stdout, stderr, args[1:])
	case "move":
...
func runStoreVerify(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	if *key != "" {
		...
		opts = append(opts, store.TrustKey(pub))
	}
	if *quick {
		opts = append(opts, store.Quick())
	}
	if *verbose {
		opts = append(opts, store.Progress(func(s store.Shard, err error) {
			if err != nil {
				fmt.Fprintf(stdout, "  %s  FAILED\n", s.Name)
				return
			}
			fmt.Fprintf(stdout, "  %s  ok  %d documents, %s\n", s.Name, s.Documents, fleet.Size(s.Bytes))
		}))
	}

	report, err := store.Verify(dir, opts...)
	if err != nil {
		fmt.Fprintf(stderr, "gao store verify: %v\n", err)
		return 1
	}
	...
	fmt.Fprintf(stdout, "  shards    %d checked, %s\n", report.Checked, fleet.Size(report.Bytes))
	fmt.Fprintln(stdout, "\nok")
	return 0
}
...
func loadVerifyKey(s string) (ed25519.PublicKey, error) {
	if pub, err := store.ParsePublicKey(s); err == nil {
		return pub, nil
	}
	return store.LoadPublicKey(s)
}

func runStoreRemove(stdout, stderr io.Writer, args []string) int {
	...
	reason := fs.String("reason", "", "why, one of "+strings.Join(store.Reasons(), ", "))
...
`)
	...
	if !slices.Contains(store.Reasons(), *reason) {
		fmt.Fprintf(stderr, "gao store remove: -reason must be one of %s\n", strings.Join(store.Reasons(), ", "))
		return 2
	}
	...
	key, err := store.LoadPrivateKey(*keyPath)
	if err != nil {
		fmt.Fprintf(stderr, "gao store remove: %v\n", err)
		return 1
	}

	rs := make([]store.Removal, len(ids))
	for i, id := range ids {
		rs[i] = store.Removal{DocID: id, Reason: *reason, Note: *note}
	}

	opts := []store.RemoveOption{}
	if *verbose {
		opts = append(opts, store.RemoveProgress(func(shard string, rewritten bool) {
			what := "copied"
			if rewritten {
				what = "rewritten"
			}
	...
	report, err := store.Remove(*from, *to, *name, key, rs, opts...)
	if err != nil {
	...
	if _, err := store.Verify(*to, store.TrustKey(key.Public().(ed25519.PublicKey))); err != nil {
...
func runStoreKeygen(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	pub, priv, err := store.GenerateKey()
	if err != nil {
		fmt.Fprintf(stderr, "gao store keygen: %v\n", err)
		return 1
	}
	if err := store.WritePrivateKey(privPath, priv); err != nil {
		fmt.Fprintf(stderr, "gao store keygen: %v\n", err)
		return 1
	}
	if err := store.WritePublicKey(pubPath, pub); err != nil {
...
func runStoreDatasets(stdout, stderr io.Writer, args []string) int {
	...
	if where, ok := fleet.Store(); ok && where != store.HubStore {
		fmt.Fprintf(stdout, "%s is set to %s, so this run writes there instead\n", fleet.StoreEnv, where)
	}

	fmt.Fprint(stdout, "\npublished\n")
	printDatasets(stdout, store.Published, *snapshot)
	fmt.Fprint(stdout, "\nworking, public like the rest, rewritten when a source is pinned again\n")
	printDatasets(stdout, store.Working, *snapshot)

	fmt.Fprintf(stdout, "\none parquet file per shard, at %s\n", store.DataPath(*snapshot, 1, 774))
...
func runStoreColumns(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	columns := store.Columns(store.SchemaFor(d))
...
func runStoreSchema(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	case *def:
		...
		fmt.Fprintln(stdout, store.Definition(d))
	default:
		cols := append(store.Schema(), store.Nested()...)
		fmt.Fprintf(stdout, "%d columns, schema version %d\n\n", len(store.Schema()), doc.SchemaVersion)
...
func runStorePush(stdout, stderr io.Writer, args []string) int {
	fs := flag.NewFlagSet("store push", flag.ContinueOnError)
	fs.SetOutput(stderr)
	name := fs.String("dataset", store.StageRepo, "the dataset repo to push to")
...
Writing needs a token in `+fleet.TokenEnv+` with write access to the `+store.Org+`
organization.
...
func runStoreCard(stdout, stderr io.Writer, args []string) int {
...
With -push the card is committed to the repo as `+store.CardName+`, which is what
a release does after it seals a snapshot. A card that already says the same thing
is left alone. Writing needs a token in `+fleet.TokenEnv+` with write access to the
...
`)
	...
	if *from != "" {
		loaded, err := store.ReadManifest(*from)
		if err != nil {
	...
	if !*push {
		fmt.Fprint(stdout, store.Card(d, m, x))
		return 0
	}
...
func printFileColumns(stdout, stderr io.Writer, path string) int {
	columns, err := store.PartColumns(path)
	if err != nil {
	...
	if slices.Contains(columns, store.TokensColumn) && meta["gao.tokenizer"] == "" {
		fmt.Fprintf(stdout, "\nno tokenizer ran, so %s is zero for every document here rather than counted\n", store.TokensColumn)
	}
	return 0
}

func printDatasets(w io.Writer, tier store.Tier, snapshot string) {
...
func runStoreOrder(stdout, stderr io.Writer, args []string) int {
	fs := flag.NewFlagSet("store order", flag.ContinueOnError)
	fs.SetOutput(stderr)
	asJSON := fs.Bool("json", false, "print JSON")
	target := fs.Int64("target", fleet.ShardBytes, "the compressed size each shard is aimed at, in bytes")
...
`)
	...
	readings, err := store.ReadReadings(fs.Arg(0))
	if err != nil {
		fmt.Fprintf(stderr, "gao store: %v\n", err)
		return 1
	}

	c := store.Compare(*target, readings)
	out := storeOrderReport{Comparison: c, Verdict: c.Verdict(), Settled: c.Settled()}
	if *text > 0 {
	...
	if *asJSON {
		...
		printStoreOrder(stdout, out, *text)
	}
	if !c.Settled() {
		return 1
	}
	...
	if len(c.Gains) > 0 {
		...
		for _, g := range c.Gains {
			fmt.Fprintf(gw, "  %s\t%s\t%s\t%.1f%%\t%d\t%.0f%%\n",
				g.Shard, fleet.Size(g.Arrival.Compressed), fleet.Size(g.Sorted.Compressed), 100*g.Fraction, g.Sorted.Hosts, 100*g.Sorted.Biggest)
		}
		_ = gw.Flush()
	}
