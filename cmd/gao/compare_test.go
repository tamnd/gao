...
const (
	compareNative     = "com-8b-sft-native"
	compareTranslated = "com-8b-sft-translated"
)

// compareProtocol writes an evaluation that was run properly: every fifth item read
...
func compareProtocol(t *testing.T, items int, pick func(item int) string) string {
	t.Helper()
	...
	for i := range items {
		...
		for k := range reads {
			left, right := compareNative, compareTranslated
			if (i%2 == 0) != (k == 1) {
				left, right = compareTranslated, compareNative
			}
...
func twoToOne(item int) string {
	if item%3 == 0 {
		return compareTranslated
	}
	return compareNative
}

func TestCompareReadsAProtocolThatWasRunProperlyAsAResult(t *testing.T) {
	out, errOut, code := exec(t, "compare", compareProtocol(t, 400, twoToOne))
...
func TestCompareSaysWhenAWinRateIsATie(t *testing.T) {
	out, errOut, code := exec(t, "compare", compareProtocol(t, 400, func(item int) string {
		if item%25 == 0 {
			return compareNative
		}
		if item%2 == 0 {
...
func TestCompareCatchesAnEvaluationOfItsOwnLayout(t *testing.T) {
	path := compareProtocol(t, 400, twoToOne)
	...
	out, _, code := exec(t, "compare", path)
	if code != 2 {
...
func TestComparePrintsTheSameReadingAsJSON(t *testing.T) {
	out, _, code := exec(t, "compare", "-json", compareProtocol(t, 400, twoToOne))
	if code != 0 {
	...
	if got.A != compareNative || got.B != compareTranslated {
...
func TestCompareRefusesAThirdSystem(t *testing.T) {
	path := compareProtocol(t, 400, twoToOne)
	body, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	...
	lines[3] = strings.Replace(lines[3], compareTranslated, "com-8b-sft-mixed", 1)
	if err := os.WriteFile(path, []byte(strings.Join(lines, "\n")+"\n"), 0o600); err != nil {
		t.Fatal(err)
	}

	out, errOut, code := exec(t, "compare", path)
	if code != 1 {
...
func TestCompareRefusesAnEvaluationTooSmallToReadAnIntervalOff(t *testing.T) {
	out, _, code := exec(t, "compare", compareProtocol(t, 100, twoToOne))
...
func TestCompareSaysWhichLineOfTheProtocolIsWrong(t *testing.T) {
	path := filepath.Join(t.TempDir(), "pairs.jsonl")
...
`), 0o600); err != nil {
	...
	_, errOut, code := exec(t, "compare", path)
	if code != 1 {
...
func TestCompareUsageErrors(t *testing.T) {
	if _, _, code := exec(t, "compare"); code != 2 {
		t.Errorf("no file: exit %d, want 2", code)
	}

	_, errOut, code := exec(t, "compare", "-h")
	if code != 2 {
...
func TestCompareIsInTheCommandList(t *testing.T) {
