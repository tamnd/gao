...
`)
	...
	for _, e := range t.Runs {
		if _, ok := may.Lookup(e.Box); !ok && e.Box != "" {
	...
	report := ghepReport{
		...
		MinGain: ghep.MinGain, MaxShare: ghep.MaxShare,
	...
	for _, e := range t.Ranked() {
		report.Readings = append(report.Readings, ghepReading{
...
// ghepReading is one initialization method as the table carries it.
type ghepReading struct {
...
type ghepReport struct {
	...
	Readings []ghepReading `json:"readings"`
...
func printGhep(w io.Writer, t ghep.Trial, claims []string) {
	...
	fmt.Fprintf(w, "A graft has to buy %s of fertility to be worth the parameters, and may spend %s of the run getting back to the loss it started at.\n",
		percent(ghep.MinGain), percent(ghep.MaxShare))
