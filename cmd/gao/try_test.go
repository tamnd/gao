...
// tryResults writes a full set of results for the fixed slate, with change
// applied to them first so a test can break exactly one thing.
func tryResults(t *testing.T, change func([]try.Result) []try.Result) string {
	t.Helper()
	s := try.Fixed()
	digest := s.Digest()
...
func TestTheSlatePrintsEveryRunItHolds(t *testing.T) {
	out, errOut, code := exec(t, "try", "slate")
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestTheKnobsViewSaysWhatTheSlateIsFor(t *testing.T) {
	out, _, code := exec(t, "try", "slate", "-knobs")
	if code != 0 {
		t.Fatalf("exit %d", code)
	}
...
func TestTheSlateSpeaksJSON(t *testing.T) {
	out, _, code := exec(t, "try", "slate", "-json")
	if code != 0 {
		t.Fatalf("exit %d", code)
	}
	var report struct {
	...
	if len(report.Slate.Runs) != try.Runs {
...
func TestABrokenSlateFromAFileIsReported(t *testing.T) {
	...
	out, _, code := exec(t, "try", "slate", "-slate", path)
	if code != 1 {
...
func TestTheReportPublishesTheRunsThatFoundNothing(t *testing.T) {
	out, errOut, code := exec(t, "try", "read", tryResults(t, nil))
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestTheNoiseFloorIsPrintedWithWhatItWasMeasuredFrom(t *testing.T) {
	out, _, code := exec(t, "try", "read", tryResults(t, nil))
	if code != 0 {
		t.Fatalf("exit %d", code)
	}
...
func TestASlateMissingRunsIsRefused(t *testing.T) {
	path := tryResults(t, func(r []try.Result) []try.Result { return r[:33] })
	out, _, code := exec(t, "try", "read", path)
	if code != 1 {
		t.Fatalf("exit %d, want 1", code)
	}
...
func TestAResultWithNoHardwareIsRefused(t *testing.T) {
	path := tryResults(t, func(rs []try.Result) []try.Result {
		for i, r := range rs {
			if r.Run == "E02" {
				rs[i].Box = ""
			}
...
func TestTheReportSpeaksJSONToo(t *testing.T) {
	out, _, code := exec(t, "try", "read", "-json", tryResults(t, nil))
	if code != 0 {
		t.Fatalf("exit %d", code)
	}
	var report struct {
	...
	if report.Baselines != try.Repeats || report.Noise <= 0 {
		t.Errorf("the JSON does not carry a measured floor: %+v", report)
	}
	if len(report.Findings) != try.Runs-try.Repeats {
...
func TestAResultsFileThatIsNotThereSaysSo(t *testing.T) {
	_, errOut, code := exec(t, "try", "read", filepath.Join(t.TempDir(), "nope.jsonl"))
	if code != 1 {
