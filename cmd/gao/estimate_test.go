...
// estimateSample writes k readings whose sizes vary the way real shards do.
func estimateSample(t *testing.T, k int) string {
	t.Helper()
	lines := make([]string, 0, k)
	for i := range k {
		lines = append(lines, partLine(i, int64(220+(i%9)*180), 0.225+0.004*float64(i%7)))
	}
	return estimateFile(t, lines...)
}

func estimateFile(t *testing.T, lines ...string) string {
	t.Helper()
...
func estimateArgs(path string, extra ...string) []string {
...
func TestEstimatePublishesAnIntervalRatherThanANumber(t *testing.T) {
	out, errOut, code := exec(t, estimateArgs(estimateSample(t, 44))...)
	if code != 0 {
...
func TestEstimateRefusesASampleTooSmallToCarryAnInterval(t *testing.T) {
	out, _, code := exec(t, estimateArgs(estimateSample(t, 12))...)
	if code != 1 {
...
func TestEstimateRefusesASampleWithNoSeed(t *testing.T) {
	path := estimateSample(t, 44)
	args := []string{"estimate", "-source", "hplt-v3", "-parts", "1214", "-bytes", "703000000000", path}
...
func TestEstimateExitsTwoWhenTheEstimateIsTooWideToPublish(t *testing.T) {
	...
	out, _, code := exec(t, estimateArgs(estimateFile(t, lines...))...)
	if code != 2 {
...
func TestEstimateChecksAnExactCountAgainstTheIntervalItPublished(t *testing.T) {
	path := estimateSample(t, 44)
	out, _, code := exec(t, estimateArgs(path, "-exact", "166000000000")...)
	if code != 0 {
	...
	out, _, code = exec(t, estimateArgs(path, "-exact", "142000000000")...)
	if code != 2 {
...
func TestEstimateJSONCarriesBothEstimatorsAndTheInterval(t *testing.T) {
	out, _, code := exec(t, estimateArgs(estimateSample(t, 44), "-json")...)
	if code != 0 {
		t.Fatalf("exit %d\n%s", code, out)
	}
...
func TestEstimateWithoutTheManifestTotalsIsAUsageError(t *testing.T) {
	path := estimateSample(t, 44)
	for _, args := range [][]string{
		{"estimate"},
		{"estimate", "-source", "x", path},
		{"estimate", "-source", "x", "-parts", "1214", path},
	} {
		if _, _, code := exec(t, args...); code != 2 {
			t.Errorf("%v exited %d and it is a usage error", args, code)
		}
	}
	if _, _, code := exec(t, estimateArgs(filepath.Join(t.TempDir(), "nothing.jsonl"))...); code != 1 {
