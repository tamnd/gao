...
// The logs these tests run against are the real training runs in spike/testdata,
...
func log(name string) string { return filepath.Join("..", "..", "spike", "testdata", name+".jsonl") }

func TestSpikePrintsWhatTheRunDidAndWhatItWouldHaveCost(t *testing.T) {
	out, errOut, code := exec(t, "spike", "-run", "on-dinh", "-total", "100000", "-checkpoint", "200", log("on-dinh"))
...
func TestSpikeExitsTwoWhenTheRunIsNotTheOneItLooksLike(t *testing.T) {
	out, _, code := exec(t, "spike", "-run", "phan-ky", "-total", "100000", "-checkpoint", "200", log("phan-ky"))
...
func TestSpikeExitsOneWhenTheLogCannotBeReadAgainstTheProtocol(t *testing.T) {
	out, _, code := exec(t, "spike", "-run", "on-dinh", "-total", "100000", log("on-dinh"))
...
func TestSpikePrintsJSON(t *testing.T) {
	out, _, _ := exec(t, "spike", "-run", "vot-len", "-total", "100000", "-checkpoint", "200", "-json", log("vot-len"))
...
// at.
func TestSpikePrintsAsManySpikesAsItWasAsked(t *testing.T) {
	out, _, code := exec(t, "spike", "-run", "vot-nhieu", "-total", "100000", "-checkpoint", "200", "-top", "3", log("vot-nhieu"))
...
func TestSpikeRefusesTheUsageErrors(t *testing.T) {
	for _, c := range []struct {
		name string
		args []string
	}{
		{"no log", []string{"spike", "-run", "on-dinh"}},
		{"two logs", []string{"spike", "-run", "on-dinh", log("on-dinh"), log("vot-len")}},
		{"a table of no rows at all", []string{"spike", "-run", "on-dinh", "-top", "-1", log("on-dinh")}},
		{"a flag nobody has", []string{"spike", "-nope"}},
		{"the help", []string{"spike", "-h"}},
	} {
...
func TestSpikeRefusesALogThatIsNotThere(t *testing.T) {
	missing := filepath.Join(t.TempDir(), "khong-co.jsonl")

	out, errOut, code := exec(t, "spike", "-run", "on-dinh", "-total", "100000", "-checkpoint", "200", missing)
...
func TestSpikeIsInTheCommandList(t *testing.T) {
	out, _, _ := exec(t, "help")
