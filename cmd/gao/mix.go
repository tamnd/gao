...
import (
	...
	"github.com/tamnd/gao/mix"
)

func runMix(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	s, err := mix.ReadSet(*name, fs.Arg(0))
	if err != nil {
	...
	report := mixReport{
		...
		Target:       mix.Target,
		Examples:     s.Examples(),
		Native:       s.Origin(mix.Native),
		Translated:   s.Origin(mix.Translated),
		Made:         s.Origin(mix.Made),
		Composition:  s.Composition(),
		Compared:     s.Compared(),
		Excluded:     s.Excluded(),
		NativeArm:    s.Arm(mix.Native),
		Arm:          s.ArmSize(),
		MinArm:       mix.MinArm,
		Drift:        s.Drift(),
		Drifted:      s.Drifted(),
		MaxDrift:     mix.MaxDrift,
		Matched:      s.Matched(),
...
type mixReport struct {
	...
	Native     mix.Mix `json:"native"`
	Translated mix.Mix `json:"translated"`
	Made       mix.Mix `json:"synthetic"`

	// Aside is the translated arm, which is composed and trained on and is not
	// in the number above.
	Aside int64 `json:"aside"`

	Composition []mix.Row `json:"composition"`
...
func printSlate(stdout, stderr io.Writer, asJSON bool) int {
	...
	fmt.Fprintf(stdout, "\n%s examples over %d capabilities, with the floor on each one set by how much of that capability translation gets wrong rather than by how much of it we have.\n",
		examples(mix.Target), len(mix.Slate))
	return 0
}

func printMix(w io.Writer, r mixReport) {
	...
	for _, m := range []mix.Mix{r.Native, r.Translated, r.Made} {
		if m.Slices == 0 {
			continue
		}
	...
	if len(r.Unproven) > 0 {
		fmt.Fprintf(w, "%s did not keep their origin label through the audit and stopped counting: %s.\n",
			plural(len(r.Unproven), "slice"), strings.Join(r.Unproven, ", "))
	}
