...
import (
	...
	"github.com/tamnd/gao/total"
)

func runTotal(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	s, err := total.ReadSheet(*release, fs.Arg(0))
	if err != nil {
...
type totalReport struct {
	...
	Corpus      total.Total `json:"corpus"`
	Publishable total.Total `json:"publishable"`
	Generated   total.Total `json:"generated"`

	Sources []total.Total `json:"sources"`
	Classes []total.Total `json:"classes"`
...
func printTotal(w io.Writer, s total.Sheet, r totalReport) {
