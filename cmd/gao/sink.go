...
import (
	...
	"github.com/tamnd/gao/sink"
)

func runSink(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	s, err := sink.ReadStep(*model, *loss, *ref, fs.Arg(0))
	if err != nil {
	...
	report := sinkReport{
		...
		Flushed: sink.Flushed, Saturated: sink.Saturated, Aligned: sink.Aligned,
	...
	for _, t := range s.Ranked() {
		report.Readings = append(report.Readings, sinkReading{
...
// sinkReading is one tensor as the report carries it, which is shares and where
...
type sinkReport struct {
	...
	Readings []sinkReading `json:"readings"`
...
func printSink(w io.Writer, s sink.Step) {
	...
	for _, t := range s.Ranked() {
		...
		fmt.Fprintf(tw, "%s\t%s\t%s\t%s\t%s\t%g\t%.2e\t%.1fx\t%.4f\t%s\n",
			t.Name, t.Kind, elementCount(t.Live()), sinkShare(t.Underflow()), sinkShare(t.Saturation()),
	...
	fmt.Fprintf(w, "E4M3 tops out at %g and its floor is %g, so a value under %.2e times the scale is a zero.\n",
		sink.MaxNormal, sink.MinSubnormal, sink.MinSubnormal)
	fmt.Fprintf(w, "The lines are %s of live values flushed, %s clipped, and %.3f against the same tensor in BF16.\n",
		sinkShare(sink.Flushed), sinkShare(sink.Saturated), sink.Aligned)
	if s.Reference > 0 {
