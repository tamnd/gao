...
// chamRolloutLine is the rollout file record, written here the way a sampler
// would write it rather than by reusing the reader's own struct.
type chamRolloutLine struct {
...
func TestChamRosterSaysWhichVerifiersAreNotBuilt(t *testing.T) {
	out, _, code := exec(t, "cham", "roster")
	if code != 0 {
...
func TestChamRosterPrintsTheSameThingAsJSON(t *testing.T) {
	out, _, code := exec(t, "cham", "roster", "-json")
	if code != 0 {
		t.Fatalf("exit %d", code)
	}
	var got []cham.Specialist
	if err := json.Unmarshal([]byte(out), &got); err != nil {
		t.Fatalf("%v in %q", err, out)
	}
	if len(got) != len(cham.Specialists()) {
		t.Fatalf("the JSON roster holds %d arms and the package holds %d", len(got), len(cham.Specialists()))
	}
	for _, s := range got {
		if s.Checked == "" {
...
func TestChamDauGradesRolloutsAgainstThePagesTheyCameFrom(t *testing.T) {
	page := dauPages[0]
	prompt := phoi.Bare(page)
	...
	for i := range half {
		if i%2 == 1 {
			half[i] = phoi.Bare(half[i])
		}
	}
	rollouts := writeLines(t, "rollouts.jsonl", chamRolloutLine{
		Prompt:  prompt,
		Answers: []string{page, strings.Join(half, " "), prompt, dauPages[1]},
	})

	out, errOut, code := exec(t, append([]string{"cham", "dau", "-rollouts", rollouts, "-v"}, dauCorpus(t, dauPages)...)...)
	if code != 0 {
		t.Fatalf("exit %d\n%s\n%s", code, out, errOut)
	}
...
func TestChamDauScoresThePerfectAnswerAboveTheQuestionHandedBack(t *testing.T) {
	page := dauPages[2]
	prompt := phoi.Bare(page)
	rollouts := writeLines(t, "rollouts.jsonl", chamRolloutLine{
		Prompt:  prompt,
	...
	out, errOut, code := exec(t, append([]string{"cham", "dau", "-rollouts", rollouts, "-json"}, dauCorpus(t, dauPages)...)...)
	if code != 0 {
		t.Fatalf("exit %d\n%s", code, errOut)
	}
	var report struct {
...
func TestChamDauDropsWhatTheSamplerCutOff(t *testing.T) {
	page := dauPages[0]
	prompt := phoi.Bare(page)
	rollouts := writeLines(t, "rollouts.jsonl", chamRolloutLine{
		Prompt:   prompt,
	...
	out, errOut, code := exec(t, append([]string{"cham", "dau", "-rollouts", rollouts}, dauCorpus(t, dauPages)...)...)
	if code == 0 {
...
func TestChamDauRefusesACorpusTypedWithoutMarks(t *testing.T) {
	bare := make([]string, len(dauPages))
	for i, p := range dauPages {
		bare[i] = phoi.Bare(p)
	}
	rollouts := writeLines(t, "rollouts.jsonl", chamRolloutLine{
		Prompt:  bare[0],
		Answers: []string{dauPages[0]},
	})

	_, errOut, code := exec(t, append([]string{"cham", "dau", "-rollouts", rollouts}, dauCorpus(t, bare)...)...)
	if code == 0 {
...
func TestChamDauSaysWhichFileItCouldNotRead(t *testing.T) {
	missing := filepath.Join(t.TempDir(), "khong-co.jsonl")
	_, errOut, code := exec(t, append([]string{"cham", "dau", "-rollouts", missing}, dauCorpus(t, dauPages)...)...)
	if code != 1 {
...
// chamRegisterLine is a register file record.
type chamRegisterLine struct {
	Kind     string `json:"kind"`
	ID       string `json:"id"`
	Articles int    `json:"articles"`
}

func chamRegister(t *testing.T) string {
	t.Helper()
	return writeLines(t, "register.jsonl",
		chamRegisterLine{Kind: "nghị định", ID: "13/2023/NĐ-CP", Articles: 44},
		chamRegisterLine{Kind: "luật", ID: "24/2018/QH14", Articles: 43},
		chamRegisterLine{Kind: "quyết định", ID: "749/QĐ-TTg"},
	)
}

const chamQuestion = "Doanh nghiệp phải làm gì khi dữ liệu cá nhân của khách hàng bị lộ?"

func TestChamTrichGradesCitationsAgainstTheRegister(t *testing.T) {
	rollouts := writeLines(t, "rollouts.jsonl", chamRolloutLine{
		Prompt: chamQuestion,
	...
	out, errOut, code := exec(t, "cham", "trich", "-register", chamRegister(t), "-json", rollouts)
	if code != 0 {
		t.Fatalf("exit %d\n%s", code, errOut)
	}
	var report struct {
...
func TestChamTrichRefusesAQuestionNoAnswerCouldWin(t *testing.T) {
	rollouts := writeLines(t, "rollouts.jsonl", chamRolloutLine{
		Prompt:  chamQuestion,
	...
	_, errOut, code := exec(t, "cham", "trich", "-register", chamRegister(t), rollouts)
	if code != 1 {
		t.Fatalf("exit %d, want 1", code)
	}
...
func TestChamTrichSaysWhichRegisterLineItCannotUse(t *testing.T) {
	bad := writeLines(t, "register.jsonl",
		chamRegisterLine{Kind: "nghị định", ID: "13/2023/NĐ-CP", Articles: 44},
		chamRegisterLine{Kind: "nghị định", ID: "749/QĐ-TTg"},
	)
	rollouts := writeLines(t, "rollouts.jsonl", chamRolloutLine{
		Prompt:  chamQuestion,
		Must:    []string{"nghị định số 13/2023/NĐ-CP"},
	...
	_, errOut, code := exec(t, "cham", "trich", "-register", bad, rollouts)
	if code != 1 {
		t.Fatalf("exit %d, want 1", code)
	}
...
func TestChamTrichPrintsWhatTheRegisterAndTheKeyHold(t *testing.T) {
	rollouts := writeLines(t, "rollouts.jsonl", chamRolloutLine{
		Prompt: chamQuestion,
		Must:   []string{"nghị định số 13/2023/NĐ-CP"},
		Answers: []string{
	...
	out, errOut, code := exec(t, "cham", "trich", "-register", chamRegister(t), rollouts)
	if code != 0 {
		t.Fatalf("exit %d\n%s", code, errOut)
	}
...
func TestChamIsInTheHelp(t *testing.T) {
	...
	_, errOut, code := exec(t, "cham", "khong-co")
	if code != 2 {
