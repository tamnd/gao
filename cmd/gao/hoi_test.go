...
var hoiKinds = []string{"tong-hop", "so-sanh", "trinh-tu", "sua-doi", "dem-so"}

// hoiLine is one question that came back the way a good one comes back.
func hoiLine(i, tokens int, doc string, edit func(*map[string]any)) string {
	q := map[string]any{
		"id":          fmt.Sprintf("vi-longdoc-%04d", i),
		"document":    doc,
		"kind":        hoiKinds[i%len(hoiKinds)],
		"tokens":      tokens,
		"closed_book": true,
		"recalled":    false,
		"graders":     2,
...
// hoiSet writes n questions across all three rungs and enough documents that no
// one of them carries the set.
func hoiSet(t *testing.T, n int, edit func(i int, q *map[string]any)) string {
	t.Helper()
	...
	for i := range n {
		...
		lines = append(lines, hoiLine(i, lengths[i%len(lengths)], doc, apply))
	}
	return hoiFile(t, lines...)
}

func hoiFile(t *testing.T, lines ...string) string {
	t.Helper()
...
func TestHoiReportsTheLadderRatherThanTheCount(t *testing.T) {
	out, errOut, code := exec(t, "hoi", hoiSet(t, 600, nil))
	if code != 0 {
...
func TestHoiExitsTwoWhenNothingInTheSetLivesAtTheTopRung(t *testing.T) {
	lengths := []int{38_000, 44_000, 71_000, 88_000}
	lines := make([]string, 0, 600)
	for i := range 600 {
		lines = append(lines, hoiLine(i, lengths[i%len(lengths)], fmt.Sprintf("vbpl-%d-%03d", 2004+i%20, i%97), nil))
	}
	out, _, code := exec(t, "hoi", hoiFile(t, lines...))
	if code != 2 {
...
func TestHoiCountsTheQuestionsAModelAnsweredWithNoDocument(t *testing.T) {
	path := hoiSet(t, 640, func(i int, q *map[string]any) {
		if i%16 == 0 {
			(*q)["recalled"] = true
		}
	})
	out, _, code := exec(t, "hoi", path)
	if code != 0 {
...
func TestHoiRefusesASetBuiltOnOneDocument(t *testing.T) {
	path := hoiSet(t, 600, func(i int, q *map[string]any) {
		if i%4 == 0 {
...
func TestHoiNamesWhatEachRejectedQuestionFailed(t *testing.T) {
	path := hoiSet(t, 600, func(i int, q *map[string]any) {
		switch i {
		case 5:
	...
	out, _, code := exec(t, "hoi", "-rejects", path)
	if code != 0 {
		t.Fatalf("exit %d\n%s", code, out)
	}
...
func TestHoiJSONCarriesTheLadderAndTheClosedBookRun(t *testing.T) {
	out, _, code := exec(t, "hoi", "-json", hoiSet(t, 600, nil))
	if code != 0 {
		t.Fatalf("exit %d\n%s", code, out)
	}
	for _, want := range []string{
...
func TestHoiWithoutASetIsAUsageError(t *testing.T) {
	if _, _, code := exec(t, "hoi"); code != 2 {
		t.Error("hoi with no argument did not exit 2")
	}
	if _, _, code := exec(t, "hoi", filepath.Join(t.TempDir(), "nothing.jsonl")); code != 1 {
