...
func TestNormalizeWritesTheNormalizedText(t *testing.T) {
	path := writeDocument(t, "Hoà bình\r\n")

	var stdout, stderr bytes.Buffer
	if code := runNormalize(&stdout, &stderr, []string{path}); code != 0 {
...
func TestNormalizeReportsWhatItDid(t *testing.T) {
	path := writeDocument(t, "Hoà bình và dduwowngj dài\n")

	var stdout, stderr bytes.Buffer
	if code := runNormalize(&stdout, &stderr, []string{"-report", path}); code != 0 {
...
func TestNormalizeSaysWhyADocumentDoesNotGoOn(t *testing.T) {
...
func TestNormalizeReportsSeveralDocumentsWithATotal(t *testing.T) {
	dir := t.TempDir()
	...
	var stdout, stderr bytes.Buffer
	if code := runNormalize(&stdout, &stderr, append([]string{"-report"}, paths...)); code != 0 {
		t.Fatalf("gao normalize -report = %d, want 0\n%s", code, stderr.String())
	}
...
func TestNormalizeReportsJSON(t *testing.T) {
	path := writeDocument(t, "Hoà bình\n")

	var stdout, stderr bytes.Buffer
	if code := runNormalize(&stdout, &stderr, []string{"-report", "-json", path}); code != 0 {
		t.Fatalf("gao normalize -report -json = %d, want 0\n%s", code, stderr.String())
	}
	var got normalizeReport
...
func TestNormalizeRefusesJSONWithoutReport(t *testing.T) {
	var stdout, stderr bytes.Buffer
	if code := runNormalize(&stdout, &stderr, []string{"-json"}); code != 2 {
...
func TestNormalizeSaysWhichFileItCouldNotRead(t *testing.T) {
	var stdout, stderr bytes.Buffer
	if code := runNormalize(&stdout, &stderr, []string{filepath.Join(t.TempDir(), "gone.txt")}); code != 1 {
...
func TestNormalizeReportsEveryRowOfAPart(t *testing.T) {
...
func TestNormalizePrintsATotalWithoutALinePerDocument(t *testing.T) {
	path := writePart(t, "Hoà bình\n", "Hà Nội\n", "Thuỷ chung\n")

	var stdout, stderr bytes.Buffer
	if code := runNormalize(&stdout, &stderr, []string{"-report", "-total", path}); code != 0 {
...
func TestNormalizeSeparatesRepairFromLayout(t *testing.T) {
	path := writePart(t, "Hoà bình\n", "Thuỷ chung\n", "Hà Nội  \n", "Hà Nội\n")
...
func TestNormalizeRefusesTotalWithoutReport(t *testing.T) {
	var stdout, stderr bytes.Buffer
	if code := runNormalize(&stdout, &stderr, []string{"-total"}); code != 2 {
...
func TestNormalizeRefusesToNormalizeAPartOntoOneStream(t *testing.T) {
	path := writePart(t, "Hoà bình\n", "Hà Nội\n")

	var stdout, stderr bytes.Buffer
	if code := runNormalize(&stdout, &stderr, []string{path}); code != 2 {
...
// encoding it was.
func TestNormalizeNamesTheFontEncodingItReadADocumentOutOf(t *testing.T) {
...
func TestNormalizeWritesTheTranscodedText(t *testing.T) {
...
func TestNormalizeBreaksTheFontEncodingsDownByName(t *testing.T) {
