...
const (
	xayArticle = "Hà Nội sẽ mở rộng mạng lưới đường sắt đô thị trong mười năm tới. " +
	...
	xaySyndicated = "Thủ đô chi thêm cho đường sắt đô thị. " +
	...
	xayRiver = "Mùa nước nổi năm nay về muộn hơn mọi năm ở đầu nguồn sông Cửu Long. " +
...
func TestXayCountsTheCopiesAndTheDocumentsLeft(t *testing.T) {
	dir := t.TempDir()
	files := []string{
		writeText(t, dir, "a.txt", xayArticle),
		writeText(t, dir, "b.txt", xaySyndicated),
		writeText(t, dir, "c.txt", xayRiver),
	}
...
func TestXaySaysWhatItRanAt(t *testing.T) {
	dir := t.TempDir()
	files := []string{
		writeText(t, dir, "a.txt", xayArticle),
		writeText(t, dir, "b.txt", xaySyndicated),
...
func TestXayReadsAParquetPart(t *testing.T) {
...
func TestXayPrintsTheAblationCurve(t *testing.T) {
	dir := t.TempDir()
	files := []string{
		writeText(t, dir, "a.txt", xayArticle),
		writeText(t, dir, "b.txt", xaySyndicated),
...
func TestXayPrintsTheCurveAsATable(t *testing.T) {
	dir := t.TempDir()
	files := []string{
		writeText(t, dir, "a.txt", xayArticle),
		writeText(t, dir, "b.txt", xaySyndicated),
...
func TestXayRefusesAThresholdThatIsNotASimilarity(t *testing.T) {
	path := writeText(t, t.TempDir(), "a.txt", xayArticle)
...
func TestXayIsInTheUsage(t *testing.T) {
