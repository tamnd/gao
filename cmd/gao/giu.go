...
`)
	...
	report := giuReport{
		...
		Recovers: giu.Recovers, Merges: giu.Merges,
	...
	for _, s := range p.Ranked() {
		report.Kept = append(report.Kept, giuKept{
...
// giuKept is one specialist as the report carries it, which is a retention
...
func printGiu(w io.Writer, p giu.Panel) {
	...
	fmt.Fprintf(w, "P09-2 asks for %.0f%% kept by distillation and %.0f%% or less by averaging the same checkpoints.\n",
		100*giu.Recovers, 100*giu.Merges)
	if p.Hides() {
