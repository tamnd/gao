...
func frontierRun(t *testing.T, in string, args ...string) (string, string, int) {
	t.Helper()
	if in != "" {
		old := stdin
	...
	code := run(&out, &errb, append([]string{"frontier"}, args...))
	return out.String(), errb.String(), code
}

func TestFrontierCanonShowsWhatMergedWithWhat(t *testing.T) {
	out, _, code := frontierRun(t, "",
		"canon",
...
func TestFrontierCanonSaysWhatItWillNotFollow(t *testing.T) {
	out, _, code := frontierRun(t, "", "canon", "javascript:void(0)", "https://vnexpress.net/a")
	if code != 0 {
		t.Fatalf("exit %d: %s", code, out)
	}
...
func TestFrontierShapeCountsTemplatesRatherThanURLs(t *testing.T) {
	out, _, code := frontierRun(t, "", "shape", "-count",
...
// what it is called.
func TestFrontierShapeSaysWhatIsWrongWithATemplate(t *testing.T) {
	out, _, code := frontierRun(t, "", "shape",
...
func TestFrontierBudgetSaysWhatItWouldNotAskFor(t *testing.T) {
	out, _, code := frontierRun(t, "", "budget",
...
func TestFrontierBudgetReportsWhatEachTemplateSpent(t *testing.T) {
	out, _, code := frontierRun(t, "", "budget", "-shapes",
		"https://diendan.vn/thread/1",
		"https://diendan.vn/thread/2",
	)
	if code != 0 {
...
func TestFrontierReadsAFrontierFromStandardInput(t *testing.T) {
	in := `
...
https://tuoitre.vn/a
`
	out, _, code := frontierRun(t, in, "shape", "-count")
	if code != 0 {
		t.Fatalf("exit %d: %s", code, out)
	}
...
func TestFrontierNeedsSomethingToWorkOn(t *testing.T) {
	out, errb, code := frontierRun(t, "", "canon")
	if code == 0 {
...
func TestFrontierIsInTheSubcommandList(t *testing.T) {
	var out, errb strings.Builder
	...
	out.Reset()
	if code := run(&out, &errb, []string{"frontier", "help"}); code != 0 {
		t.Fatalf("exit %d", code)
	}
...
func TestFrontierRefusesASubcommandItDoesNotHave(t *testing.T) {
	var out, errb strings.Builder
	if code := run(&out, &errb, []string{"frontier", "frontier"}); code == 0 {
...
func TestTheFrontierWePlanToRunFitsOnTheBoxWePlanToRunItFrom(t *testing.T) {
	out, errOut, code := exec(t, "frontier", "fit")
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestAFrontierThatDoesNotFitStopsTheCrawl(t *testing.T) {
	out, _, code := exec(t, "frontier", "fit", "-active", "900000", "-ready", "64")
	if code != 1 {
...
func TestTheFitCheckRunsAgainstAnyBoxOnTheFleet(t *testing.T) {
	out, _, code := exec(t, "frontier", "fit", "-box", "server3")
	if code != 0 {
		t.Fatalf("exit %d:\n%s", code, out)
	}
...
func TestAFitCheckAgainstABoxWeDoNotHaveNamesTheOnesWeDo(t *testing.T) {
	_, errOut, code := exec(t, "frontier", "fit", "-box", "server9")
	if code != 2 {
...
func TestTheFitCheckTakesNoURLsAndSaysWhy(t *testing.T) {
	_, errOut, code := exec(t, "frontier", "fit", "https://vnexpress.net/")
	if code != 2 {
		t.Errorf("exit %d, want 2", code)
	}
...
func TestTheFitCheckSpeaksJSON(t *testing.T) {
	out, _, code := exec(t, "frontier", "fit", "-json")
	if code != 0 {
		t.Fatalf("exit %d:\n%s", code, out)
	}
	var report struct {
...
func TestTheArithmeticCanBeCheckedAgainstThisMachine(t *testing.T) {
	...
	out, _, code := exec(t, "frontier", "fit", "-measure", "2000")
	if code != 0 {
		t.Fatalf("exit %d:\n%s", code, out)
	}
...
func TestAPlanThatIsNotAPlanIsFaultedBeforeItIsCosted(t *testing.T) {
	out, _, code := exec(t, "frontier", "fit", "-active", "0")
	if code != 1 {
		t.Fatalf("exit %d, want 1:\n%s", code, out)
	}
