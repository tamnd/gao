...
// layersLayers writes a layer file with the named buckets read at 40 MB each.
func layersFile(t *testing.T, read ...int) string {
	t.Helper()
	want := map[int]bool{}
	for _, r := range read {
		want[r] = true
	...
	lines := make([]string, 0, len(layersBuckets))
	for _, b := range layersBuckets {
		if !want[b.rank] {
...
func layersSample(t *testing.T) string { return layersFile(t, 5, 7, 8, 9, 10) }

func TestLayersSaysWhatTheLayersNobodyOpenedAreWorth(t *testing.T) {
	out, errOut, code := exec(t, "layers", "-source", "hplt-v3 vie_Latn", layersSample(t))
...
func TestLayersSaysTheRangeDoesNotCloseByReadingTheSameLayersAgain(t *testing.T) {
	out, _, _ := exec(t, "layers", "-source", "hplt-v3 vie_Latn", layersSample(t))
...
// layersWhole writes a layer file with every layer read end to end rather than
// sampled, which is the only shape that has nothing left to assume.
func layersWhole(t *testing.T) string {
	t.Helper()
	lines := make([]string, 0, len(layersBuckets))
	for _, b := range layersBuckets {
...
func TestLayersHoldsWhenEveryLayerWasRead(t *testing.T) {
	out, errOut, code := exec(t, "layers", "-source", "hplt-v3 vie_Latn", layersWhole(t))
...
func TestLayersOverTheRealReadingOfEveryBucket(t *testing.T) {
	out, errOut, code := exec(t, "layers", "-source", "hplt3", filepath.Join("..", "..", "layers", "testdata", "hplt3-vie_Latn-s1.jsonl"))
...
func TestLayersChecksTheNumberTheProjectPublishes(t *testing.T) {
	out, _, code := exec(t, "layers", "-source", "hplt-v3 vie_Latn", "-quoted", "400000000000", layersSample(t))
...
func TestLayersPrintsTheSameReadingAsJSON(t *testing.T) {
	out, _, code := exec(t, "layers", "-source", "hplt-v3 vie_Latn", "-json", layersSample(t))
	if code != 2 {
...
func TestLayersRefusesAReadingThatIsNotStratified(t *testing.T) {
	out, errOut, code := exec(t, "layers", "-source", "hplt-v3 vie_Latn", layersFile(t))
...
func TestLayersSaysWhichLineOfTheLayersIsWrong(t *testing.T) {
	path := filepath.Join(t.TempDir(), "layers.jsonl")
...
`), 0o600); err != nil {
	...
	_, errOut, code := exec(t, "layers", "-source", "hplt-v3 vie_Latn", path)
	if code != 1 {
...
func TestLayersUsageErrors(t *testing.T) {
	if _, _, code := exec(t, "layers"); code != 2 {
		t.Errorf("no source and no file: exit %d, want 2", code)
	}
	if _, _, code := exec(t, "layers", "layers.jsonl"); code != 2 {
		t.Errorf("a file and no source: exit %d, want 2", code)
	}

	_, errOut, code := exec(t, "layers", "-h")
	if code != 2 {
...
func TestLayersIsInTheCommandList(t *testing.T) {
