...
// takedownRun runs a gao takedown command against a register written for the test, and
...
func takedownRun(t *testing.T, register string, args ...string) (string, string, int) {
	t.Helper()
	...
	code := runTakedown(&stdout, &stderr, append([]string{args[0], "-file", path}, args[1:]...))
...
func TestTakedownStatusReportsWhatWasPromisedAndWhatHappened(t *testing.T) {
	stdout, _, code := takedownRun(t, filed, "status")
	if code == 0 {
...
func TestTakedownStatusCanShowTheWholeRecord(t *testing.T) {
	stdout, _, _ := takedownRun(t, filed, "status", "-all")
...
func TestTakedownStatusOverAnEmptyRegisterSaysSo(t *testing.T) {
	stdout, _, code := takedownRun(t, "", "status")
	if code != 0 {
...
func TestTakedownStatusReportsTheObjectionRate(t *testing.T) {
	stdout, _, _ := takedownRun(t, filed, "status", "-crawled", "1000")
	if !strings.Contains(stdout, "0.200% of 1000 hosts crawled") {
		t.Errorf("the rate is missing:\n%s", stdout)
	}
}

func TestTakedownCheckFindsARowThatCannotBeTrue(t *testing.T) {
	_, stderr, code := takedownRun(t, `
[[request]]
issue = 92
host = "a.vn"
scope = "stop"
...
func TestTakedownCheckPassesAGoodRegister(t *testing.T) {
	stdout, stderr, code := takedownRun(t, filed, "check")
	if code != 0 {
...
func TestTheRegisterInTheRepositoryIsReadable(t *testing.T) {
	var stdout, stderr bytes.Buffer
	if code := runTakedown(&stdout, &stderr, []string{"check", "-file", filepath.Join("..", "..", "GO-BO.toml")}); code != 0 {
...
func TestTakedownURLAnswersBothGates(t *testing.T) {
	stdout, _, code := takedownRun(t, filed, "url", "-fetched", "2026-01-05", "https://tin.example.vn/bai/1")
	if code != 1 {
	...
	stdout, _, _ = takedownRun(t, filed, "url", "-fetched", "2026-03-15", "https://tin.example.vn/bai/1")
	if !strings.Contains(stdout, "do not fetch, do not publish") {
		t.Errorf("a document fetched after the request was kept:\n%s", stdout)
	}
}

func TestTakedownURLLeavesAStrangerAlone(t *testing.T) {
	stdout, _, code := takedownRun(t, filed, "url", "https://notexample.vn/a")
	if code != 0 {
...
func TestTakedownURLWantsAURL(t *testing.T) {
	if _, _, code := takedownRun(t, filed, "url"); code != 2 {
		t.Errorf("gao takedown url with nothing to check exited %d", code)
	}
}

func TestTakedownOnARegisterThatIsNotThere(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := runTakedown(&stdout, &stderr, []string{"status", "-file", filepath.Join(t.TempDir(), "khong-co.toml")})
	if code != 1 {
...
func TestTakedownIsInTheCommandList(t *testing.T) {
	var stdout, stderr bytes.Buffer
	if code := run(&stdout, &stderr, []string{"takedown"}); code != 2 {
