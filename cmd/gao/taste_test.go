...
func hpltPlan() (layers, files string) {
	return filepath.Join("..", "..", "sample", "testdata", "hplt3-vie_Latn-layers.jsonl"),
		filepath.Join("..", "..", "sample", "testdata", "hplt3-vie_Latn-listing.jsonl")
}

func TestTasteRefusesBeforeItFetchesAnything(t *testing.T) {
	...
	for _, c := range []struct {
		...
		t.Run(c.name, func(t *testing.T) {
			out, errOut, code := exec(t, append([]string{"taste"}, c.args...)...)
			if code == 0 {
...
func TestTasteLoadsTheTokenizerBeforeTheFirstByteMoves(t *testing.T) {
	layers, files := hpltPlan()
	out := filepath.Join(t.TempDir(), "read.jsonl")

	_, errOut, code := exec(t, "taste", "-source", "hplt3", "-seed", "s1", "-layers", layers,
...
func TestTasteUsageNamesWhatItDoesAndWhatItCosts(t *testing.T) {
	_, errOut, code := exec(t, "taste")
	if code != 2 {
