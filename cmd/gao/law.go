...
func runLaw(stdout, stderr io.Writer, args []string) int {
	...
	if *source != "" {
		...
		printDeterminations(stdout, law.For(s), true)
		return 0
	}

	fmt.Fprintf(stdout, "questions for counsel, filed %s\n\n", law.FiledOn)
	for i, q := range law.Questions() {
		if i > 0 {
			fmt.Fprintln(stdout)
		}
	...
	printDeterminations(stdout, law.Determinations(), *verbose)
...
func printQuestion(w io.Writer, q law.Question, verbose bool) {
	fmt.Fprintf(w, "%-4s%s\n", q.ID, q.Ask)
	if q.Answered() {
...
func printDeterminations(w io.Writer, ds []law.Determination, verbose bool) {
	if len(ds) == 0 {
...
func perItem(d law.Determination) string {
	if d.PerItem {
