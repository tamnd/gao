...
func runTighten(stdout, stderr io.Writer, args []string) int {
	if len(args) == 0 {
		tightenUsage(stderr)
		return 2
	}
	switch args[0] {
	case "recipe":
		return runTightenRecipe(stdout, stderr, args[1:])
	case "read":
		return runTightenRead(stdout, stderr, args[1:])
	case "help", "-h", "--help":
		tightenUsage(stdout)
		return 0
	default:
...
// tightenConfig reads the configuration a subcommand was pointed at, which is the
// plan's own when nobody pointed it anywhere.
func tightenConfig(stderr io.Writer, path string) (tighten.Recipe, bool) {
	if path == "" {
		return tighten.Plan(), true
	}
	r, err := tighten.ReadRecipe(path)
	if err != nil {
		fmt.Fprintf(stderr, "gao tighten: %v\n", err)
		return tighten.Recipe{}, false
	}
	return r, true
}

func runTightenRecipe(stdout, stderr io.Writer, args []string) int {
	...
	recipe, ok := tightenConfig(stderr, *config)
	if !ok {
		return 1
	}
	return tightenRecipe(stdout, recipe, *asJSON, *why)
}

func runTightenRead(stdout, stderr io.Writer, args []string) int {
	...
	return tightenRead(stdout, stderr, recipe, *specialist, fs.Arg(0), *asJSON)
}
...
type tightenRecipeReport struct {
	Recipe   tighten.Recipe `json:"recipe"`
	Rows     []tightenRow   `json:"rows"`
	Context  int            `json:"context"`
	Holds    bool           `json:"holds"`
	Blocking []string       `json:"blocking,omitempty"`
}

func tightenRecipe(stdout io.Writer, r tighten.Recipe, asJSON, why bool) int {
	report := tightenRecipeReport{
		Recipe:   r,
		Context:  tighten.Context,
		Holds:    r.Holds(),
		Blocking: r.Blocking(),
	}
	for _, row := range r.Rows() {
		report.Rows = append(report.Rows, tightenRow(row))
	}
	...
	if asJSON {
		...
		if len(report.Blocking) > 0 {
			...
			fmt.Fprintf(stdout, "\nA prompt of %d and an answer of %d sit inside the %d the base model has, and the settings above are the ones the plan fixed.\n",
				r.Prompt, r.MaxResponse, tighten.Context)
		}
	}
...
func tightenRead(stdout, stderr io.Writer, recipe tighten.Recipe, specialist, path string, asJSON bool) int {
	r, err := tighten.ReadRun(specialist, path, recipe)
	if err != nil {
		fmt.Fprintf(stderr, "gao tighten: %v\n", err)
		return 1
	}
...
func printTighten(w io.Writer, r tightenRunReport) {
	if len(r.Blocking) > 0 {
