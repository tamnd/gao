...
`)
	...
	s := nem.Of(p, tokenizerLabel(tok), takes)
	report := nemReport{
		Sample:   s,
		Faults:   s.Faults(),
		Blocking: s.Blocking(),
...
func readTake(ctx context.Context, stdout io.Writer, f *gat.Fetcher, d gat.Decoder, pin gat.Pinned, layer string, t mau.Take, tok *dem.Tokenizer) (nem.Take, error) {
	file, ok := fileIn(pin, t.Path)
	if !ok {
		return nem.Take{}, fmt.Errorf("the listing draws %s and the manifest does not pin it, so there is nothing to fetch it from", t.Path)
	}
	...
	out := nem.Take{Layer: layer, Path: t.Path, Bytes: t.Bytes, Of: t.Of, Whole: t.Bytes >= t.Of}
	...
	if err != nil && !endedInsideARecord(err, out.Whole) {
		return nem.Take{}, fmt.Errorf("reading %s: %w", t.Path, err)
	}
...
func fileIn(p gat.Pinned, path string) (gat.File, bool) {
	for _, f := range p.Files {
		if f.Path == path {
			return f, true
		}
	}
	return gat.File{}, false
}

// pinnedFor reads the ingest manifest and returns the one source wanted.
func pinnedFor(s doc.Source) (gat.Pinned, error) {
	p, ok := gat.Pin(s)
	if !ok {
		return gat.Pinned{}, fmt.Errorf("the ingest manifest does not pin %s, so there is nowhere to fetch its shards from", s)
	}
	return p, nil
}

// writeLayers writes the layer file tang reads, one JSON object per line.
func writeLayers(path string, layers []tang.Layer) error {
...
func printNem(w io.Writer, r nemReport) {
	if len(r.Blocking) > 0 {
