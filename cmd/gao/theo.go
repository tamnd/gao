...
func runTheo(stdout, stderr io.Writer, args []string) int {
	if len(args) == 0 {
		theoUsage(stderr)
		return 2
	}
	switch args[0] {
	case "items":
		return runTheoItems(stdout, stderr, args[1:])
	case "grade":
		return runTheoGrade(stdout, stderr, args[1:])
	case "help", "-h", "--help":
		theoUsage(stdout)
		return 0
	default:
...
func runTheoItems(stdout, stderr io.Writer, args []string) int {
	...
	s, code := readTheoSet(stderr, *path)
	if code != 0 {
		return code
	}
	faults := s.Faults()
	...
	switch {
	case *asJSON:
		if code := printJSON(stdout, stderr, theoItemsReport{Set: s, Digest: s.Digest().String(), Faults: faults}); code != 0 {
			return code
		}
	case *prompts:
		printTheoPrompts(stdout, s)
	default:
		printTheoKinds(stdout, s)
	}
...
`)
	...
	out := theoGradeReport{Score: score, Faults: append(s.Faults(), score.Publishable()...)}
	if *asJSON {
		if code := printJSON(stdout, stderr, out); code != 0 {
			return code
		}
	} else {
		printTheoScore(stdout, s, out)
	}
...
type theoItemsReport struct {
	Set    theo.Set `json:"set"`
	Digest string   `json:"digest"`
	Faults []string `json:"faults,omitempty"`
}

type theoGradeReport struct {
	theo.Score
	Faults []string `json:"faults,omitempty"`
}

func readTheoSet(stderr io.Writer, path string) (theo.Set, int) {
	if path == "" {
		return theo.Fixed(), 0
	}
...
func printTheoScore(w io.Writer, s theo.Set, out theoGradeReport) {
	...
	if sc.Passed {
		...
		fmt.Fprintf(w, "fail: %.1f%% of the answers came back in the language the item asked for, against a floor of %.0f%%.\n",
			100*sc.Adherence, 100*theo.MinAdherence)
	}
	if sc.Drifted > 0 {
