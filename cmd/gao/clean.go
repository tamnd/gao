...
func runClean(stdout, stderr io.Writer, args []string) int {
	...
	keys := fs.Int("keys", clean.DefaultKeys, "how many documents the deduplication set is sized for")
...
`)
	...
	raw, cleaned := store.Staging(), clean.Clean()
	token := fleet.Token()
	if token == "" {
	...
	from := &count.Store{Repo: raw.Repo(), Token: token}
	to := &store.Pusher{Repo: cleaned.Repo(), Token: token}
	...
	fmt.Fprintf(stdout, "%s -> %s\n", raw.Repo(), cleaned.Repo())
	fmt.Fprintf(stdout, "%d parts%s, %s to read, %d workers, dedup set %s for %s documents\n",
		len(parts), ofSource(*source), fleet.GB(bytes), *workers,
		fleet.Size(clean.NewSeen(*keys).Bytes()), thousands(int64(*keys)))
	if *plan {
		return 0
	}
	...
	if *push {
		if err := to.EnsureRepo(ctx, cleaned); err != nil {
			fmt.Fprintf(stderr, "gao clean: %v\n", err)
			return 1
		}
	}
	...
	pass := &clean.Pass{
		From: from, To: to, Clean: cleaned,
		Dir: *dir, Box: fleet.Label(), Workers: *workers, Keys: *keys, Push: *push,
	}

	tw := tabwriter.NewWriter(stdout, 0, 0, 2, ' ', 0)
	note := func(c clean.Cleaned, done, of int) {
		if c.Skipped {
			fmt.Fprintf(tw, "%d/%d\t%s\thad it already\n", done, of, c.From.Path)
		} else {
			fmt.Fprintf(tw, "%d/%d\t%s\t%s kept of %s\t%s\t%s\n", done, of, c.From.Path,
				thousands(c.Kept), thousands(c.Documents), fleet.Size(c.Bytes), round(c.Took))
		}
		_ = tw.Flush()
	}
...
func rawParts(ctx context.Context, s *count.Store, source string) ([]store.Stored, error) {
	...
	for _, snapshot := range snapshots {
		if len(want) > 0 && !want[count.SourceOf(snapshot)] {
			continue
		}
...
func writeReport(path string, rep clean.Report) error {
...
func printClean(w io.Writer, r clean.Report) {
	tw := tabwriter.NewWriter(w, 0, 0, 2, ' ', 0)
	fmt.Fprintf(tw, "\nread\t%s documents\t%s of text\t%d parts\n",
		thousands(r.Documents), fleet.GB(r.TextIn), r.Parts)
	fmt.Fprintf(tw, "kept\t%s documents\t%s of text\t%s of parquet\n",
		thousands(r.Kept), fleet.GB(r.TextOut), fleet.GB(r.Bytes))
	if r.Skipped > 0 {
	...
	tw = tabwriter.NewWriter(w, 0, 0, 2, ' ', 0)
	for _, stage := range []clean.Stage{clean.StageNormalize, clean.StageSift, clean.StageMill, clean.StageContract} {
	...
	fmt.Fprintf(w, "Normalization changed %.1f%% of them by at least one byte and repaired %.1f%%.\n",
		100*r.Normalize.ChangedShare(), 100*r.Normalize.RepairedShare())
	fmt.Fprintf(w, "%s documents carried personal data, %s spans of it, and %s were covered at %s.\n",
		thousands(r.Cover.Carrying), thousands(r.Cover.Spans), thousands(r.Cover.Covered), che1)
	fmt.Fprintf(w, "The deduplication set finished with %s of %s clusters.\n",
		thousands(int64(r.Clusters)), thousands(int64(cleanCap(r))))
	if r.Unchecked > 0 {
...
func cleanCap(r clean.Report) int {
	if r.Clusters == 0 {
		return 0
	}
...
func byReason(m map[reject.Reason]int64) []reasonLine {
