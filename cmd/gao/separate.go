...
import (
	...
	"github.com/tamnd/gao/separate"
)

func runSeparate(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	o := separate.Options{MinPosts: *minPosts, MinChars: *minChars}
	report := separateReport{Pages: make([]separateLine, 0, len(pages))}
	for _, name := range pages {
		...
		var thread *separate.Thread
		if t, ok := separate.Forum(page, o); ok {
			thread = t
		}
		report.Run.Add(thread)
		report.Pages = append(report.Pages, separateLine{Page: name, Thread: thread})
	}
	...
	if *asJSON {
		...
		printSeparateText(stdout, report)
	} else {
		printSeparate(stdout, report)
	}
...
// separateLine is one page and what it turned out to be. A page that was not a
...
type separateLine struct {
	Page   string           `json:"page"`
	Thread *separate.Thread `json:"thread"`
}

type separateReport struct {
	Pages []separateLine  `json:"pages"`
	Run   separate.Report `json:"run"`
}

func printSeparate(w io.Writer, report separateReport) {
...
func printSeparateText(w io.Writer, report separateReport) {
	for _, l := range report.Pages {
		if l.Thread == nil {
			continue
		}
