...
func runPick(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	x, err := pick.NewIndex(l)
	if err != nil {
		fmt.Fprintf(stderr, "gao pick: %v\n", err)
		return 1
	}

	tally := pick.NewTally(x)
	var flagged []pickDocument
	for _, name := range files {
		...
		if err := eachDocument(name, func(text string) error {
			...
			if r.Flagged() && len(flagged) < *show {
				flagged = append(flagged, pickDocument{Document: row(name, i), Result: r})
			}
			i++
			return nil
		}); err != nil {
	...
	run := pickRun{Roster: ros.Version, List: l.Version, Grams: x.Grams(), Tally: tally, Flagged: flagged}
	if *asJSON {
		return printJSON(stdout, stderr, run)
	}
	printPick(stdout, run)
	return 0
}
...
func readRoster(path string) (pick.Roster, error) {
	if path == "" {
		return pick.Rostered()
	}
	return pick.ReadRoster(path)
}
...
type pickDocument struct {
	...
	Result   pick.Result `json:"result"`
}
...
type pickRun struct {
	...
	Tally   *pick.Tally    `json:"tally"`
	Flagged []pickDocument `json:"flagged,omitempty"`
}

func printRoster(w io.Writer, ros pick.Roster) {
	tw := tabwriter.NewWriter(w, 0, 0, 2, ' ', 0)
	fmt.Fprint(tw, "benchmark\torigin\trevision\thome\tdrops at\tsource\n")
	for _, e := range ros.Benchmarks {
		held := fmt.Sprintf("%d windows", pick.DropAt)
		if e.HeldOut {
...
func printPick(w io.Writer, run pickRun) {
	t := run.Tally
	tw := tabwriter.NewWriter(w, 0, 0, 2, ' ', 0)
	...
	for _, d := range run.Flagged {
		...
		for _, b := range d.Result.Benchmarks {
			...
			fmt.Fprintf(w, "  %-16s %s from %s, %s\n",
				b.Benchmark, plural(b.Grams, "window"), plural(b.Items, "item"), verdict)
		}
	}
}
