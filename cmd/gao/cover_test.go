...
const (
	coverAdvert = "Bán căn hộ hai phòng ngủ tại Số 25 đường Nguyễn Huệ, phường Bến Nghé, quận 1, TP Hồ Chí Minh.\n" +
		"Liên hệ chính chủ anh Nguyễn Văn Minh, điện thoại 0912 345 678, hoặc email minh.nguyen@gmail.com."

	coverArticle = "Chủ tịch Hồ Chí Minh đọc bản Tuyên ngôn độc lập ngày 2 tháng 9 năm 1945 tại quảng trường Ba Đình.\n" +
...
func TestCoverCoversIdentifiersByDefault(t *testing.T) {
	dir := t.TempDir()
	path := writeText(t, dir, "advert.txt", coverAdvert)

	var stdout, stderr bytes.Buffer
	if code := runCover(&stdout, &stderr, []string{path}); code != 0 {
...
func TestCoverAtLevelTwoCoversTheNameAndTheAddress(t *testing.T) {
	dir := t.TempDir()
	path := writeText(t, dir, "advert.txt", coverAdvert)

	var stdout, stderr bytes.Buffer
	if code := runCover(&stdout, &stderr, []string{"-level", "L2", path}); code != 0 {
...
func TestCoverLeavesANewsStoryAlone(t *testing.T) {
	dir := t.TempDir()
	path := writeText(t, dir, "article.txt", coverArticle)

	var stdout, stderr bytes.Buffer
	if code := runCover(&stdout, &stderr, []string{"-level", "L2", path}); code != 0 {
		t.Fatalf("gao cover = %d, want 0\n%s", code, stderr.String())
	}
	if stdout.String() != coverArticle {
...
func TestCoverReportsWhatEachLevelWouldCover(t *testing.T) {
	dir := t.TempDir()
	files := []string{
		writeText(t, dir, "advert.txt", coverAdvert),
		writeText(t, dir, "article.txt", coverArticle),
	}
	...
	for _, c := range []struct {
		...
		if code := runCover(&stdout, &stderr, args); code != 0 {
...
func TestCoverCountsTheSameSpansAtEveryLevel(t *testing.T) {
	dir := t.TempDir()
	path := writeText(t, dir, "advert.txt", coverAdvert)

	var stdout, stderr bytes.Buffer
	if code := runCover(&stdout, &stderr, []string{"-report", "-json", "-level", "L0", path}); code != 0 {
	...
	for _, k := range []cover.Kind{cover.KindPhone, cover.KindEmail, cover.KindName, cover.KindAddress} {
...
func TestCoverListsTheMatchesWhenAsked(t *testing.T) {
	dir := t.TempDir()
	path := writeText(t, dir, "advert.txt", coverAdvert)

	var stdout, stderr bytes.Buffer
	if code := runCover(&stdout, &stderr, []string{"-report", "-spans", "-level", "L2", path}); code != 0 {
...
func TestCoverSaysWhatTheLevelDid(t *testing.T) {
	dir := t.TempDir()
	path := writeText(t, dir, "advert.txt", coverAdvert)

	var stdout, stderr bytes.Buffer
	if code := runCover(&stdout, &stderr, []string{"-report", path}); code != 0 {
...
func TestCoverRefusesALevelItDoesNotHave(t *testing.T) {
	var stdout, stderr bytes.Buffer
	if code := runCover(&stdout, &stderr, []string{"-level", "L3"}); code != 2 {
...
func TestCoverRefusesReportFlagsWithoutTheReport(t *testing.T) {
	for _, flag := range []string{"-json", "-spans"} {
		var stdout, stderr bytes.Buffer
		if code := runCover(&stdout, &stderr, []string{flag}); code != 2 {
...
func TestCoverMeasuresItselfAgainstTheLabeledSet(t *testing.T) {
	var stdout, stderr bytes.Buffer
	if code := runCover(&stdout, &stderr, []string{"-recall"}); code != 0 {
...
func TestCoverReportsTheRecallAsJSON(t *testing.T) {
	var stdout, stderr bytes.Buffer
	if code := runCover(&stdout, &stderr, []string{"-recall", "-json"}); code != 0 {
...
func TestCoverRecallTakesNoFiles(t *testing.T) {
	dir := t.TempDir()
	path := writeText(t, dir, "advert.txt", coverAdvert)

	for _, args := range [][]string{{"-recall", path}, {"-recall", "-report"}, {"-recall", "-spans"}} {
		var stdout, stderr bytes.Buffer
		if code := runCover(&stdout, &stderr, args); code != 2 {
...
func TestCoverRefusesToCoverAPartOntoOneStream(t *testing.T) {
	dir := t.TempDir()
	path := writeText(t, dir, "part.parquet", "")

	var stdout, stderr bytes.Buffer
	if code := runCover(&stdout, &stderr, []string{path}); code != 2 {
