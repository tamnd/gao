...
import (
	...
	"github.com/tamnd/gao/listen"
)

func runListen(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	s, err := listen.ReadSet(*name, fs.Arg(0))
	if err != nil {
	...
	report := listenReport{
		...
		MaxLost:    listen.MaxLost, MaxRepeat: listen.MaxRepeat, MinVariety: listen.MinVariety,
		MinRate: listen.MinRate, MaxRate: listen.MaxRate,
	...
	for _, t := range s.Ranked() {
		report.Readings = append(report.Readings, listenReading{
...
// listenReading is one track as the table carries it.
type listenReading struct {
...
type listenReport struct {
	...
	Readings []listenReading `json:"readings"`
...
func printListen(w io.Writer, s listen.Set, claims []string) {
	...
	fmt.Fprintf(w, "A track is dropped when one line runs %d times back to back, or under %s of its lines are distinct, or the words do not fit the speech at %.1f to %.1f syllables a second.\n",
		listen.MaxRepeat, percent(listen.MinVariety), listen.MinRate, listen.MaxRate)
	...
	if beaten := s.Superseded(); len(beaten) > 0 {
		fmt.Fprintf(w, "%s superseded by subtitles a person wrote for the same recording, which is not hours lost.\n",
			plural(len(beaten), "machine transcript"))
	}
...
func outcome(t listen.Track, superseded bool) string {
	switch {
	case superseded:
		return "written"
	case t.Looped():
