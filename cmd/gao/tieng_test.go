...
// tiengPool is marked Vietnamese, most of it function words. The carrier walks
...
var tiengPool = strings.Fields(`của là có và với một này đã để các
...
// tiengSample writes docs documents, each holding the phrase each times behind
// a carrier, and hands back the paths.
func tiengSample(t *testing.T, docs, each int, phrase string) []string {
	t.Helper()
	...
	for d := range docs {
		...
		for i := range each {
			...
			for k := range 6 {
				line = append(line, tiengPool[((d*each+i)*7+k)%len(tiengPool)])
			}
...
func TestTiengPrintsWhatTheRuleGovernsAndWhatItGivesUp(t *testing.T) {
	args := append([]string{"tieng", "-source", "a sample of gao"}, tiengSample(t, 4, 60, "việt nam")...)
...
// four documents.
func TestTiengExitsTwoWhenTheSampleCannotAnswerTheQuestion(t *testing.T) {
	args := append([]string{"tieng", "-source", "a sample of gao"}, tiengSample(t, 2, 4, "việt nam")...)
...
func TestTiengExitsOneWhenThereIsNoReadingToTake(t *testing.T) {
	args := append([]string{"tieng"}, tiengSample(t, 1, 60, "việt nam")...)
...
func TestTiengPrintsJSON(t *testing.T) {
	args := append([]string{"tieng", "-source", "a sample of gao", "-json"}, tiengSample(t, 4, 60, "việt nam")...)
...
func TestTiengPrintsAsManyRunsAsItWasAsked(t *testing.T) {
	args := append([]string{"tieng", "-source", "a sample of gao", "-top", "1"}, tiengSample(t, 4, 60, "việt nam")...)
...
func TestTiengRefusesTheUsageErrors(t *testing.T) {
	for _, c := range []struct {
		name string
		args []string
	}{
		{"no documents", []string{"tieng", "-source", "a sample"}},
		{"a table of no rows at all", []string{"tieng", "-source", "a sample", "-top", "-1", "x.txt"}},
		{"a flag nobody has", []string{"tieng", "-nope"}},
		{"the help", []string{"tieng", "-h"}},
	} {
...
func TestTiengRefusesADocumentThatIsNotThere(t *testing.T) {
	out, errOut, code := exec(t, "tieng", "-source", "a sample", filepath.Join(t.TempDir(), "khong-co.txt"))
...
func TestTiengIsInTheCommandList(t *testing.T) {
	out, _, _ := exec(t, "help")
