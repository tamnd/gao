...
func runPlace(stdout, stderr io.Writer, args []string) int {
	if len(args) == 0 {
		placeUsage(stderr)
		return 2
	}
	switch args[0] {
	case "agree":
		return runPlaceAgree(stdout, stderr, args[1:])
	case "frame":
		return runPlaceFrame(stdout, stderr, args[1:])
	case "read":
		return runPlaceRead(stdout, stderr, args[1:])
	case "help", "-h", "--help":
		placeUsage(stdout)
		return 0
	default:
...
func runPlaceFrame(stdout, stderr io.Writer, args []string) int {
	...
	f, code := readPlaceFrame(stderr, *path)
	if code != 0 {
		return code
	}
	faults := f.Faults()
	...
	switch {
	case *asJSON:
		if code := printJSON(stdout, stderr, placeFrameReport{Frame: f, Digest: f.Digest().String(), Faults: faults}); code != 0 {
			return code
		}
	case *rubric:
		printPlaceRubric(stdout, f)
	default:
		printPlaceFrame(stdout, f)
	}
...
func runPlaceRead(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	labels, err := place.ReadLabels(fs.Arg(0))
	if err != nil {
	...
	score := f.Read(labels)
	out := placeReadReport{Score: score, Faults: append(f.Faults(), score.Publishable()...)}
	if *asJSON {
		if code := printJSON(stdout, stderr, out); code != 0 {
			return code
		}
	} else {
		printPlaceScore(stdout, f, out)
	}
...
type placeFrameReport struct {
	Frame  place.Frame `json:"frame"`
	Digest string      `json:"digest"`
	Faults []string    `json:"faults,omitempty"`
}

type placeReadReport struct {
	place.Score
	Faults []string `json:"faults,omitempty"`
}

func readPlaceFrame(stderr io.Writer, path string) (place.Frame, int) {
	if path == "" {
		return place.Fixed(), 0
	}
	f, err := place.ReadFrame(path)
	if err != nil {
		fmt.Fprintf(stderr, "gao place: %v\n", err)
		return place.Frame{}, 1
	}
	return f, 0
}

func printPlaceFrame(w io.Writer, f place.Frame) {
	fmt.Fprintf(w, "%s\n\nthe draw:\n", f.Describe())
...
func printPlaceRubric(w io.Writer, f place.Frame) {
	fmt.Fprintf(w, "%s\n", f.Describe())
	for _, b := range place.Bands {
		r, ok := f.Rule(b)
		if !ok {
...
func printPlaceScore(w io.Writer, f place.Frame, out placeReadReport) {
	sc := out.Score
	fmt.Fprintf(w, "%s\n\n", f.Describe())
	...
	if sc.Wanted < place.Documents {
		fmt.Fprintf(w, "This frame draws %d and %s is %d, so this is a pilot: it says whether the rubric decides anything and it is not the reference set.\n",
			sc.Wanted, place.Repo, place.Documents)
	}
...
func runPlaceAgree(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	a := f.Agree(labels)
	out := placeAgreeReport{Agreement: a, Verdict: a.Verdict(), Faults: a.Blocking()}
	if *asJSON {
		if code := printJSON(stdout, stderr, out); code != 0 {
			return code
		}
	} else {
		printPlaceAgree(stdout, f, out)
	}
	if !a.Passed() {
		return 1
	}
...
type placeAgreeReport struct {
	place.Agreement
	Verdict string   `json:"verdict"`
	Faults  []string `json:"faults,omitempty"`
}

func printPlaceAgree(w io.Writer, f place.Frame, out placeAgreeReport) {
	a := out.Agreement
	fmt.Fprintf(w, "%s\n\n", f.Describe())
