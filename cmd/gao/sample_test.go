...
// sampleSource writes the two files the plan is drawn from: the layers, five of
// them read at 40 MB each, and a listing that cuts every layer into shards of
// about a gigabyte.
func sampleSource(t *testing.T, shard int64) (layers, files string) {
	t.Helper()
...
func TestSamplePlansTheReadingTheEstimateHasBeenWaitingOn(t *testing.T) {
	layers, files := sampleSource(t, 900_000_000)

	out, errOut, code := exec(t, "sample", "-source", "hplt-v3", "-seed", "hplt-v3-2026-08", "-layers", layers, files)
	if code != 0 {
...
// it.
func TestSamplePrintsTheReadListOnItsOwn(t *testing.T) {
	layers, files := sampleSource(t, 900_000_000)

	out, _, code := exec(t, "sample", "-source", "hplt-v3", "-seed", "s", "-layers", layers, "-takes", files)
	if code != 0 {
...
func TestSampleDrawsTheSameShardsForTheSameSeedAndOthersForAnother(t *testing.T) {
	layers, files := sampleSource(t, 900_000_000)

	first, _, _ := exec(t, "sample", "-source", "hplt-v3", "-seed", "one", "-layers", layers, "-takes", files)
	again, _, _ := exec(t, "sample", "-source", "hplt-v3", "-seed", "one", "-layers", layers, "-takes", files)
	other, _, _ := exec(t, "sample", "-source", "hplt-v3", "-seed", "two", "-layers", layers, "-takes", files)
...
func TestSamplePrintsTheSamePlanAsJSON(t *testing.T) {
	layers, files := sampleSource(t, 900_000_000)

	out, _, code := exec(t, "sample", "-source", "hplt-v3", "-seed", "s", "-layers", layers, "-json", files)
	if code != 0 {
		t.Fatalf("exit %d, want 0\n%s", code, out)
	}
...
func TestSampleSaysWhenALayerCannotBeSpreadAcrossEnoughShards(t *testing.T) {
	layers, files := sampleSource(t, 12_000_000_000)

	out, _, code := exec(t, "sample", "-source", "hplt-v3", "-seed", "s", "-layers", layers, files)
	if code != 2 {
...
func TestSampleRefusesAPlanNobodyCanDrawAgain(t *testing.T) {
	layers, files := sampleSource(t, 900_000_000)

	out, _, code := exec(t, "sample", "-source", "hplt-v3", "-layers", layers, files)
	if code != 1 {
...
func TestSampleSaysWhichLineOfTheListingIsWrong(t *testing.T) {
	layers, files := sampleSource(t, 900_000_000)
	...
	_, errOut, code := exec(t, "sample", "-source", "hplt-v3", "-seed", "s", "-layers", layers, files)
	if code != 1 {
...
func TestSampleUsageErrors(t *testing.T) {
	...
	if _, _, code := exec(t, "sample", "-source", "hplt-v3", "-seed", "s", files); code != 2 {
		t.Errorf("no layer file: exit %d, want 2", code)
	}
	if _, _, code := exec(t, "sample", "-source", "hplt-v3", "-seed", "s", "-layers", layers); code != 2 {
		t.Errorf("no listing: exit %d, want 2", code)
	}

	_, errOut, code := exec(t, "sample", "-h")
	if code != 2 {
...
func TestSampleIsInTheCommandList(t *testing.T) {
