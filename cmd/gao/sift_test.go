...
const (
	siftArticle = "Bộ Giao thông vận tải vừa trình phương án nâng cấp tuyến quốc lộ chạy qua bốn tỉnh miền Trung.\n" +
	...
	siftCaption = "Người dân đi lại trên phố Hàng Bài chiều 12 tháng 8. Ảnh: Ngọc Thành."
	...
	siftNotice = "Cửa hàng của chúng tôi mở cửa từ bảy giờ sáng và đóng cửa lúc chín giờ tối, kể cả ngày lễ và chủ nhật."
	...
	siftEnglish = "The committee met for the third time this year to review the proposal, and after a long " +
...
func TestSiftKeepsTheArticleAndSaysWhyItDroppedTheRest(t *testing.T) {
	dir := t.TempDir()
	files := []string{
		writeText(t, dir, "article.txt", siftArticle),
		writeText(t, dir, "caption.txt", siftCaption),
		writeText(t, dir, "menu.txt", siftMenu),
	}

	var stdout, stderr bytes.Buffer
	if code := runSift(&stdout, &stderr, append([]string{"-json"}, files...)); code != 0 {
...
func TestSiftSaysWhichNumberDroppedEachDocument(t *testing.T) {
	dir := t.TempDir()
	files := []string{
		writeText(t, dir, "article.txt", siftArticle),
		writeText(t, dir, "caption.txt", siftCaption),
...
func TestSiftPutsTheMeasurementsOnTheRow(t *testing.T) {
	path := writeText(t, t.TempDir(), "article.txt", siftArticle)

	var stdout, stderr bytes.Buffer
	if code := runSift(&stdout, &stderr, []string{"-json", path}); code != 0 {
		t.Fatalf("gao sift = %d, want 0\n%s", code, stderr.String())
	}
	var got siftReport
	if err := json.Unmarshal(stdout.Bytes(), &got); err != nil {
...
func TestSiftFilesAnotherLanguageUnderLanguage(t *testing.T) {
	path := writeText(t, t.TempDir(), "english.txt", siftEnglish)
...
// what it let through.
func TestSiftPutsTheLanguageSharesOnTheRow(t *testing.T) {
	dir := t.TempDir()
	files := []string{
		writeText(t, dir, "article.txt", siftArticle),
		writeText(t, dir, "english.txt", siftEnglish),
	}
...
func TestSiftReadsAParquetPart(t *testing.T) {
	...
	rows := []string{siftArticle, siftCaption}
	...
	var stdout, stderr bytes.Buffer
	if code := runSift(&stdout, &stderr, []string{"-json", filepath.Join(dir, file.Path)}); code != 0 {
...
func TestSiftPrintsTheMeasurementsAsATable(t *testing.T) {
	dir := t.TempDir()
	files := []string{
		writeText(t, dir, "article.txt", siftArticle),
		writeText(t, dir, "menu.txt", siftMenu),
	...
	var stdout, stderr bytes.Buffer
	if code := runSift(&stdout, &stderr, files); code != 0 {
		t.Fatalf("gao sift = %d, want 0\n%s", code, stderr.String())
	}
	out := stdout.String()
...
func TestSiftLetsTheLengthFloorBeMoved(t *testing.T) {
	path := writeText(t, t.TempDir(), "notice.txt", siftNotice)

	var stdout, stderr bytes.Buffer
	if code := runSift(&stdout, &stderr, []string{"-json", path}); code != 0 {
		t.Fatalf("gao sift = %d, want 0\n%s", code, stderr.String())
	}
	var atDefault siftReport
	...
	stdout.Reset()
	stderr.Reset()
	if code := runSift(&stdout, &stderr, []string{"-min-syllables=5", "-json", path}); code != 0 {
		t.Fatalf("gao sift -min-syllables=5 = %d, want 0\n%s", code, stderr.String())
	}
	var lowered siftReport
...
func TestSiftRefusesALengthFloorThatIsNotALength(t *testing.T) {
	path := writeText(t, t.TempDir(), "article.txt", siftArticle)

	var stdout, stderr bytes.Buffer
	if code := runSift(&stdout, &stderr, []string{"-min-syllables=-1", path}); code != 2 {
...
func TestSiftSaysWhenItWasGivenNothingToRead(t *testing.T) {
	var stdout, stderr bytes.Buffer
	if code := runSift(&stdout, &stderr, nil); code != 2 {
...
func TestSiftSaysWhichFileItCouldNotRead(t *testing.T) {
	var stdout, stderr bytes.Buffer
	if code := runSift(&stdout, &stderr, []string{filepath.Join(t.TempDir(), "gone.txt")}); code != 1 {
