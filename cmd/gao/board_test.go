...
// boardScores writes a scores file and returns the path to it.
func boardScores(t *testing.T, lines ...string) string {
	t.Helper()
...
// boardSuite scores every benchmark on the repository roster, a fixed margin for
// the ones written in Vietnamese and another for the ones translated into it.
func boardSuite(t *testing.T, native, translated float64) string {
	t.Helper()
	...
	return boardScores(t, lines...)
}

func TestBoardKeepsTheTwoArmsApart(t *testing.T) {
	...
	out, _, code := exec(t, "board", "board", boardSuite(t, 3.0, 3.0))
	if code != 2 {
...
func TestBoardNamesTheModelThatOnlyReadsTranslatedEnglish(t *testing.T) {
	out, _, code := exec(t, "board", "board", boardSuite(t, 0.5, 6.0))
	if code != 2 {
...
func TestBoardRowsMarksTheBenchmarksGaoBuiltItself(t *testing.T) {
	out, _, code := exec(t, "board", "rows", boardSuite(t, 3.0, 3.0))
	if code != 2 {
...
func TestBoardPrintsTheSameBoardAsJSON(t *testing.T) {
	out, _, code := exec(t, "board", "board", "-json", boardSuite(t, 3.0, 3.0))
	if code != 2 {
	...
	if got.Arms[0].Origin != pick.Native || got.Arms[0].Benchmarks != 16 {
...
func TestBoardHoldsWhenEveryRowCanBeRunAgain(t *testing.T) {
	...
	for i, e := range []struct {
		...
		{"vmlu", pick.Native}, {"vimmrc", pick.Native}, {"uit-vsfc", pick.Native},
		{"visfd", pick.Native}, {"vihsd", pick.Native}, {"victsd", pick.Native},
		{"mmlu-vi", pick.Translated}, {"arc-vi", pick.Translated},
	} {
		ros.Benchmarks = append(ros.Benchmarks, pick.Entry{
			Name:    e.name,
	...
	out, errOut, code := exec(t, "board", "board", "-roster", path, boardScores(t, lines...))
	if code != 0 {
...
func TestBoardRefusesScoresThatAreNotAScoreboard(t *testing.T) {
	path := boardScores(t,
	...
	out, _, code := exec(t, "board", "board", path)
	if code != 1 {
...
func TestBoardSaysWhichLineOfTheScoresIsWrong(t *testing.T) {
	path := boardScores(t,
		`{"benchmark":"vmlu","score":58.2,"baseline":54.1,"against":"sailor2-8b","runs":3,"spread":0.4,"box":"gamingpc","measured_on":"2026-08-12"}`,
		`{"benchmark":"vimmrc","score":61.4,"baseline":58.9,"against":"sailor2-8b","runs":3,"f1":0.81}`,
	)

	_, errOut, code := exec(t, "board", "board", path)
	if code != 1 {
...
func TestBoardWithoutASubcommandSaysWhatItTakes(t *testing.T) {
	for _, args := range [][]string{
		{"board"},
		{"board", "publish"},
		{"board", "board"},
		{"board", "board", "one.jsonl", "two.jsonl"},
	} {
...
func TestBoardHelpSaysWhyTheArmsAreNotAdded(t *testing.T) {
	out, _, code := exec(t, "board", "help")
	if code != 0 {
