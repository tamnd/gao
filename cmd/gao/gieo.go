...
func runGieo(stdout, stderr io.Writer, args []string) int {
	if len(args) == 0 {
		gieoUsage(stderr)
		return 2
	}
	switch args[0] {
	case "recipe":
		return runGieoRecipe(stdout, stderr, args[1:])
	case "card":
		return runGieoCard(stdout, stderr, args[1:])
	case "help", "-h", "--help":
		gieoUsage(stdout)
		return 0
	default:
...
func runGieoRecipe(stdout, stderr io.Writer, args []string) int {
	...
	r, code := readGieoRecipe(stderr, *path)
	if code != 0 {
		return code
	}

	if *asJSON {
		return printJSON(stdout, stderr, gieoRecipeReport{Recipe: r, Digest: r.Digest().String(), Faults: gieoFaults(r)})
	}
	if *prompts {
		printGieoPrompts(stdout, r)
		return 0
	}
	printGieoRecipe(stdout, r)
	if faults := gieoFaults(r); len(faults) > 0 {
...
type gieoRecipeReport struct {
	Recipe gieo.Recipe `json:"recipe"`
	Digest string      `json:"digest"`
	Faults []string    `json:"faults,omitempty"`
}

type gieoCardReport struct {
	Card   gieo.Card `json:"card"`
	Repo   string    `json:"repo"`
	Faults []string  `json:"faults,omitempty"`
}

func readGieoRecipe(stderr io.Writer, path string) (gieo.Recipe, int) {
	if path == "" {
		return gieo.Fixed(), 0
	}
...
// gieoFaults reports a recipe's own problems as lines, since the package returns
// them joined into one error and a report wants them one to a row.
func gieoFaults(r gieo.Recipe) []string {
	c := gieo.Card{Recipe: r.Digest(), Version: r.Version, Box: ".", Batch: ".",
...
func printGieoRecipe(w io.Writer, r gieo.Recipe) {
	fmt.Fprintf(w, "%s\n\n", r.Describe())
...
func printGieoPrompts(w io.Writer, r gieo.Recipe) {
...
func printGieoCard(w io.Writer, r gieo.Recipe, report gieoCardReport) {
	c := report.Card
	tw := tabwriter.NewWriter(w, 0, 0, 2, ' ', 0)
