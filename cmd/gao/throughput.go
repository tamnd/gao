...
import (
	...
	"github.com/tamnd/gao/throughput"
)

func runThroughput(stdout, stderr io.Writer, args []string) int {
	fs := flag.NewFlagSet("throughput", flag.ContinueOnError)
	fs.SetOutput(stderr)
	asJSON := fs.Bool("json", false, "print JSON")
	docs := fs.Int64("docs", throughput.Corpus, "how many documents the pipeline is costed over, which every hours figure is linear in")
...
`)
	...
	p, err := throughput.ReadPipeline(*docs, *counted, fs.Arg(0))
	if err != nil {
	...
	report := throughputReport{
		...
		Hours: p.Hours(), Ceiling: throughput.Ceiling,
	...
	for _, s := range p.Ranked() {
		report.Rates = append(report.Rates, throughputRate{
	...
	if *asJSON {
		...
		printThroughput(stdout, p, claims)
	}
...
// throughputRate is one stage as the table carries it, which is rates and what a
...
type throughputReport struct {
	...
	Rates  []throughputRate `json:"rates"`
...
func printThroughput(w io.Writer, p throughput.Pipeline, claims []string) {
	...
	fmt.Fprintf(w, "The memory line is %s per worker, because server3 has eight cores and %s and wants all eight busy.\n",
		gigabytes(throughput.Ceiling), gigabytes(server3Memory()))
...
func server3Memory() int64 {
	if b, ok := fleet.Lookup("server3"); ok {
		return b.Memory
	}
	return 0
}
