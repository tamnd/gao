...
func congFile(t *testing.T, lines ...string) string {
	t.Helper()
...
func TestCongPublishesTheNaturalNumberAndNothingElse(t *testing.T) {
	path := congFile(t,
	...
	out, errOut, code := exec(t, "cong", path)
	if code != 0 {
...
func TestCongStatesWhatShipsApartFromWhatWasCounted(t *testing.T) {
	path := congFile(t,
...
func TestCongRefusesTwoTokenizersRatherThanAddingThem(t *testing.T) {
	other := strings.Replace(countLine("hplt-v3", "open", "natural", 110_000_000_000), "gao-64k", "gao-32k", 1)
	path := congFile(t, countLine("gao-crawl", "open", "natural", 200_000_000_000), other)
	out, _, code := exec(t, "cong", path)
	if code != 1 {
...
func TestCongExitsTwoOnTheKillCriterion(t *testing.T) {
	path := congFile(t, countLine("gao-crawl", "open", "natural", 180_000_000_000))
	out, _, code := exec(t, "cong", path)
	if code != 2 {
...
func TestCongShortOfTheTargetIsNotTheSameAsDead(t *testing.T) {
	path := congFile(t, countLine("gao-crawl", "open", "natural", 270_000_000_000))
	out, _, code := exec(t, "cong", path)
	if code != 0 {
...
func TestCongJSONCarriesTheThreeTotalsSeparately(t *testing.T) {
	path := congFile(t,
		countLine("gao-crawl", "open", "natural", 260_000_000_000),
		countLine("phap-luat", "restricted", "natural", 30_000_000_000),
		countLine("gao-synth", "open", "synthetic", 40_000_000_000),
	)
	out, _, code := exec(t, "cong", "-json", path)
	if code != 0 {
		t.Fatalf("exit %d\n%s", code, out)
	}
...
func TestCongWithoutAFileIsAUsageError(t *testing.T) {
	if _, _, code := exec(t, "cong"); code != 2 {
		t.Errorf("cong with no counts file exited %d", code)
	}
	if _, _, code := exec(t, "cong", filepath.Join(t.TempDir(), "nothing.jsonl")); code != 1 {
