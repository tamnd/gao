...
import (
	...
	"github.com/tamnd/gao/attach"
)

func runAttach(stdout, stderr io.Writer, args []string) int {
...
`)
	...
	b, err := attach.ReadBatch(*name, fs.Arg(0))
	if err != nil {
	...
	report := attachReport{
		...
		Paired:      b.Paired(),
		Attached:    b.Attached(),
		MinAttached: attach.MinAttached,
		Blank:       b.Blank(),
		Lost:        b.Lost(),
		Dropped:     b.Dropped(),
		MaxLost:     attach.MaxLost,
		Routes:      b.Routes(),
		Bytes:       b.Bytes(),
		Stored:      b.Stored(),
		Resident:    b.Resident(),
		Window:      attach.Free(*free),
		Fits:        b.Fits(*free),
		Gaps:        b.Gaps(),
		Holds:       b.Holds(*free),
		Blocking:    b.Blocking(),
...
type attachReport struct {
	...
	Routes []attach.Route `json:"routes"`
...
func printAttach(w io.Writer, r attachReport) {
