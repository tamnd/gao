...
func TestTheCriteriaArePrintedInOrder(t *testing.T) {
	out, errOut, code := exec(t, "chon", "criteria")
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestFertilityDecidesInsideTheBandAndNotOutsideIt(t *testing.T) {
	...
	out, _, code := exec(t, "chon", "score", baseLog(t, quality, fertility))
	if code != 0 {
		t.Fatalf("exit %d:\n%s", code, out)
	}
...
func TestTwoSuitesIsNotARanking(t *testing.T) {
	...
	out, _, code := exec(t, "chon", "score", path)
	if code != 1 {
		t.Fatalf("exit %d, want 1:\n%s", code, out)
	}
...
func TestTheTableIsAlsoMachineReadable(t *testing.T) {
	quality, fertility := wholeRoster()
	out, _, code := exec(t, "chon", "score", "-json", baseLog(t, quality, fertility))
	if code != 0 {
		t.Fatalf("exit %d:\n%s", code, out)
	}
	var report struct {
...
func TestAChoiceLogThatIsNotThereIsAFailure(t *testing.T) {
	_, errOut, code := exec(t, "chon", "score", filepath.Join(t.TempDir(), "nowhere.jsonl"))
	if code != 1 {
...
func TestChonRejectsWhatItDoesNotUnderstand(t *testing.T) {
	if _, _, code := exec(t, "chon"); code != 2 {
		t.Error("no subcommand did not read as a usage error")
	}
	if _, _, code := exec(t, "chon", "pick"); code != 2 {
		t.Error("a subcommand that does not exist did not read as a usage error")
	}
	if _, _, code := exec(t, "chon", "score"); code != 2 {
		t.Error("score with no log did not read as a usage error")
	}
	if out, _, code := exec(t, "chon", "help"); code != 0 || !strings.Contains(out, "in the order the criteria were written down") {
