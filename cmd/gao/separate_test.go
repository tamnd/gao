...
const separateThread = `<!doctype html>
...
const separateArticle = `<!doctype html>
...
func TestSeparateReadsAThreadAndSaysWhatItDropped(t *testing.T) {
	out, _, code := exec(t, "separate", writePage(t, "thread.html", separateThread))
	if code != 0 {
...
func TestSeparateSaysSoWhenThePageIsNotAThread(t *testing.T) {
	out, _, code := exec(t, "separate", writePage(t, "article.html", separateArticle))
	if code != 1 {
...
func TestSeparateCountsThePagesThatWereNotThreads(t *testing.T) {
	dir := t.TempDir()
	thread := filepath.Join(dir, "thread.html")
	article := filepath.Join(dir, "article.html")
	for path, page := range map[string]string{thread: separateThread, article: separateArticle} {
		if err := os.WriteFile(path, []byte(page), 0o600); err != nil {
			t.Fatal(err)
		}
	}

	out, _, code := exec(t, "separate", thread, article)
	if code != 0 {
		t.Fatalf("gao separate: exit %d, want 0\n%s", code, out)
	}
...
func TestSeparateTextPrintsTheThreadItself(t *testing.T) {
	out, _, code := exec(t, "separate", "-text", writePage(t, "thread.html", separateThread))
	if code != 0 {
...
func TestSeparateJSONCarriesThePostsAndTheRun(t *testing.T) {
	out, _, code := exec(t, "separate", "-json",
		writePage(t, "thread.html", separateThread), writePage(t, "article.html", separateArticle))
	if code != 0 {
		t.Fatalf("gao separate -json: exit %d, want 0\n%s", code, out)
	}

	var report separateReport
...
func TestSeparateFloorsAreReachableFromTheCommandLine(t *testing.T) {
	page := writePage(t, "thread.html", separateThread)
	if _, _, code := exec(t, "separate", "-min-posts", "9", page); code != 1 {
		t.Errorf("a thread of three posts cleared a floor of nine: exit %d", code)
	}
	if _, _, code := exec(t, "separate", "-min-chars", "4000", page); code != 1 {
		t.Errorf("ordinary posts cleared a four thousand character floor: exit %d", code)
	}
}

func TestSeparateWithoutPagesExplainsItself(t *testing.T) {
	_, stderr, code := exec(t, "separate")
	if code != 2 {
...
func TestSeparateOnAMissingFileFails(t *testing.T) {
	missing := filepath.Join(t.TempDir(), "khong-co.html")
	_, stderr, code := exec(t, "separate", missing)
	if code != 1 {
...
func TestSeparateRejectsAnUnknownFlag(t *testing.T) {
	if _, _, code := exec(t, "separate", "-nosuchflag"); code != 2 {
