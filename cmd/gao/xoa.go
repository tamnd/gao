...
func runXoa(stdout, stderr io.Writer, args []string) int {
	if len(args) == 0 {
		xoaUsage(stderr)
		return 2
	}
	switch args[0] {
	case "status":
		return runXoaStatus(stdout, stderr, args[1:])
	case "check":
		return runXoaCheck(stdout, stderr, args[1:])
	case "url":
		return runXoaURL(stdout, stderr, args[1:])
	case "help", "-h", "--help":
		xoaUsage(stdout)
		return 0
	default:
...
// runXoaStatus prints the record LIEN-HE.md says is kept in public.
//
...
// [xoa.ErrNothingFiled]: an unmeasured promise reads as unmeasured.
func took(d time.Duration, err error) string {
	if errors.Is(err, xoa.ErrNothingFiled) {
...
// runXoaCheck is here because the register is edited by hand by somebody who
