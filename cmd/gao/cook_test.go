...
func TestCookCheckIsTheOneThatCanFail(t *testing.T) {
	out, _, code := exec(t, "cook", "check")
	if code != 0 {
...
func TestCookBudgetPrintsEveryComponentAndWhatItAddsUpTo(t *testing.T) {
	out, _, code := exec(t, "cook", "budget")
	if code != 0 {
...
func TestCookBudgetDoesNotLeaveTheCorpusTargetToBeAddedUp(t *testing.T) {
	out, _, _ := exec(t, "cook", "budget")
...
func TestCookCurriculumPrintsEveryPhaseAndWhatItReads(t *testing.T) {
	out, _, code := exec(t, "cook", "curriculum")
	if code != 0 {
...
func TestCookReconcilePrintsBothSidesAndTheDifference(t *testing.T) {
	out, _, code := exec(t, "cook", "reconcile")
	if code != 0 {
...
func TestCookQuestionsPrintsWhatIsStillOpen(t *testing.T) {
	out, _, code := exec(t, "cook", "questions")
	if code != 0 {
...
func TestCookArmsPrintsTheRecipeOnceForAllThree(t *testing.T) {
	out, _, code := exec(t, "cook", "arms")
	if code != 0 {
...
func TestCookFleetSaysTheRunDoesNotGoOnTheBoxesWeOwn(t *testing.T) {
	out, _, code := exec(t, "cook", "fleet")
	if code != 0 {
...
func TestCookWithoutASubcommandSaysWhatThereIs(t *testing.T) {
	_, errOut, code := exec(t, "cook")
	if code != 2 {
...
func TestCookRefusesASubcommandItDoesNotHave(t *testing.T) {
	_, errOut, code := exec(t, "cook", "train")
	if code != 2 {
