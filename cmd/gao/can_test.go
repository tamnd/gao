...
// canRun is one arm the way one is meant to come back: the locked recipe, one
// checkpoint, one harness, one seed, and a curve that goes down.
func canRun(arm, data string, vmlu float64) can.Run {
	r := can.Run{
		Arm:        arm,
		Data:       data,
		Base:       "b3e0f1a9c2d4",
		Tokenizer:  "gemma-3",
		Tokens:     nau.Matched().Tokens,
		Batch:      nau.Matched().Batch,
		Sequence:   8192,
		PeakLR:     3e-5,
		Warmup:     0.01,
		Decay:      0.2,
		...
		Scores:     map[string]float64{can.Metric: vmlu, "vi-adherence": 0.86},
		BaseScores: map[string]float64{can.Metric: 44.1, "vi-adherence": 0.91},
	}
	loss := 2.31
	for step := range 40 {
		loss -= 0.004
		r.Curve = append(r.Curve, can.Point{Step: step * 500, Tokens: int64(step) * 5_000_000_000, Loss: loss})
	}
	return r
}

// canThree returns the three locked arms with the scores given, in the order nau
// locks them.
func canThree(gao, culturax, filtered float64) []can.Run {
	arms := nau.Arms()
	return []can.Run{
		canRun(arms[0].ID, arms[0].Data, gao),
		canRun(arms[1].ID, arms[1].Data, culturax),
		canRun(arms[2].ID, arms[2].Data, filtered),
	}
}

func canArms(t *testing.T, runs []can.Run) string {
	t.Helper()
...
func TestCanReportsTheGateOnThreeMatchedArms(t *testing.T) {
	out, errOut, code := exec(t, "can", canArms(t, canThree(52.4, 46.9, 48.2)))
	if code != 0 {
...
func TestCanReportsAdherenceWithoutGatingOnIt(t *testing.T) {
	out, _, code := exec(t, "can", canArms(t, canThree(52.4, 46.9, 48.2)))
	if code != 0 {
		t.Fatalf("exit %d\n%s", code, out)
	}
...
func TestCanExitsTwoWhenTheGapIsUnderE6(t *testing.T) {
	out, _, code := exec(t, "can", canArms(t, canThree(48.9, 46.9, 48.4)))
	if code != 2 {
...
func TestCanRefusesToQuoteTheGateOnAnUncontrolledComparison(t *testing.T) {
	runs := canThree(52.4, 46.9, 48.2)
	runs[1].Seed = 23
	out, _, code := exec(t, "can", canArms(t, runs))
	if code != 1 {
	...
	short := canThree(52.4, 46.9, 48.2)
	short[2].Tokens -= 20_000_000_000
	out, _, code = exec(t, "can", canArms(t, short))
	if code != 1 {
...
func TestCanRefusesAnArmScoredOffTheFleet(t *testing.T) {
	runs := canThree(52.4, 46.9, 48.2)
	...
	blank := canThree(52.4, 46.9, 48.2)
	blank[2].EvalBox = ""
	out, _, code = exec(t, "can", canArms(t, blank))
	if code != 1 {
...
func TestCanJSONCarriesTheGapAndWhatItRestsOn(t *testing.T) {
	out, _, code := exec(t, "can", "-json", canArms(t, canThree(52.4, 46.9, 48.2)))
	if code != 0 {
		t.Fatalf("exit %d\n%s", code, out)
	}
