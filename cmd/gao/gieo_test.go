...
import (
	...
	"github.com/tamnd/gao/gieo"
)

// gieoCard writes a card to a directory and returns it, so a test can bend one
// field and see what the checker says.
func gieoCard(t *testing.T, change func(*gieo.Card)) string {
	t.Helper()
	r := gieo.Fixed()
	c := gieo.Card{
		Recipe:    r.Digest(),
	...
	dir := t.TempDir()
	if err := gieo.WriteCard(dir, c); err != nil {
		t.Fatal(err)
	}
	return dir
}
...
func TestTheRecipePrintsWhatDecidesTheText(t *testing.T) {
	out, errOut, code := exec(t, "gieo", "recipe")
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestThePromptsComeOutVerbatim(t *testing.T) {
	out, _, code := exec(t, "gieo", "recipe", "-prompts")
	if code != 0 {
		t.Fatalf("exit %d", code)
	}
	want := gieo.Fixed().Styles[0].Prompt
	if !strings.Contains(out, want) {
...
func TestTheRecipeSpeaksJSON(t *testing.T) {
	...
	if report.Digest != gieo.Fixed().Digest().String() {
		t.Errorf("the JSON carries a different digest: %s", report.Digest)
	}
	if len(report.Recipe.Styles) != gieo.Styles || len(report.Faults) != 0 {
...
func TestAnHonestCardPasses(t *testing.T) {
	dir := gieoCard(t, nil)
	out, errOut, code := exec(t, "gieo", "card", dir)
	if code != 0 {
		t.Fatalf("exit %d: %s\n%s", code, out, errOut)
	}
...
func TestACardFromAnotherRecipeFails(t *testing.T) {
	dir := gieoCard(t, func(c *gieo.Card) { c.Recipe = gieo.Fixed().Digest(); c.Version = "0.9" })
	out, _, code := exec(t, "gieo", "card", dir)
	if code != 1 {
...
func TestACardWhereNothingWasRejectedFails(t *testing.T) {
	dir := gieoCard(t, func(c *gieo.Card) {
		c.Kept = c.Generated
...
func TestACardCarryingBenchmarkItemsIsNotPublishable(t *testing.T) {
	dir := gieoCard(t, func(c *gieo.Card) { c.Contaminated = 214 })
	out, _, code := exec(t, "gieo", "card", dir)
	if code != 1 {
		t.Fatalf("exit %d, want 1", code)
	}
...
func TestACardWhoseArithmeticDoesNotCloseFails(t *testing.T) {
	dir := gieoCard(t, func(c *gieo.Card) { c.Rejects["degenerate"] = 0 })
	out, _, code := exec(t, "gieo", "card", dir)
	if code != 1 {
		t.Fatalf("exit %d, want 1", code)
	}
...
func TestTheCardSpeaksJSON(t *testing.T) {
	dir := gieoCard(t, nil)
	out, _, code := exec(t, "gieo", "card", "-json", dir)
	if code != 0 {
		t.Fatalf("exit %d", code)
	}
	var report struct {
	...
	if report.Card.Box != "gamingpc" || report.Card.Kept != 812_400 || report.Repo != gieo.Repo {
...
func TestARecipeFromAFileCanBeUsed(t *testing.T) {
	...
	out, _, code := exec(t, "gieo", "recipe", "-recipe", path)
	if code != 0 {
		t.Fatalf("exit %d:\n%s", code, out)
	}
	...
	dir := gieoCard(t, nil)
	out, _, code = exec(t, "gieo", "card", "-recipe", path, dir)
	if code != 1 {
...
func TestACardThatIsNotThereSaysSo(t *testing.T) {
	_, errOut, code := exec(t, "gieo", "card", t.TempDir())
	if code != 1 {
		t.Errorf("exit %d, want 1", code)
	}
	if !strings.Contains(errOut, gieo.CardName) {
		t.Errorf("the error does not name what was missing: %s", errOut)
	}
}

func TestSayingNothingToGieoAsksForTheUsage(t *testing.T) {
	_, errOut, code := exec(t, "gieo")
	if code != 2 {
...
func TestAGieoSubcommandNobodyWroteIsNamed(t *testing.T) {
	_, errOut, code := exec(t, "gieo", "sow")
	if code != 2 {
		t.Errorf("exit %d, want 2", code)
	}
