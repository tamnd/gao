...
var meanings = map[string]struct{ stage, meaning string }{
	"doc_id":         {"phoi", "blake3 of the normalized text, which is the document's identity: two documents with the same normalized text are the same document whichever path found them"},
	"raw_id":         {"gat", "blake3 of the bytes before extraction, which is what links this row back to the WARC record or the source file it came out of"},
	"text":           {"phoi", "the document text, normalized to NFC with canonical tone mark placement and legacy encodings already transcoded"},
	"schema_version": {"kho", "the version of this layout, carried per row because a store appended to across a pipeline upgrade holds two versions at once and a reader has to be able to tell"},

	"source":           {"gat", "which acquisition path produced the document, one of the six gao runs"},
	"source_locator":   {"gat", "where in that source it came from: shard and offset for an ingested corpus, file, offset and length for a WARC record"},
	"url":              {"gat", "the page it came from, canonicalized"},
	"host":             {"gat", "the host of that URL, which is the unit politeness, budgets and takedowns are all measured in"},
	"url_template":     {"gat", "the URL with its variable path and query parts replaced by placeholders, which is what the crawl budgets against and how a calendar trap reads as one URL rather than ten thousand"},
	"fetched_at":       {"gat", "when the document was fetched, in UTC milliseconds"},
	"media_type":       {"gat", "the media type the response declared, before extraction decided what to do with it"},
	"extractor":        {"gat", "name and semver of the extractor, because two documents extracted by different versions of the same extractor are not comparable"},
	"pipeline_version": {"kho", "semver of the cleaning pipeline that produced this row"},

	"http_status":     {"gat", "the status the fetch got, empty for a document that arrived through somebody else's corpus rather than through our crawl"},
	"robots_decision": {"gat", "what robots.txt said about this fetch, recorded per fetch rather than assumed from a global setting so that a consent question years later has an answer"},
	"robots_rule":     {"gat", "the rule that decided it, in the spelling the site wrote"},
	"robots_hash":     {"gat", "blake3 of the robots.txt the decision was made against, so the decision can be rechecked against the file rather than against the file as it is today"},
	"tdm_signals":     {"gat", "the machine readable text and data mining reservations the response carried, keyed by mechanism and holding what that mechanism said"},
	"consent":         {"gat", "what the page said about being kept and trained on, in one word, where empty means nobody asked rather than the page said yes"},

	"lang":       {"sang", "the language identifier's verdict, which is vie for everything in gao and is stored anyway because a column that is constant today needs no migration tomorrow"},
	"lang_score": {"sang", "how sure the identifier was"},
	"diacritics": {"sang", "present, absent or mixed, because Vietnamese written without tone marks is still Vietnamese and is still not the same distribution"},
	"translated": {"sang", "the machine translation detector's verdict, since translated Vietnamese reads as fluent to a metric and as wrong to a native speaker"},

	"gao_qual":    {"sang", "the quality classifier's score for this document"},
	"gao_edu":     {"sang", "the educational value score, which is what the gao-edu slice is selected on"},
	"hplt_bucket": {"gat", "the source corpus's own quality bucket where it had one, kept so gao's classifier can be compared against an independent one rather than only against itself"},
	"register":    {"gat", "the source corpus's own register label, kept for the same reason"},
	"heuristics":  {"sang", "the raw heuristic measurements rather than the verdicts, so the corpus can be refiltered at a different threshold without being recomputed from the text"},

	"dup_cluster":       {"xay", "which duplicate cluster the document belongs to, empty when it is in none"},
	"dup_cluster_size":  {"xay", "how large that cluster is"},
	"is_representative": {"xay", "marks the one document per cluster a deduplicated view keeps, with the rest left in the store because deduplication is tuned rather than maximized"},

	"pii_level": {"che", "how much personal data has been removed: none, the structured identifiers, or those plus addresses and identifying names"},
	"pii_types": {"che", "which kinds of personal data were found"},
	"pii_spans": {"che", "where they were found, present at levels 0 and 1 and absent at level 2, because shipping the offsets of redacted data alongside the redacted text re-identifies it"},

	"license_class":    {"luat", "the per document redistribution determination, stored by name so a file read without gao says restricted rather than 3"},
	"license_evidence": {"luat", "what determined that class, since a class without evidence is a guess"},

	"structure":       {"gat", "what the document is: article, forum thread, legal, thesis, gazette, transcript, which drives both the extraction handler and the mixture weights"},
	"n_chars":         {"phoi", "how many characters the text holds"},
	"n_syllables":     {"phoi", "how many Vietnamese syllables it holds, which is the unit that survives a change of tokenizer"},
	"n_tokens":        {"dem", "how many gao tokens it holds, under the tokenizer named in the manifest"},
	"contam_flags":    {"nhat", "which evaluation benchmarks this document overlaps, flagged rather than deleted so one store can serve a training run that excludes them and an analysis that counts them"},
	"upstream_fields": {"gat", "the source corpus's own metadata, verbatim, which is the difference between answering a provenance question later and having to ingest again"},
}
...
func Page() string {
	...
	p("# The gao record")
	p("<!-- Generated by `gao store schema -md`. The meanings live in kho/schema.go, beside the type they describe, and a test fails when this file falls behind them. -->")
