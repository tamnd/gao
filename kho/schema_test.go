...
func TestEveryPublishedColumnIsExplained(t *testing.T) {
	missing, stale := kho.Undocumented()
	if len(missing) > 0 {
...
func TestTheDocumentedColumnsAreTheOnesInTheFile(t *testing.T) {
	...
	for _, path := range kho.Columns(kho.SchemaFor(kho.Dataset{Text: true})) {
...
func TestEveryColumnSaysWhichStageFillsIt(t *testing.T) {
	stages := map[string]bool{
		"gat": true, "phoi": true, "sang": true, "xay": true,
		"dem": true, "che": true, "luat": true, "nhat": true, "kho": true,
	}
	for _, c := range append(kho.Schema(), kho.Nested()...) {
		if !stages[c.Stage] {
...
func TestThePublishedSchemaIsNamedForWhatItHolds(t *testing.T) {
	for _, d := range []kho.Dataset{{Text: true}, {Text: false}} {
		def := kho.Definition(d)
...
func TestTheDefinitionCoversTheColumnsAndTheirTypes(t *testing.T) {
	def := kho.Definition(kho.Dataset{Text: true})
