...
func TestTheCrawlAsPlannedKeepsUpWithItself(t *testing.T) {
	...
	if r.Held() >= r.Mark() {
		t.Errorf("steady state holds %s against a %s mark", fleet.Size(r.Held()), fleet.Size(r.Mark()))
	}
}
...
func TestKeepingUpOnRateIsNotTheSameAsFitting(t *testing.T) {
	...
	if r.Fits() {
		t.Fatalf("a twelve hour confirmation lag still fit in %s", fleet.Size(r.Mark()))
	}
...
func TestTheCrawlStopsBeforeTheDiskIsFull(t *testing.T) {
	...
	if r.Mark() >= r.Scratch() {
		t.Errorf("the mark is %s and scratch is %s, so there is no room for anything in flight",
			fleet.Size(r.Mark()), fleet.Size(r.Scratch()))
	}
...
func TestTheBacklogIsCountedInWholeFiles(t *testing.T) {
	...
	if got := r.Held() % r.Volume; got != 0 {
		t.Errorf("steady state holds %s, which is not a whole number of volumes", fleet.Size(r.Held()))
	}
	if r.Held() <= r.Volume {
