// Package stretch is giãn, to stretch: the context extension ladder, and whether
...
func Measure(name string, paths []string) (Pool, error) {
	...
	for _, path := range paths {
		read, err := store.ScanLengths(path, func(l store.Length) error {
			p.add(rungs, top, l)
			return nil
		})
		p.Read += read
...
func (p *Pool) add(rungs []Rung, top int, l store.Length) {
	p.Documents++
	if l.NTokens == 0 {
		p.Untokenized++
		return
