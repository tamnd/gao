...
func roster(t *testing.T) nhat.Roster {
	t.Helper()
...
// is its own test.
func pinned(t *testing.T) nhat.Roster {
	t.Helper()
	ros := roster(t)
	out := make([]nhat.Entry, len(ros.Benchmarks))
	copy(out, ros.Benchmarks)
	for i := range out {
		if out[i].Version == nhat.Unpinned {
...
func TestABoardIsTwoArmsAndNeverOneAverage(t *testing.T) {
	...
	translated, ok := b.Arm(nhat.Translated)
	if !ok {
		t.Fatal("a roster with six translated benchmarks on it produced no translated arm")
	}
	if native.Origin != nhat.Native || translated.Origin != nhat.Translated {
