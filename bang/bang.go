// Package bang is the release scoreboard, with the native benchmarks kept apart
...
func Read(ros nhat.Roster, scores []Score) Board {
...
func Origins() []string { return []string{nhat.Native, nhat.Translated, nhat.Neutral} }
...
func (b Board) Edge() float64 {
	native, ok := b.Arm(nhat.Native)
	translated, also := b.Arm(nhat.Translated)
	if !ok || !also {
		return 0
	}
...
func (b Board) Faults() []string {
	...
	if edge := b.Edge(); edge > MaxTranslationEdge {
		translated, _ := b.Arm(nhat.Translated)
...
func (b Board) Verdict() string {
	...
	native, _ := b.Arm(nhat.Native)
	translated, hasTranslated := b.Arm(nhat.Translated)
