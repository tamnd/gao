...
func shapeOf(f *parquet.File) (Counts, error) {
	...
	for _, col := range []struct {
		...
		{store.CharsColumn, &c.Chars},
		{store.SyllablesColumn, &c.Syllables},
		{store.TokensColumn, &c.Tokens},
	} {
...
// that did not run.
type Counting func(part store.Stored, i, of int, c Counts, moved int64, resumed bool)
...
// bytes moved.
func recountPart(ctx context.Context, s *Store, part store.Stored, moved int64) (Counts, int64, error) {
