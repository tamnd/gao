...
// spotOf reads one part out of the fake hub and checks it, which is what a
// real run does over the sample.
func spotOf(t *testing.T, s *hub, snap string, tok *Tokenizer) Spot {
	t.Helper()
	parts, err := s.client().Parts(t.Context(), snap)
	if err != nil {
	...
	got, err := SpotPart(t.Context(), s.client(), parts[0], tok)
	if err != nil {
...
func TestAColumnThatDoesNotDescribeItsTextIsCaught(t *testing.T) {
	...
	if m.Column != store.CharsColumn {
		t.Errorf("the mismatch names column %s, want %s", m.Column, store.CharsColumn)
	}
...
func TestARunWithoutATokenizerSaysWhichColumnsItChecked(t *testing.T) {
	...
	got := spotOf(t, s, snapshot, nil)
	if contains(got.Checked, store.TokensColumn) {
		t.Error("a run with no tokenizer says it checked the token column")
	}
	for _, want := range []string{store.CharsColumn, store.SyllablesColumn} {
...
func TestTheTokenColumnIsCheckedWhenThereIsATokenizerToCheckItWith(t *testing.T) {
	...
	if got.Mismatches[0].Column != store.TokensColumn || got.Mismatches[0].Stored != 0 {
...
func textlessPart(t *testing.T) *bytes.Reader {
	t.Helper()
	d, ok := store.Lookup("vietnamese-web-urls")
	if !ok {
...
// listing is a snapshot's worth of parts as the hub lists them.
func listing(n int) []store.Stored {
	out := make([]store.Stored, n)
	for i := range out {
		out[i] = store.Stored{Path: store.StagePath(snapshot, i, 0), Bytes: 700 << 20}
	}
	return out
}

func paths(parts []store.Stored) string {
