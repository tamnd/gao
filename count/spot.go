...
func SpotOf(name string, r io.ReaderAt, size int64, tok *Tokenizer) (Spot, error) {
	...
	s := Spot{Part: name, Columns: columns, Checked: []string{store.CharsColumn, store.SyllablesColumn}}
	if tok != nil {
		s.Checked = append(s.Checked, store.TokensColumn)
	}
	err = store.ScanRows(name, r, func(row store.Row) error {
		s.Stored.Documents++
		...
		bad := s.compare(row, store.CharsColumn, row.NChars, chars)
		bad = s.compare(row, store.SyllablesColumn, row.NSyllables, syllables) || bad
		if tok != nil {
			tokens := uint32(tok.Count(row.Text))
			s.Counted.Tokens += int64(tokens)
			bad = s.compare(row, store.TokensColumn, row.NTokens, tokens) || bad
		}
		if bad {
			s.Wrong++
		}
...
func SpotPart(ctx context.Context, s *Store, part store.Stored, tok *Tokenizer) (Spot, error) {
...
func (s *Spot) compare(row store.Row, column string, stored, counted uint32) bool {
	if stored == counted {
		return false
	}
...
func Sample(parts []store.Stored, k int, seed string) []store.Stored {
	if k <= 0 || len(parts) == 0 {
		return nil
	}
	ranked := make([]store.Stored, len(parts))
	copy(ranked, parts)
	rank := make(map[string]doc.Hash, len(parts))
	for _, p := range parts {
		rank[p.Path] = doc.SumString(seed + "\x00" + p.Path)
	}
	slices.SortFunc(ranked, func(a, b store.Stored) int {
	...
	slices.SortFunc(ranked, func(a, b store.Stored) int { return strings.Compare(a.Path, b.Path) })
	return ranked
}
...
func Planned(snapshot string, parts []store.Stored, documents int64, share, confidence float64, seed string) Plan {
	p := Plan{
		Snapshot:   snapshot,
		Parts:      len(parts),
		Documents:  documents,
