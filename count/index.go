...
type Indexing func(row store.Indexed, done, of int, moved int64)
...
func IndexOf(ctx context.Context, s *Store, note Indexing) (IndexReport, error) {
	...
	// the lock.
	rows := make([]store.Indexed, len(parts))
