...
// hub is enough of the Hub to read parts back out of: the listing and the
...
func newStore(t *testing.T) *hub {
	t.Helper()
	s := &hub{t: t, repo: store.Org + "/vietnamese-source-text", files: map[string][]byte{}}
	s.srv = httptest.NewServer(s)
	t.Cleanup(s.srv.Close)
	return s
}

func (s *hub) client() *Store {
...
// storeAtRealWindows is hub() without the test's small window, so that a test
// about which window a door opens with measures the real one.
func (s *hub) storeAtRealWindows() *Store {
...
func (s *hub) tree(w http.ResponseWriter, r *http.Request) {
...
func (s *hub) resolve(w http.ResponseWriter, r *http.Request) {
...
func (s *hub) put(snapshot string, file, part int, texts ...string) {
	s.t.Helper()
...
func (s *hub) putDocs(snapshot string, file, part int, docs ...*doc.Document) {
	s.t.Helper()
...
func TestKeysComeOutOfTheStoreWithoutTheCorpusComingWithThem(t *testing.T) {
	...
	got, err := KeysOf(t.Context(), s.client(), snapshot, filepath.Join(dir, "work"), out, nil)
	if err != nil {
...
func TestTheKeysAreTheDocumentsThatWereWritten(t *testing.T) {
	...
	dir := t.TempDir()
	out := filepath.Join(dir, "glotcc"+KeysExt)
	if _, err := KeysOf(t.Context(), s.client(), snapshot, filepath.Join(dir, "work"), out, nil); err != nil {
		t.Fatalf("KeysOf: %v", err)
	}
...
func TestReadingIdentitiesDoesNotMoveTheText(t *testing.T) {
	...
	s.mu.Lock()
	size := int64(len(s.files[store.StagePath(snapshot, 0, 0)]))
	s.mu.Unlock()

	dir := t.TempDir()
	if _, err := KeysOf(t.Context(), s.client(), snapshot, filepath.Join(dir, "work"), filepath.Join(dir, "k"+KeysExt), nil); err != nil {
		t.Fatalf("KeysOf: %v", err)
	}
...
func TestAnInterruptedPassPicksUpAtThePartItStoppedOn(t *testing.T) {
	...
	out := filepath.Join(dir, "glotcc"+KeysExt)
	first, err := KeysOf(t.Context(), s.client(), snapshot, work, out, nil)
	if err != nil {
		t.Fatalf("KeysOf: %v", err)
	}
	...
	second, err := KeysOf(t.Context(), s.client(), snapshot, work, out, nil)
	if err != nil {
...
func TestAPassReportsEveryPartAsItGoes(t *testing.T) {
	...
	var last int
	dir := t.TempDir()
	_, err := KeysOf(t.Context(), s.client(), snapshot, filepath.Join(dir, "work"), filepath.Join(dir, "k"+KeysExt),
		func(part store.Stored, i, of int, keys Keys, moved int64) {
...
func TestASnapshotWithNoPartsSaysSoRatherThanWritingAnEmptyAnswer(t *testing.T) {
	...
	dir := t.TempDir()
	_, err := KeysOf(t.Context(), s.client(), "hplt3-0123456789ab", filepath.Join(dir, "work"), filepath.Join(dir, "k"+KeysExt), nil)
	if err == nil {
...
func TestTheSnapshotsInTheStoreAreWhatHasBeenIngested(t *testing.T) {
	...
	got, err := s.client().Snapshots(t.Context())
	if err != nil {
...
func TestAPartsKeyFileIsNamedAfterThePart(t *testing.T) {
	if got, want := partKeys(store.StagePath(snapshot, 3, 7)), "f00003-p00007"+KeysExt; got != want {
...
func TestTheColumnDoorAsksForLessThanTheRowDoor(t *testing.T) {
	...
	if got := column.Window(); got != harvest.ColumnWindow {
		t.Errorf("the column pass opened with a %d byte window, want the %d byte column window", got, harvest.ColumnWindow)
	}
	...
	if got := rows.Window(); got != harvest.DefaultWindow {
		t.Errorf("the row pass opened with a %d byte window, want the %d byte default window", got, harvest.DefaultWindow)
	}
}
