...
func parent() *kho.Manifest {
	return &kho.Manifest{
		...
		Pipeline:        "0.1.0",
		Stages:          []kho.Stage{{Name: "phoi@0.1.0", ConfigHash: id("phoi")}},
		Counts: kho.Counts{
...
func TestASupersededParentMakesTheSliceStale(t *testing.T) {
	...
	head.Tombstones = []kho.Tombstone{{DocID: id("a"), Reason: kho.ReasonTakedown, RemovedAt: time.Date(2026, 8, 3, 0, 0, 0, 0, time.UTC)}}
