// Package lat publishes a release slice as a view over a sealed snapshot rather
...
// lives. When a takedown arrives, [github.com/tamnd/gao/kho.Remove] rewrites the
...
func (s *Slice) Counts() kho.Counts {
	c := kho.Counts{Tokenizer: s.Tokenizer}
	for _, m := range s.Members {
...
func (s *Slice) Query(d kho.Dataset) string {
