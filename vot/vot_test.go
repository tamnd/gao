...
func load(t *testing.T, name string) []vot.Step {
	t.Helper()
	steps, err := vot.ReadSteps("testdata/" + name + ".jsonl")
	if err != nil {
...
func read(t *testing.T, name string) vot.Curve {
	t.Helper()
	c := vot.ReadCurve(name, total, every, load(t, name))
...
func TestARealBlowupIsFoundAndPlacedOnTheRun(t *testing.T) {
	...
	if s.Band < s.Base*(1+vot.Rise) {
		t.Errorf("the band came back at %.4f, under the floor of %.4f", s.Band, s.Base*(1+vot.Rise))
	}
...
func TestACadenceThatMakesARewindTooExpensiveIsAFaultAboutTheCadence(t *testing.T) {
	c := vot.ReadCurve("vot-len", total, 5000, load(t, "vot-len"))
...
func TestTheRiseOnItsOwnWouldReportTheCleanRun(t *testing.T) {
	...
	var over int
	for i := vot.Window; i < len(steps); i++ {
		window := make([]float64, 0, vot.Window)
		for _, s := range steps[i-vot.Window : i] {
			window = append(window, s.Loss)
		}
		slices.Sort(window)
		if steps[i].Loss > window[len(window)/2]*(1+vot.Rise) {
			over++
		}
	}
...
func TestAHoleInTheLogIsNamedAtTheStepItStarts(t *testing.T) {
	in := load(t, "on-dinh")
	gapped := append(append([]vot.Step(nil), in[:200]...), in[240:]...)

	c := vot.ReadCurve("on-dinh", total, every, gapped)
...
func TestALogWithNoGradientNormOrNoLearningRateSaysWhatThatCosts(t *testing.T) {
	...
	c := vot.ReadCurve("vot-len", total, every, in)
...
func TestTheGradientNormSeparatesARealBlowupFromNoiseTheLossCannot(t *testing.T) {
	...
	slices.SortFunc(byGrad, func(a, b vot.Spike) int {
		switch {
		case a.Grad > b.Grad:
			return -1
		case a.Grad < b.Grad:
...
func TestALogTheProtocolCannotBeRunAgainstIsRefused(t *testing.T) {
	edit := func(f func([]vot.Step) []vot.Step) []string {
		in := f(load(t, "on-dinh"))
		return vot.ReadCurve("on-dinh", total, every, in).Blocking()
	}
	...
	for _, c := range []struct {
		...
		{"no run", vot.ReadCurve("", total, every, load(t, "on-dinh")).Blocking(), "does not say what run it came off"},
		{"no cadence", vot.ReadCurve("on-dinh", total, 0, load(t, "on-dinh")).Blocking(), "there is a detector here and no protocol"},
		{"no length", vot.ReadCurve("on-dinh", 0, every, load(t, "on-dinh")).Blocking(), "how long it is"},
		{"no steps", vot.ReadCurve("on-dinh", total, every, nil).Blocking(), "holds no steps"},
		{"a loss that is not a number", edit(func(in []vot.Step) []vot.Step {
			in[300].Loss = math.NaN()
			return in
		}), "step 3000 carries a loss that is not a number"},
		{"a step logged twice", edit(func(in []vot.Step) []vot.Step {
			in[300].Step = in[299].Step
			return in
		}), "step 2990 is logged twice"},
		{"a log that goes backwards", edit(func(in []vot.Step) []vot.Step {
			in[300].Step = 5
			return in
		}), "two runs concatenated"},
		{"a log past the end of the run", edit(func(in []vot.Step) []vot.Step {
			in[len(in)-1].Step = total + 1
			return in
		}), "one of the two numbers is from a different run"},
		{"too short to take a band off", edit(func(in []vot.Step) []vot.Step {
			return in[:100]
...
func TestTheSameLogReadTwiceGivesTheSameAnswer(t *testing.T) {
	...
	first := vot.ReadCurve("phan-ky", total, every, in)
	second := vot.ReadCurve("phan-ky", total, every, in)
