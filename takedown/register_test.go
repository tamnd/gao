...
func read(t *testing.T, body string) *takedown.Register {
	t.Helper()
	g, err := takedown.Read(strings.NewReader(body))
	if err != nil {
...
func TestAnEmptyRegisterIsNotAPerfectRecord(t *testing.T) {
	g := read(t, "")
	if _, err := g.Median(); !errors.Is(err, takedown.ErrNothingFiled) {
		t.Errorf("an empty register reported a median: %v", err)
	}
	if _, err := g.Worst(); !errors.Is(err, takedown.ErrNothingFiled) {
...
func TestSomethingThatIsNotARegister(t *testing.T) {
	if _, err := takedown.Read(strings.NewReader("day khong phai toml [[[")); err == nil {
